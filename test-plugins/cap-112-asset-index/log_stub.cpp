// CAP-112 — standalone log stub for compiling the engine asset_index.cpp +
...
// OWN DLL (the cap-110/111 shape), so those log symbols are otherwise unresolved
...
// long), and the static KV::BareStr (asset_index's data_dir/pak-skip detail
// lines). pak_reader.cpp uses the first two; asset_index.cpp adds BareStr.
...
// The KV constructors + factory the two TUs' LOG_*_KV call sites use. The
