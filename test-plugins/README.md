...
| Test plugin | [`cap-33-author-targets/`](cap-33-author-targets/) — pure Lua + `targets.toml`. Declares a `pattern` target with a `signature` (`ui_pump_by_pattern`, the verified CGame_per_frame_ui_pump 25-byte entry AOB from seed.csv id 1003 — the §36 pattern-by-name row, **BLOCKED**), a verified-`address_id` target (`ui_pump_by_id`, id 1003 — same function by RVA, no scan) for the prefix + alias proofs, and a verified-`address_id` bytes target (`bool_leaf_safe_site`, lua_toboolean id 1124 — a DISTINCT verified leaf that NOTHING hooks, pristine prologue). |
| Auto-pass check | All five rows assert `:applied()` at `kcdx.on("ready")` (boot-only, no player input). **`CAP-33-pattern-by-name`** (THE §36 HEADLINE, **BLOCKED**): `kcdx.hook{ target="ui_pump_by_pattern" }` with NO `signature=` — the pure "author names a target BY AOB PATTERN and hooks it by name" proof. Blocked because the entry-prologue AOB is overwritten by cap-03's production hook on the same function, so the post-patch scan finds 0 (`patch_engine.cpp:124-130`); needs a verified `.text`-unique entry AOB for a function NOTHING entry-hooks. **`CAP-33-engine-tier`**: `kcdx.hook{ target="luaL_loadfile" }` (engine seed name) resolves — engine tier coexists with the author's own targets. **`CAP-33-prefixed`**: the explicit `"cap_33_author_targets.ui_pump_by_id"` (address_id=1003, RVA — immune to the prologue blocker) resolves directly. **`CAP-33-alias`**: `kcdx.alias("up", "cap_33_author_targets.ui_pump_by_id")` then `kcdx.hook{ target="up" }` resolves via the alias. **`CAP-33-bytes-by-name`**: `kcdx.bytes{ target="bool_leaf_safe_site", original="48", replacement="48" }` (address_id=1124; idempotent no-op — byte 0 of the lua_toboolean entry is 0x48, read from WHGame.dll; a DISTINCT verified leaf NOTHING hooks → pristine prologue, original-verify correct) → `:applied()==true` proves the author-target resolved to a writable VA. |
| Last result | ⏳ PENDING (engine path built+reviewed; the 4 repointed rows are in-suite PASS pending the verification launch; `CAP-33-pattern-by-name` is `[manual]`/blocked — see Notes) |
| Notes | The whole-feature regression for author-declared targets. `CAP-33-pattern-by-name` is the cornerstones §36 proof (a pattern site named ONCE hookable BY NAME with zero hex/ABI, the shareability guarantee) — **engine path built+reviewed; the row is `[manual]`/blocked on an unminted verified `.text`-unique entry AOB for an UNHOOKED function** (a hook overwrites the prologue the AOB matches): see [`docs/outstanding-work/section36-pattern-target-aob.md`](../docs/outstanding-work/section36-pattern-target-aob.md). The other four rows were repointed at verified `address_id` locators (RVA, no scan) so they prove their actual resolution path and pass. NEW plugins use VALID bare `[plugin].name` (`cap_33_author_targets`, charset `[a-z0-9_]`) — a target/alias-using plugin with the OLD illegal `kcdx.cap-NN-...` name is rejected by `RegisterAuthorTarget`/`RegisterAlias` (`naming-namespaces.md`). |
...
| Field | Value |
|---|---|
| Scenario | Two plugins (A + B) EACH declare a target with the SAME bare name `combat_check` in their own `targets.toml`. A's is a GOOD locator (CGame_per_frame_ui_pump by `address_id=1003` — verified RVA + ABI; located by id, no scan, so it resolves and applies regardless of any hook on the function); B's is a DELIBERATELY-BOGUS non-matching pattern. A third reference — here plugin A itself — hooks the bare name `combat_check`. The bare collision must resolve by self > engine > other (`naming-namespaces.md`): the CALLING plugin's OWN target wins. |
...
| Last result | ⏳ PENDING (engine path built+reviewed; in-suite PASS pending the verification launch) |
| Notes | The cross-plugin precedence proof for author-declared targets (COMP companion to CAP-33). A self-wins is value-distinguishable: A's GOOD locator (CGame_per_frame_ui_pump by `address_id=1003` — resolves + applies; located by id so no scan, immune to the entry-prologue-overwrite problem an entry AOB would hit) vs. B's BOGUS pattern (would fail to apply if it leaked), so `:applied()==true` cannot false-pass. B's target carries a signature so its only failure mode is "pattern did not resolve", never "missing ABI". |
...
| CAP-30-export | ✅ LIVE | cap-30 | `kcdx.code` export-symbol interlock (asserted at `ready`): `kcdx.code{ export="kcdx.cap-30-lua-code.region" }` publishes the address (`symbols::Register`); a deferred `kcdx.bytes{ target_symbol="kcdx.cap-30-lua-code.region", original="90", replacement="90" }` (NOP-over-NOP, idempotent) resolves it at the apply pass; PASS iff the handle `:applied()==true` — proving `target_symbol` resolved the export to the live region. No Lua symbol-resolve surface exists (`kcdx.addr` is the Address Library snapshot, not the runtime symbols table), so `kcdx.bytes` is the consumer (`cap-30-lua-code`) |
...
| CAP-33-pattern-by-name | ⛔ [manual] BLOCKED | author-targets | THE §36 HEADLINE: `kcdx.hook{ target="ui_pump_by_pattern" }` with NO `signature=` — the pure "author names a target BY AOB PATTERN and hooks it by name" proof. Engine path built+reviewed; BLOCKED on an unminted verified `.text`-unique entry AOB for a function NOTHING entry-hooks (a hook overwrites the prologue the AOB matches — `patch_engine.cpp:124-130`); see [`docs/outstanding-work/section36-pattern-target-aob.md`](../docs/outstanding-work/section36-pattern-target-aob.md) (`cap-33-author-targets`) |
| CAP-33-engine-tier | ⏳ PENDING | author-targets | `kcdx.hook{ target="luaL_loadfile" }` (engine seed name) still resolves — engine tier of self>engine>other coexists with the author's own targets. Engine path built+reviewed; in-suite PASS pending the verification launch (`cap-33-author-targets`) |
| CAP-33-prefixed | ⏳ PENDING | author-targets | the explicit `"cap_33_author_targets.ui_pump_by_id"` (address_id=1003, RVA — immune to the prologue blocker) resolves directly. Engine path built+reviewed; in-suite PASS pending the verification launch (`cap-33-author-targets`) |
| CAP-33-alias | ⏳ PENDING | author-targets | `kcdx.alias("up", "cap_33_author_targets.ui_pump_by_id")` + `kcdx.hook{ target="up" }` resolves via the local alias to the verified-id target. Engine path built+reviewed; in-suite PASS pending the verification launch (`cap-33-author-targets`) |
| CAP-33-bytes-by-name | ⏳ PENDING | author-targets | `kcdx.bytes{ target="bool_leaf_safe_site" }` (lua_toboolean, address_id=1124 — a DISTINCT verified leaf NOTHING hooks → pristine prologue) resolves to a writable VA; idempotent no-op write (0x48 over 0x48, byte 0 read from WHGame.dll) applies. Engine path built+reviewed; in-suite PASS pending the verification launch (`cap-33-author-targets`) |
| COMP-12-self-wins | ⏳ PENDING | author-targets | cross-plugin author-target name collision: A + B both declare bare `combat_check`; A's is the GOOD locator (CGame_per_frame_ui_pump by address_id=1003, RVA — resolves+applies), B's a BOGUS pattern. A hooks the bare name and self>engine>other picks A's OWN target — applied()==true (B's bogus pattern would fail if it leaked). Bare-collision NAMESPACE warn observable [manual]. Engine path built+reviewed; in-suite PASS pending the verification launch (`comp-12-target-collision`) |
