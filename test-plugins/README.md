...
| Auto-pass check | **`CAP-38-cpp-gate-proceeds`** (C++, at `kcdxPlugin_PostGameLoad`, boot-only): install on the named target + wrong sig returns a non-zero handle with `IsApplied()==true` (the cap-33/34/35 install-is-the-proof idiom, same as the Lua peer). FALSIFIABLE against a hypothetical (a)-reject impl: a reject gives handle 0 / IsApplied false → row FAILS. (The `before` observer's firing is reported as an informational liveness note, NOT a pass gate — the apply pass installs the hook only a few ms before `PostGameLoad` runs, so whether `lua_settable` is called in that window is incidental boot timing, not the contract.) **`CAP-38-lua-gate-proceeds`** (Lua, at `kcdx.on("ready")`, boot-only): `kcdx.hook{ target="kcdx.lua_settable", signature="void (ptr L)", before=... }` returns a handle whose `:applied()==true` (the install proceeded — the cap-33/34/35 install-is-the-proof idiom; a reject would leave `applied()==false` + non-empty `:reason()` → FAIL). The before observers mutate nothing and do not skip the original (the wrong sig mis-describes the ABI — the gate's whole point — but the detour preserves `lua_settable`'s real registers, so it runs untouched: no Lua-state corruption). No player input. |
...
| CAP-38-cpp-gate-proceeds | ⏳ PENDING (assertion fixed, awaiting re-run) | sig-gate | sig-mismatch gate (C++ `kcdxHookInterface`): named target `kcdx.lua_settable` (verified `void (ptr L, i32 idx)`, id 1186) + WRONG explicit `opts.signature="void (ptr L)"` → the gate WARNs + PROCEEDS with the explicit sig (behavior-c). At `PostGameLoad`: handle != 0 && `IsApplied()==true` (install-is-the-proof; observer-fired is an informational note, not a gate). Falsifiable vs an (a)-reject impl (handle 0 / IsApplied false → FAIL). NOTE: first run (`18f5e6a`) FLAKED FAIL because the original assertion also required the before-observer to have fired by `PostGameLoad`, which races the apply pass (hook installed at .060, report at .062 — `lua_settable` not called in that 2ms window). Assertion corrected to install-proceeded (parity with the Lua peer); the gate itself worked (WARN fired, IsApplied=1) (`cap-38-sig-mismatch-gate`) |
