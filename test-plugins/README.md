...
| Last result | ⏳ PENDING (probe — cross-engine coexist is UNVERIFIED; the first launch confirms whether the kcdx.bytes patch engine + the kcdx.hook hook_chain coexist on one site like the legacy `[[patch]]`+`[[hook]]`). |
