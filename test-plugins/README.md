...
| Auto-pass check | **`CAP-38-cpp-gate-proceeds`** (C++, at `kcdxPlugin_PostGameLoad`, boot-only): install on the named target + wrong sig returns a non-zero handle, `IsApplied()==true`, AND a `before` observer fired (`lua_settable` is called all over boot → the hook is live and marshalling per the explicit 1-arg sig). FALSIFIABLE against a hypothetical (a)-reject impl: a reject gives handle 0 / IsApplied false / observer never fires → row FAILS. **`CAP-38-lua-gate-proceeds`** (Lua, at `kcdx.on("ready")`, boot-only): `kcdx.hook{ target="kcdx.lua_settable", signature="void (ptr L)", before=... }` returns a handle whose `:applied()==true` (the install proceeded — the cap-33/34/35 install-is-the-proof idiom; a reject would leave `applied()==false` + non-empty `:reason()` → FAIL). The before observers mutate nothing and do not skip the original (the wrong sig mis-describes the ABI — the gate's whole point — but the detour preserves `lua_settable`'s real registers, so it runs untouched: no Lua-state corruption). No player input. |
...
| CAP-38-cpp-gate-proceeds | ⏳ PENDING | sig-gate | sig-mismatch gate (C++ `kcdxHookInterface`): named target `kcdx.lua_settable` (verified `void (ptr L, i32 idx)`, id 1186) + WRONG explicit `opts.signature="void (ptr L)"` → the gate WARNs + PROCEEDS with the explicit sig (behavior-c). At `PostGameLoad`: handle != 0 && `IsApplied()==true` && before observer fired. Falsifiable vs an (a)-reject impl (handle 0 / IsApplied false → FAIL) (`cap-38-sig-mismatch-gate`) |
| CAP-38-cpp-gate-warn | ⏳ PENDING [manual] | sig-gate | the C++ gate-WARN log line. Orchestrator greps the engine log for category `HOOK_SIG_GATE`, action `explicit_overrides_verified`, `plugin=cap_38_sig_mismatch_gate`, `explicit_sig="void (ptr L)"`, `verified_sig="void (ptr L, i32 idx)"`, `used=explicit`. Pre-fix (silent trust) NO line fires → FAIL. The falsifiable signal the gate exists (`cap-38-sig-mismatch-gate`) |
| CAP-38-lua-gate-proceeds | ⏳ PENDING | sig-gate | sig-mismatch gate (Lua `kcdx.hook`): named target `kcdx.lua_settable` + WRONG explicit `signature="void (ptr L)"` → gate WARNs + PROCEEDS. At `kcdx.on("ready")`: `h:applied()==true` (install-is-the-proof). Falsifiable vs an (a)-reject impl (`applied()==false` + non-empty `:reason()` → FAIL) (`cap-38-sig-mismatch-gate-lua`) |
| CAP-38-lua-gate-warn | ⏳ PENDING [manual] | sig-gate | the Lua gate-WARN log line. Orchestrator greps category `HOOK_SIG_GATE`, action `explicit_overrides_verified`, `plugin=cap_38_sig_mismatch_gate_lua` (same keys as the C++ row, distinguished by `plugin`). Pre-fix NO line fires → FAIL (`cap-38-sig-mismatch-gate-lua`) |
