...
| Engine status | LIVE-PENDING (zone_gate feature, sub-1 `448d9ff` two-flag + sub-2 `5828373` gating engine + wiring + log enrichment + sub-3 `a3bd3df` synthetic stub + `kcdx.plugin.is_rejected` + docs + this sub-4 comp-13 regression). Awaiting checkpoint launch. |
...
| Last result | ⏳ PENDING (the assertion fires at "ready" after the apply pass; verified at the next checkpoint launch). |
