...
| Engine status | LIVE-PENDING (kcdx.cosave feature) — binder `src/lua_bind_cosave.cpp` + value codec `src/lua_cosave_serial.cpp` over the existing `kcdxSerializationInterface`. This row is the regression test only. Awaiting checkpoint launch. |
