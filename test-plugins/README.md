...
| Engine status | LIVE — the kcdx `package.loaders` searcher + per-require `RegisterScriptOwner` (commits `7640fde` probe + `3371de0` searcher). |
...
| Last result | ✅ LIVE (kcdx-dev 14:32, suite 47/53; `require` resolved `helper.lua` via `package.loaders` entry #5, publish from the deferred callback stamped `kcdx.cap-25-multifile-attribution:multifile_event` → 1 subscriber → PASS) |
