...
| Engine status | LIVE-PENDING (per-entry-zone execution model step 6) — `src/lua_require_searcher.{h,cpp}` (kcdx require closure + `<owner>:<modname>` namespaced cache as a registry-ref'd GC Lua table + shared kcdx env table with `__index`→`_G` + `ResolveAndCompile`) + the entrypoint-fenv set in `src/lua_plugin_loader.cpp::LoadOneFileGuarded`. Awaiting checkpoint launch. |
...
| Engine status | LIVE-PENDING (per-entry-zone execution model step 7) — no engine change; uses the `[entrypoints].lua` + `.lua_after` slots (built earlier in this feature) over the existing `kcdx.publish` / `kcdx.on` (sub-9) cross-plugin pub/sub. Awaiting checkpoint launch. |
