...
| Test plugin | [`cap-33-author-targets/`](cap-33-author-targets/) — pure Lua + `targets.toml`. Declares a `pattern` target with a `signature` (`loadfile_by_pattern`, the verified luaL_loadfile entry AOB from `_research/phase7-recon/lua-rvas-harvested.csv:20`) + a second `pattern` target for the bytes test (`loadbuffer_safe_site`, the luaL_loadbuffer entry AOB, line 19 — a DISTINCT function so the byte write does not overlap the hooked luaL_loadfile). Both are dormant lua-aux functions post-install (the safe-target reasoning of CAP-20-target), so the no-op detours / write are harmless. |
| Auto-pass check | All five rows assert `:applied()` at `kcdx.on("ready")` (boot-only, no player input). **`CAP-33-pattern-by-name`** (THE §36 HEADLINE): `kcdx.hook{ target="loadfile_by_pattern" }` with NO `signature=` → `:applied()==true` proves the author-declared PATTERN target supplied BOTH address and ABI by name (had the target carried no sig, the binder would have rejected). **`CAP-33-engine-tier`**: `kcdx.hook{ target="luaL_loadfile" }` (engine seed name) still resolves — engine tier coexists with the author's own targets. **`CAP-33-prefixed`**: the explicit `"cap_33_author_targets.loadfile_by_pattern"` form resolves directly. **`CAP-33-alias`**: `kcdx.alias("lf", "cap_33_author_targets.loadfile_by_pattern")` then `kcdx.hook{ target="lf" }` resolves via the alias. **`CAP-33-bytes-by-name`**: `kcdx.bytes{ target="loadbuffer_safe_site", original="48", replacement="48" }` (idempotent no-op — byte 0 of the luaL_loadbuffer entry is 0x48 per the AOB; a DISTINCT function from the hooked luaL_loadfile, so no hook/bytes overlap) → `:applied()==true` proves the pattern author-target resolved to a writable VA. |
| Last result | ⏳ PENDING (boot auto-pass; verified at the checkpoint launch) |
| Notes | The whole-feature regression for author-declared targets. `CAP-33-pattern-by-name` is the cornerstones §36 proof: a pattern site named ONCE is hookable BY NAME with zero hex/ABI at the call site, by anyone (the shareability guarantee). NEW plugins use VALID bare `[plugin].name` (`cap_33_author_targets`, charset `[a-z0-9_]`) — a target/alias-using plugin with the OLD illegal `kcdx.cap-NN-...` name is rejected by `RegisterAuthorTarget`/`RegisterAlias` (`naming-namespaces.md`). All targets are the dormant luaL_loadfile, chosen safe; the no-op hooks chain on one entry without conflict. |
...
| Field | Value |
|---|---|
| Scenario | Two plugins (A + B) EACH declare a target with the SAME bare name `combat_check` in their own `targets.toml`. A's is a GOOD locator (the verified luaL_loadfile entry AOB + ABI); B's is a DELIBERATELY-BOGUS non-matching pattern. A third reference — here plugin A itself — hooks the bare name `combat_check`. The bare collision must resolve by self > engine > other (`naming-namespaces.md`): the CALLING plugin's OWN target wins. |
...
| Last result | ⏳ PENDING (boot auto-pass; verified at the checkpoint launch) |
| Notes | The cross-plugin precedence proof for author-declared targets (COMP companion to CAP-33). A self-wins is value-distinguishable: A's GOOD locator (resolves + applies) vs. B's BOGUS pattern (would fail to apply if it leaked), so `:applied()==true` cannot false-pass. B's target carries a signature so its only failure mode is "pattern did not resolve", never "missing ABI". luaL_loadfile is the dormant safe target (CAP-20/CAP-33 reasoning). |
...
| CAP-33-pattern-by-name | ⏳ PENDING | author-targets | THE §36 HEADLINE: `kcdx.hook{ target="loadfile_by_pattern" }` with NO `signature=` applies — the author-declared PATTERN target supplied BOTH address and ABI by name (a pattern site named once, hookable by name with zero hex/ABI) (`cap-33-author-targets`) |
| CAP-33-engine-tier | ⏳ PENDING | author-targets | `kcdx.hook{ target="luaL_loadfile" }` (engine seed name) still resolves — engine tier of self>engine>other coexists with the author's own targets (`cap-33-author-targets`) |
| CAP-33-prefixed | ⏳ PENDING | author-targets | the explicit `"cap_33_author_targets.loadfile_by_pattern"` form resolves directly (`cap-33-author-targets`) |
| CAP-33-alias | ⏳ PENDING | author-targets | `kcdx.alias("lf", "cap_33_author_targets.loadfile_by_pattern")` + `kcdx.hook{ target="lf" }` resolves via the local alias (`cap-33-author-targets`) |
| CAP-33-bytes-by-name | ⏳ PENDING | author-targets | `kcdx.bytes{ target="loadbuffer_safe_site" }` resolves the PATTERN author-target (luaL_loadbuffer, distinct from the hooked luaL_loadfile — no overlap) to a writable VA; idempotent no-op write (0x48 over 0x48) applies (`cap-33-author-targets`) |
| COMP-12-self-wins | ⏳ PENDING | author-targets | cross-plugin author-target name collision: A + B both declare bare `combat_check`; A hooks the bare name and self>engine>other picks A's OWN (good) target — applied()==true (B's bogus pattern would fail if it leaked). Bare-collision NAMESPACE warn observable [manual] (`comp-12-target-collision`) |
