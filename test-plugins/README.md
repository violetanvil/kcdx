...
| Test plugin | [`cap-33-author-targets/`](cap-33-author-targets/) — pure Lua + `targets.toml`. Declares a `pattern` target with a `signature` (`ui_pump_by_pattern`, the verified CGame_per_frame_ui_pump 25-byte entry AOB from seed.csv id 1003 — the §36 pattern-by-name row, **BLOCKED**), a verified-`address_id` target (`luaopen_math_by_id`, id 1172 — a DISTINCT verified leaf that NOTHING entry-hooks, called once at Lua init; by RVA, no scan) for the prefix + alias proofs, and a verified-`address_id` bytes target (`bool_leaf_safe_site`, lua_toboolean id 1124 — a DISTINCT verified leaf that NOTHING hooks, pristine prologue). |
| Auto-pass check | All five rows assert `:applied()` at `kcdx.on("ready")` (boot-only, no player input). **`CAP-33-pattern-by-name`** (THE §36 HEADLINE, **BLOCKED**): `kcdx.hook{ target="ui_pump_by_pattern" }` with NO `signature=` — the pure "author names a target BY AOB PATTERN and hooks it by name" proof. Blocked because the entry-prologue AOB is overwritten by cap-03's production hook on the same function, so the post-patch scan finds 0 (`patch_engine.cpp:124-130`); needs a verified `.text`-unique entry AOB for a function NOTHING entry-hooks. **`CAP-33-engine-tier`**: `kcdx.hook{ target="luaL_loadfile" }` (engine seed name) resolves — engine tier coexists with the author's own targets. **`CAP-33-prefixed`**: the explicit `"cap_33_author_targets.luaopen_math_by_id"` (address_id=1172, RVA — immune to the prologue blocker AND unhooked, so the install wins) resolves directly. **`CAP-33-alias`**: `kcdx.alias("up", "cap_33_author_targets.luaopen_math_by_id")` then `kcdx.hook{ target="up" }` resolves via the alias. **`CAP-33-bytes-by-name`**: `kcdx.bytes{ target="bool_leaf_safe_site", original="48", replacement="48" }` (address_id=1124; idempotent no-op — byte 0 of the lua_toboolean entry is 0x48, read from WHGame.dll; a DISTINCT verified leaf NOTHING hooks → pristine prologue, original-verify correct) → `:applied()==true` proves the author-target resolved to a writable VA. |
| Last result | ✅ PASS (kcdx@a35d339, 2026-05-23) — 5/6 rows PASS: CAP-33-engine-tier, CAP-33-prefixed, CAP-33-alias, CAP-33-bytes-by-name, COMP-12-self-wins all `:applied()==true` in the confirming launch (suite 65/74). **`CAP-33-pattern-by-name` is the sole expected red** — `[manual]`/blocked on the unminted AOB (see Notes). The prefix/alias/comp-12 hook rows were repointed from id 1003 (already entry-hooked by cap-03 → first-wins loss) to the unhooked id 1172. |
...
| CAP-33-engine-tier | ✅ PASS | author-targets | `kcdx.hook{ target="luaL_loadfile" }` (engine seed name) resolves — engine tier of self>engine>other coexists with the author's own targets. PASS kcdx@a35d339 2026-05-23 (`cap-33-author-targets`) |
| CAP-33-prefixed | ✅ PASS | author-targets | the explicit `"cap_33_author_targets.luaopen_math_by_id"` (address_id=1172, RVA — unhooked verified leaf, no scan) resolves directly. PASS kcdx@a35d339 2026-05-23 (`cap-33-author-targets`) |
| CAP-33-alias | ✅ PASS | author-targets | `kcdx.alias("up", "cap_33_author_targets.luaopen_math_by_id")` + `kcdx.hook{ target="up" }` resolves via the local alias to the verified-id target. PASS kcdx@a35d339 2026-05-23 (`cap-33-author-targets`) |
...
| CAP-33-pattern-by-name | ⛔ [manual] BLOCKED | author-targets | THE §36 headline (`kcdx.hook{ target="ui_pump_by_pattern" }`, no signature) — engine path built+reviewed, BLOCKED on an unminted verified `.text`-unique entry AOB for a function NOTHING entry-hooks (a hook overwrites the prologue the AOB matches; post-patch scan finds 0, `patch_engine.cpp:124-130`). See [`docs/outstanding-work/section36-pattern-target-aob.md`](../docs/outstanding-work/section36-pattern-target-aob.md). NOT silenced — reports its real failing `:applied()`. |
