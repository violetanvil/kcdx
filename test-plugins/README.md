...
| Auto-pass check | **`CAP-38-cpp-gate-proceeds`** (C++, reported at `kcdxMessage_InputLoaded`, boot-only): the gated install PROCEEDED (non-zero handle, `IsApplied()==true`) AND the hook actually FIRES per the explicit sig (the `before` observer ran) AND the after-phase export was dispatched (`PostGameLoad` ran). Reported at InputLoaded rather than `PostGameLoad` because the deferred apply pass installs the hook only a few ms before `PostGameLoad` — too early to have observed `lua_settable`; by InputLoaded it has been called thousands of times, so the full liveness proof holds without racing the apply. FALSIFIABLE against a hypothetical (a)-reject impl: a reject gives handle 0 / IsApplied false / observer never fires → row FAILS. **`CAP-38-lua-gate-proceeds`** (Lua, at `kcdx.on("ready")`, boot-only): `kcdx.hook{ target="kcdx.lua_settable", signature="void (ptr L)", before=... }` returns a handle whose `:applied()==true` (the install proceeded — the cap-33/34/35 install-is-the-proof idiom; a reject would leave `applied()==false` + non-empty `:reason()` → FAIL). The before observers mutate nothing and do not skip the original (the wrong sig mis-describes the ABI — the gate's whole point — but the detour preserves `lua_settable`'s real registers, so it runs untouched: no Lua-state corruption). No player input. |
...
| CAP-38-cpp-gate-proceeds | ⏳ PENDING (assertion fixed, awaiting re-run) | sig-gate | sig-mismatch gate (C++ `kcdxHookInterface`): named target `kcdx.lua_settable` (verified `void (ptr L, i32 idx)`, id 1186) + WRONG explicit `opts.signature="void (ptr L)"` → the gate WARNs + PROCEEDS with the explicit sig (behavior-c). Reported at `kcdxMessage_InputLoaded`: handle != 0 && `IsApplied()==true` && before observer fired && PostGameLoad ran (full liveness proof — install proceeded AND the hook fires per the explicit sig). Falsifiable vs an (a)-reject impl (handle 0 / IsApplied false / observer never fires → FAIL). NOTE: first run (`18f5e6a`) FLAKED FAIL — the original assertion required the observer to have fired by `PostGameLoad`, which races the apply pass (hook installed at .060, report at .062 — `lua_settable` not called in that 2ms window). Moved the report to InputLoaded (apply long done, `lua_settable` called thousands of times); the gate itself always worked (WARN fired, IsApplied=1) (`cap-38-sig-mismatch-gate`) |
