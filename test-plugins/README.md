...
| Last result | ⏳ PENDING (boot auto-pass; verified at the checkpoint launch) |
| Notes | The C++ parity proof of the kcdx.hook model — first non-Lua-binder consumer of the `kcdxHookInterface` v1 surface. Idiomatic shape inline-commented verbosely in `cap-36.cpp` so chunk 6's `Kcdx.h` wrapper takes notes from it (every kcdxHookOptions sets `opts.owningPlugin = g_self`; the raw-floor row specifically proves this works WITHOUT a wrapper that would stash it for the author). The raw `opts.address` locator is the right tier here because the stubs are DLL-internal (engine carries no name; declare-once / no shareability owed — `cornerstones.md` §"author-declared targets are SHAREABLE" applies to cross-plugin author-targets, not a plugin's own stubs). Stub-target idiom + ICF defeat + PostGameLoad-re-invoke pattern lifted from cap-20 / cap-21 / cap-22 / cap-29 (the established prior art). |
...
| CAP-36-cpp-hook-before | ⏳ PENDING | chunk 5 / Kcdx.h (step 6) | C++ DLL via **Kcdx.h** empowered helper `kcdx::hook::TryBefore<int(int), &cb>(K, nullptr, &opts)` (`opts.address` on a DLL-internal stub; owningPlugin + derived signature auto-threaded — B2 no-name path); cb is the NATURAL by-ref callback `void(int& seed){ seed += 1; }` (Kcdx.h emits the `void cFn(uintptr_t args[], int* outCount, /* typed args */)` adapter + writeback); PostGameLoad re-invokes the stub: `Cap36_Add_Before(10)==111` (`cap-36-cpp-hook-interface`) |
| CAP-36-cpp-hook-after | ⏳ PENDING | chunk 5 / Kcdx.h (step 6) | C++ DLL via **Kcdx.h** `kcdx::hook::TryAfter<int(int), &cb>`; NATURAL callback `int(int origReturn, int seed)` returns `origReturn+1000`; `Cap36_Add_After(10)==1110` (`cap-36-cpp-hook-interface`) |
| CAP-36-cpp-hook-around | ⏳ PENDING | chunk 5 / Kcdx.h (step 6) | C++ DLL via **Kcdx.h** `kcdx::hook::TryAround<int(int), &cb>`; NATURAL callback `int(int(*call_original)(int), int seed)` returns `2*call_original(seed)`; `Cap36_Add_Around(10)==220` — proves the typed call_original fn pointer arrival in RCX (D-c-fn-abi-2) through the wrapper's pass-through adapter (`cap-36-cpp-hook-interface`) |
| CAP-36-cpp-hook-replace | ⏳ PENDING | chunk 5 / Kcdx.h (step 6) | C++ DLL via **Kcdx.h** `kcdx::hook::TryReplace<int(int), &cb>`; NATURAL callback `int(int seed)` returns 42; original never runs; `Cap36_Add_Replace(10)==42` (`cap-36-cpp-hook-interface`) |
