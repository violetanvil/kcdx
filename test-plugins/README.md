...
| CAP-38-lua-gate-proceeds | ✅ PASS | sig-gate | sig-mismatch gate (Lua `kcdx.hook`): named target `kcdx.lua_settable` + WRONG explicit `signature="void (ptr L)"` → gate WARNs + PROCEEDS. At `kcdx.on("ready")`: `h:applied()==true` (install-is-the-proof). Falsifiable vs an (a)-reject impl (`applied()==false` + non-empty `:reason()` → FAIL). CONFIRMED `18f5e6a` (`cap-38-sig-mismatch-gate-lua`) |
