...
| Notes | Falsifiable broken-state: if `AddTask` ran the callback INLINE on the caller's (`Plugin_Load`) thread, the two thread IDs match → the row FAILs (logged at Error, a correctness failure per fail-state-logging.md). |
...
| Test plugin | [`cap-48-crash-attribution/`](cap-48-crash-attribution/) — MANIFEST-ONLY STUB (no `plugin.lua`, no DLL). Neither half can be driven from an author surface (one fires only inside the SEH filter on a real fault, the other only when `[record+0x80]` moves), so each row is held PENDING via `test_names` and confirmed by an orchestrator log-assert post-run — the cap-43 / cap-45 / cap-46 stub pattern + the cap-38 `-gate-warn` / cap-47 Part-1 `[manual]` log-assert convention. |
| Auto-pass check | Neither row auto-passes at boot (both are fail paths needing a real fault / a moved offset). **`cap-48-culprit-none`** [manual]: on a RaiseException-class (0xC8) fault whose culprit-walk finds no non-kernel frame, the GUARD log carries a `FAULTED_CULPRIT_NONE` line whose `reason` is one of `walk_exhausted` / `stack_unreadable` / `no_progress` (or, if a culprit IS found, a `FAULTED_CULPRIT` line — either way the fault is ATTRIBUTED, never blank). FALSIFIABLE: pre-fix the false return logged nothing → a blank GUARD section on an unattributable 0xC8 crash is FAIL. Confirmed by the live 0xC8 repro crash bundle. **`cap-48-null-base`** in-game: if a load resolves a non-null `record` whose `[record+0x80]` deref fails, the SAVE_LOAD log carries the new always-on `LOG_ERROR` naming the failed deref + the suppressed `LoadGameSelected` + the un-stamped playline. FALSIFIABLE: pre-fix the signal was `KCDX_DEV` (dev-gated) only → silent in production. Under the verified `release_1_5_1164953_841` offset the deref SUCCEEDS so the line correctly does NOT fire on a clean load; its absence-on-clean-load + presence-on-moved-offset is the falsifiable pair (an inverted assertion that expected it on every load would be the AP15 secondary shape). |
| Last result | ⏳ PENDING — `cap-48-null-base` confirmed in-game by loading a save (no Error line on a clean load under the current verified offset = correct; the line is the moved-offset alarm). `cap-48-culprit-none` confirmed by the next live 0xC8 repro crash bundle (a `FAULTED_CULPRIT` or `FAULTED_CULPRIT_NONE` line present in the GUARD section, never a blank). |
| Notes | Batch A of the approved fail-state-logging sweep. Both fixes are AP14 closures on the 0xC8 known-issue diagnostic trail (`docs/known-issues/save-load crash 0xC8 raised from WHGame.md`): Fix #1 makes the culprit-walk's three false returns legible (it complements cap-47 Part 1, which repaired the walk itself — a repaired walk that still finds nothing must say so); Fix #2 makes a moved-offset cosave-corruption risk loud in production where the `KCDX_DEV` line is invisible. Reason-threading mechanism for Fix #1 is a `const char**` out-param set to a fixed string literal (process-lifetime, no allocation — SEH-filter-safe), an implementation detail not a design choice. |
