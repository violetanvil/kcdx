...
| Channels | (vi) plugin Lua. C++ PARITY: the same `scan_engine::ResolveScan` resolve path is reachable from the TOML `[[scan]]` entry (scan-demo exercises it); the Lua verb is the surface mirror over the SAME engine path (`src/scan_engine.{h,cpp}` untouched). |
...
| Field | Value |
|---|---|
| What | The signature-resolution gate on the **named-target + explicit-`signature` conflict**. When a hook names a target that carries a verified Address-Library ABI AND the author ALSO passes an explicit `signature`, the engine USED to trust the explicit sig outright and never cross-check it against the verified ABI it had in hand — so a named target + a WRONG explicit signature was **silently accepted** (an AP12 footgun on the exact surface AP12 protects, AP13 if left deferred). The gate's core behavior is **(c) WARN + keep the explicit sig**: the engine consults the verified ABI to **detect** the conflict, emits a teaching WARN naming the target + both signatures + that the explicit one is used as-authored, then **proceeds** with the explicit sig (the deliberate-override case stays authoritative — the author may know better than the seed). This removes the SILENT part of the footgun without disabling override. Both surfaces gated: C++ `src/hook_interface.cpp` `ResolveSignature` + Lua `src/lua_bind_hook.cpp` signature resolution; the comparison is `kcdx::hook_signature::SignaturesCompatible` (declared in `hook_signature.h`, body in `hook_chain.cpp`). |
| Channels | (ii) C++ DLL + (vi) plugin Lua — BOTH surfaces tested (parity-is-tested, `lua-api-surface.md`). |
| Engine status | LIVE (this cycle) — the gate added to `ResolveSignature` (`src/hook_interface.cpp`) + the Lua signature-resolution branch (`src/lua_bind_hook.cpp`); `SignaturesCompatible` promoted from a `hook_chain.cpp` file-local to a `hook_signature.h`-declared surface (body still in `hook_chain.cpp`, minimal-blast). Closes the sig-mismatch-gate debt tracked in `docs/outstanding-work/restructure-plan.md`. |
| Test plugin pair | [`cap-38-sig-mismatch-gate/`](cap-38-sig-mismatch-gate/) (the C++ DLL — `kcdxHookInterface`) + [`cap-38-sig-mismatch-gate-lua/`](cap-38-sig-mismatch-gate-lua/) (the Lua sibling — `kcdx.hook`). Both name `kcdx.lua_settable` (engine seed id 1186, verified ABI `void (ptr L, i32 idx)`) and pass the deliberately-WRONG explicit signature `void (ptr L)` (1 arg → arg-count delta → NOT `SignaturesCompatible` → the gate fires). |
| Auto-pass check | **`CAP-38-cpp-gate-proceeds`** (C++, at `kcdxPlugin_PostGameLoad`, boot-only): the gated install PROCEEDED — non-zero handle, `IsApplied()==true` (the cap-33/34/35 install-is-the-proof idiom, identical to the Lua peer). FALSIFIABLE against a hypothetical (a)-reject impl: a reject gives handle 0 / IsApplied false → row FAILS. The row does NOT assert the detour FIRES: the gate requires a NAMED game target (only a seed carries a verified ABI to conflict against), and a plugin cannot safely invoke WHGame's `lua_settable` to trigger the detour — PROBE A (see the known-issue) established that a plugin-driven call raises a Lua error across the dual-Lua boundary (`lua-bridge.md`) and is uninstrumentable from the plugin side. C-dispatch FIRING is proven by cap-36's six own-function rows; install-proceeded is behavior-c's proof here. **`CAP-38-lua-gate-proceeds`** (Lua, at `kcdx.on("ready")`, boot-only): `kcdx.hook{ target="kcdx.lua_settable", signature="void (ptr L)", before=... }` returns a handle whose `:applied()==true` (the install proceeded — same idiom; a reject would leave `applied()==false` + non-empty `:reason()` → FAIL). The before observers mutate nothing and do not skip the original (the wrong sig mis-describes the ABI — the gate's whole point — but the detour preserves `lua_settable`'s real registers, so it runs untouched: no Lua-state corruption). No player input. |
| Manual confirm | **`CAP-38-cpp-gate-warn`** + **`CAP-38-lua-gate-warn`** [manual] (log-assert, the COMP-12 pattern): the orchestrator confirms the gate-WARN KV line fired in the engine log post-run. EXACT line — category `HOOK_SIG_GATE`, action `explicit_overrides_verified`, keys `target` (`kcdx.lua_settable`) / `plugin` (the owning plugin name) / `explicit_sig` (`void (ptr L)`) / `verified_sig` (`void (ptr L, i32 idx)`) / `used` (`explicit`). TWO lines fire — one per surface, distinguished by the `plugin` key (`cap_38_sig_mismatch_gate` for C++, `cap_38_sig_mismatch_gate_lua` for Lua). This is the row that catches a regression to SILENT-trust: pre-fix NO warn line fires → the manual row FAILS. Not auto-asserted because the post-fix behavior (install proceeds, handle non-zero) is byte-identical to pre-fix; only the WARN line distinguishes them. |
| Last result | ⏳ PENDING (auto rows boot-only; `[manual]` log-assert rows confirmed at the checkpoint launch by the orchestrator log read) |
| Notes | Closes the sig-mismatch-gate AP12/AP13 debt. Target choice (`kcdx.lua_settable`, id 1186) is a verified seed whose clean 2-arg ABI a 1-arg explicit sig cleanly mismatches (arg-count delta is the bluntest `SignaturesCompatible` failure). Hooking the hot `lua_settable` is safe: the before observers are pure (no arg mutation, no skip); the GAME's `lua_settable` (WHGame.dll @ 0x71E7C0) is what's hooked, while the Lua-callback dispatch uses kcdx's *vendored* Lua C-API (`lua_setfield`/`lua_rawseti` for the args table — different functions, different binary) and Lua bytecode `SETTABLE` lowers to `luaV_settable` (0x71CCF0), NOT the hooked C-API `lua_settable` — so the before callback firing does not re-enter the hook. The two surfaces install the SAME wrong sig, so they are `SignaturesCompatible` with each other and coexist on one chain (no chain-sig conflict between the halves); no production code hooks `lua_settable`, so neither install loses to a prior hook. |
...
| Last result | ⏳ PENDING (kind-tier fix landed; coexist re-launch confirms). First launch (2026-05-25) FAILED: the hook applied first (wrote its `E9` rel32 at the entry), then the patch's byte-verify ABORTED ("got E9, expected 48") — root cause was the apply-order comparator's name tiebreak ordering `comp02_hook` < `comp02_patch` at the same plugin priority. Fix: a kind tier in the `lua_registry` ApplyZone comparator orders `Kind::Bytes` before `Kind::Hook` at the same effective priority, so the patch writes pristine bytes first and the hook then detours the patched prologue — the legacy patch-before-hook coexist order. Re-launch confirms 2 entries [comp02_patch applied, comp02_hook applied]. |
| Notes | Phase 4b Batch 3 migration off the legacy `[[patch]]`+`[[hook]]` surface onto `kcdx.bytes` + `kcdx.hook`, on ONE site, in ONE plugin. The most common patch+hook coexistence case in real mods. The identity `48`→`48` patch is PRESERVED as a genuine no-op — the test is about the OVERLAP applying, not a behavior change. No `default_priority` needed: a patch + a hook in one plugin are not a replace-vs-replace conflict. `offset = -4` on a NAMED target is exercised on BOTH `kcdxBytesOptions.offset` ("applied after locator resolution") and `kcdxHookOptions.offset` ("applied after resolution"). |
...
| CAP-38-cpp-gate-proceeds | ✅ PASS (d8e4ac3) | sig-gate | sig-mismatch gate (C++ `kcdxHookInterface`): named target `kcdx.lua_settable` (verified `void (ptr L, i32 idx)`, id 1186) + WRONG explicit `opts.signature="void (ptr L)"` → the gate WARNs + PROCEEDS with the explicit sig (behavior-c). At `PostGameLoad`: handle != 0 && `IsApplied()==true` (install-is-the-proof, identical to the Lua peer). Falsifiable vs an (a)-reject impl (handle 0 / IsApplied false → FAIL). Does NOT assert the detour fires — the gate needs a NAMED game target, and a plugin cannot safely call WHGame's `lua_settable` to trigger it (PROBE A: a plugin-driven call raises a Lua error across the dual-Lua boundary, `lua-bridge.md`; cap-36's own-function rows own the C-dispatch FIRING proof). History: two timing theories (apply-race → InputLoaded) were killed before PROBE A established firing isn't plugin-observable on a named game target; see `docs/known-issues/cap-38 cpp before-observer never fires on a named game target.md` (`cap-38-sig-mismatch-gate`) |
| CAP-38-cpp-gate-warn | ✅ PASS [manual] | sig-gate | the C++ gate-WARN log line. Orchestrator greps the engine log for category `HOOK_SIG_GATE`, action `explicit_overrides_verified`, `plugin=cap_38_sig_mismatch_gate`, `explicit_sig="void (ptr L)"`, `verified_sig="void (ptr L, i32 idx)"`, `used=explicit`. Pre-fix (silent trust) NO line fires → FAIL. The falsifiable signal the gate exists. CONFIRMED `18f5e6a` (line fired at 11:59:07.375) (`cap-38-sig-mismatch-gate`) |
| CAP-38-lua-gate-proceeds | ✅ PASS (d8e4ac3) | sig-gate | sig-mismatch gate (Lua `kcdx.hook`): named target `kcdx.lua_settable` + WRONG explicit `signature="void (ptr L)"` → gate WARNs + PROCEEDS. At `kcdx.on("ready")`: `h:applied()==true` (install-is-the-proof). Falsifiable vs an (a)-reject impl (`applied()==false` + non-empty `:reason()` → FAIL). CONFIRMED `18f5e6a`, re-confirmed `d8e4ac3` (`cap-38-sig-mismatch-gate-lua`) |
| CAP-38-lua-gate-warn | ✅ PASS [manual] | sig-gate | the Lua gate-WARN log line. Orchestrator greps category `HOOK_SIG_GATE`, action `explicit_overrides_verified`, `plugin=cap_38_sig_mismatch_gate_lua` (same keys as the C++ row, distinguished by `plugin`). Pre-fix NO line fires → FAIL. CONFIRMED `18f5e6a` (line fired at 11:59:17.911) (`cap-38-sig-mismatch-gate-lua`) |
...
| COMP-02 | ⏳ PENDING | _migration_ | `kcdx.bytes` patch + `kcdx.hook` detour on ONE function-entry VA (id 1006, offset −4); cross-engine coexist PROBE. `GetConflictReport(entry VA)` must return exactly 2 — `comp02_patch`(kind=Patch,applied) + `comp02_hook`(kind=Hook,applied). PASS → patch engine + hook_chain coexist like legacy `[[patch]]`+`[[hook]]`; FAIL present-but-applied=0 → coexist breaks (engine gap/reframe); FAIL `<` both → GetConflictReport VA-match issue. UNVERIFIED until first launch (`comp-02-hook-on-patch`) |
