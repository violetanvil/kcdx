// CAP-115 — standalone overlay-SEAM stub for compiling the engine asset_index.cpp
...
//     SetTestOverlayMap below — kept EMPTY for cap-115 (the assertions are pure
//     vanilla-pak resolution; no overlay precedence is exercised here).
//
...
namespace {
// The test-controlled overlay map. Empty by default; cap-115 keeps it empty
// (the assertions are vanilla engine-pak resolution, no overlay).
OverlayMap g_testOverlay;
}  // namespace

const OverlayMap& GetOverlayMap() { return g_testOverlay; }

// Test-only seam control — NOT part of the engine asset_overlay surface. cap-115
// calls this to RESET the overlay map to empty before building the index.
