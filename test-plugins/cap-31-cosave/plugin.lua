...
--                          cyclic table inside on_save (rides the save gesture).
--
...
-- Reject-subtest results, captured inside on_save (the only place write()
-- reaches the serializer). Reported at on_load time alongside the round-trip.
local reject = {
...
-- we assert the failure and do NOT let it abort the rest of the save.
-- ============================================================================
...
-- round-trip + the reject sub-tests.
--
...
-- We report NOTHING in that case — the rows stay PENDING, not FAIL (mirrors
...
kcdx.cosave.on_load(function()
    ...
    if not saw_any_record then
        ...
        kcdx.log.info("CAP31",
            ...
            .. "(CAP-31-roundtrip + CAP-31-reject stay PENDING until then)")
        return
    end
...
kcdx.log.info("CAP31",
    ...
    .. "(boot-only); CAP-31-roundtrip + CAP-31-reject ride the save+reboot+load "
    .. "gesture")
