...
void MinHookBackend::set_instance(const std::string& hook_name, void* target, void* detour) {
    name_   = hook_name;
    target_ = target;
    detour_ = detour;
}

void MinHookBackend::enable() {
    if (enabled_) return;
    if (!target_ || !detour_) {
    ...
    // pOriginal — that's what RoM's JIT code reads via get_original().
