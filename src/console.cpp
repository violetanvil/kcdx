...
#if 0 // === ARCHIVED PROBE PRINTLINE_OVERLAY (2026-06-01): CONFIRMED — IConsole::PrintLine
      // paints the VISIBLE ~ console overlay. The marker KCDX_PRINTLINE_OVERLAY_PROBE_OK
      // appeared on the overlay as a clean plain line (no prefix, no syntax wrapper), and
      // the call fired (log: PRINTLINE_PROBE called rva=...8DFF08). The this+0x18 ring-buffer
      // IS the rendered overlay buffer. So kcdx.console.print wraps this method directly.
      // Revive by flipping #if 0 -> #if 1 to re-confirm overlay-paint after a game update.
