...
void* KcdxFOpenMarker(void* self, const char* pName, const char* szMode,
    ...
    // Delegate to the real kcdx FOpen: resolve via the unified index, open on
    // kcdx's CRT, mint a kcdx handle-id (§5). 0 = a failed open, read by the
    // engine as a null open as its own FOpen returns null.
