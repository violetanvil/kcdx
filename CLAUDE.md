...
- **`git push` → private (comprehensive); public is reached ONLY via `pwsh ./publish-public.ps1`.** Two remotes: `private` (`violetanvil/kcdx-private`, everything; `main` tracks it) and `public` (`violetanvil/kcdx`, sanitized projection). The private working trees (`.claude/`, `CLAUDE.md`, `_research/`, `third-party-ghidra/`, `test-fixtures/`, `docs/outstanding-work/`, `docs/known-issues/`) are tracked, NOT gitignored — so **never `git push public` / `git push --all` by hand**; that leaks them. The script sanitizes. See [concurrency-git.md](.claude/rules/concurrency-git.md).
