...
| Git concurrency — no auto-branch; parallel chats share one tree; destructive ops re-confirm live state (hooks `guard-destructive-ops.ps1` warn/block + `guard-force-push.ps1` auto-safen `--force`→`--force-with-lease`) | [concurrency-git.md](.claude/rules/concurrency-git.md) |
...
- **Probes are disabled + archived in place, never reverted or deleted.** When a probe's question is answered, the probe code stays in the source file wrapped in `#if 0 // === ARCHIVED PROBE <X> (<date>): <outcome verdict>. Root cause: <one-line mechanism>. See: docs/known-issues/<title>.md §Resolution. Revive by flipping #if 0 -> #if 1.` followed by `#endif`. Reverting (deleting the diagnostic site) throws away the cost-paid investigative wiring; the same probe shape often reappears in a future investigation, and the archived block is the cheapest jumping-off point. Migration to `_research/probe-archive/` is reserved for files exceeding 2 archived probes. The no-two-LIVE-probes-in-the-tree constraint (`results-driven.md` §"Probe-archive hygiene"; warn-only `guard-probe-stack.ps1`) is unchanged — only the cleanup verb changed (revert → disable + archive).
