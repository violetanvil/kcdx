"""app.routes_confirm -- the Confirm transaction + Cancel, the commit half of the
Save-previews / Confirm-transacts flow, built on the DIRECT-WRITE model.
...
waits. It re-sends the SAME edit body the matching Save previewed, but drives the
data-core DIRECT-WRITE path with defer_commit=True (not validate_only): the db_editor
write routes through apply_direct_edit (a real INSERT/UPDATE via _apply_one_db on the
held deferred-commit connections), returning a DeferredCommit handle. The transaction
opens AND closes inside this one request -- NOTHING is held across the maintainer's
think-time. On ANY failure NOTHING lands (the robust rollback below); on success the DB
+ the derived data/db-export/ CSV record are committed and pushed to the private remote
as the durable mirror.

THE SEQUENCE AS BUILT (read the two subtleties below)
-----------------------------------------------------
  1. resolve_tag(config, version_tag) -> version=(tag, ordinal)  [the version-tag
     adapter, no DLL].
  2. DIRECT-WRITE (deferred): the chosen db_editor write with defer_commit=True, version=,
     ...
     uncommitted connections). The write routes through apply_direct_edit; the
     ...
     extra). The handle ALSO carries the SCOPED restore-point: apply_direct_edit
  ...
  3. KEEP a pre-edit copy of the data/db-export/ CSVs (the backend's CSV-revert source --
     ...
     sqlite_sequence ONLY (the write semantics it owns); the db-export CSVs are a backend
     FILE artifact the backend reverts itself. A few KB; kept just before the irreversible
     commit so a pre-commit failure pays no copy cost.
  4. COMMIT the DB: data_core.commit(handle) -- USER-first then DEV (the settled
     ordering; re-raises on a DEV-commit split). IRREVERSIBLE (see below).
  5. EXPORT the committed DB -> the data/db-export/ CSV record (the derived diff layer;
     it is also the rebuild genesis now that the retired seeds dir is gone).
  ...
  7. GIT: stage the 3 data/db-export/ CSVs BY EXACT PATH (the git-tracked record; the
     DB is the local originator, NOT committed), author with the request-context
...
WHY THE COMMIT RUNS BEFORE THE EXPORT -- the export-visibility constraint
------------------------------------------------------------------------
...
uncommitted SQLite txn is invisible to every other connection -- verified in the commit
tests + read in csv_exporter.export_seeds, which does a fresh sqlite3.connect). So
exporting the in-transaction state from a fresh connection is impossible: it would read
the PRE-edit committed DB and produce stale CSVs. The integrity check is also
export-based, so it too must run post-commit. The order is forced: COMMIT first (the DB
is authoritative), THEN export the committed DB to the data/db-export/ record
(deterministic).

THE ROBUST ROLLBACK -- "on failure NOTHING lands" holds LITERALLY
----------------------------------------------------------------
Two mechanisms split at the irreversible DB commit:
  ...
  - POST-commit failure (the commit split, export, integrity, git stage/commit/push, or a
    ...
    the export MUST run post-commit (the visibility constraint above). So the SCOPED
    restore-point is the only undo left:
      * data_core.restore(handle) -- the DATA-CORE capability: it re-opens both DBs,
        restores the touched rows + each DB's sqlite_sequence from the capture the handle
        carries (taken before the commit), byte-identical incl. PK. It undoes the DB
        WRITE SEMANTICS the data-core owns -- it does NOT touch the CSVs.
      * the BACKEND reverts the data/db-export/ CSVs from the pre-edit copy kept before the commit
        -- the CSV-revert SPLIT: the CSVs are a backend FILE artifact, not the
        data-core's, so the backend owns reverting them. After both, the DB + db-export
        CSVs are byte-identical to before. A local git commit (if one landed before a push
        failure) is a dangling commit over the restored files; the next Confirm re-stages
        from the restored state. The durable mirror (the pushed remote) never advanced, so
        "nothing lands" holds for the durable record too.
WHY THE SCOPED RESTORE-POINT, NOT a full-file snapshot: an earlier draft snapshotted the
two DB FILES whole (shutil.copy2) -- the SAME guarantee, but it copies the ~1.3GB DEV DB
on every committing Confirm. The data-core's SCOPED restore-point is used instead (only
the O(edits) touched rows + the sequence -- a few KB), the cheaper mechanism for the same
guarantee. The restore-point is a data-core capability because it owns the write semantics
(which rows each job touches); putting it in the backend would leak that rule logic. So
Confirm CALLS data_core.restore(handle), and the full-file snapshot mechanism is dropped.

THE AUTH-READY SEAMS (the SEAMS, not the auth)
----------------------------------------------
...
def _resolve_author(name_field, email_field, header_name, header_email):
    """The commit author identity, request-context-first (the auth-ready seam).

    Priority: an explicit body field > the request header the operator's login populates
    > the CONFIGURED maintainer identity (KCDX_MAINTAINER_NAME/EMAIL env, else the
...
# ---------------------------------------------------------------------------
# Confirm request bodies -- the SAME edit shapes the Save preview took, each extended
# with the auth-ready identity context. Confirm re-sends the matching Save body + the
# optional author fields; the version_tag + record/edit fields are IDENTICAL to the
# preview (Confirm transacts what Save previewed).
# ---------------------------------------------------------------------------
class _AuthContext(BaseModel):
    """The auth-ready commit-identity seam: the operator's login layer populates these
    (or the X-Kcdx-Author-* headers); absent -> the dev default. The app builds no login
    -- it reads the identity the context supplies."""
...
# ---------------------------------------------------------------------------
# The BATCH confirm body (the bulk re-verify / close-intervals worklist). A LIST of
# per-row UPDATE specs (each mirroring the UpdateVersionSave shape: kcdx_id,
# valid_from_version, edits) + the version_tag they all resolve against + the auth-ready
# identity context. ALL-UPDATE -- no new-row approval gate (a batch re-verify never
# creates a row; a genuine new/variant row is authored individually via the per-row
# create flow). The N edits land as ONE atomic transaction (all-or-nothing -- one row
# failing rolls back the whole batch).
# ---------------------------------------------------------------------------
...
class ConfirmBatch(_AuthContext):
    ...
    of per-row UPDATE specs, and the auth-ready identity context. The whole list is ONE
    atomic transaction. version_tag is the resolution tag (the version-tag adapter, no
    DLL server-side); each row names its OWN edited row's identity in `rows`."""
    version_tag: str
...
def _run_confirm(body, *, entity_label, write, author):
    ...
    `author` is (name, email) from the request context.
    ...
    rollback); a post-commit failure calls data_core.restore(handle) (the scoped
    ...
    # 1. Resolve the chosen version tag -> (tag, ordinal). No DLL (the version-tag seam).
    #    An unknown tag is the maintainer's bad input -> 422 BEFORE any txn opens.
    try:
    ...
    # 2. DIRECT-WRITE under a held deferred-commit txn. Validation + version / baseline
    #    refusals gate BEFORE any DB open -- a rejected edit leaves NOTHING written and
    #    returns NO handle (handle stays None -> no rollback needed). A pre-commit failure
    #    here means the DB file never advanced -- "nothing landed" is already literal, no
    ...
    # the scoped restore-point (apply_direct_edit captured the touched DB rows +
    ...
    # (the data-core owns the write semantics); the db-export CSVs are the backend's to
    # revert (the CSV-revert split). KEEP a pre-edit copy of the three CSVs NOW (just
    # before the commit) so a post-commit failure reverts them to their pre-edit bytes. A
    # pre-edit failure above never reaches here -> pays no copy cost.
    ...
    try:
        ...
        #    ordering); a DEV-commit split re-raises. Whatever raised from here on, the
        ...
        try:
            ...
            # held txn is gone -- the deferred rollback cannot undo it. The restore-point
        ...
        # 5. EXPORT the COMMITTED DB -> the data/db-export/ CSV record (the derived diff
        #    layer; the retired seeds dir is gone). A fresh export connection now sees the
        ...
        #    the DB is the local originator, NOT committed), author with the
        ...
        # Post-commit integrity divergence -> roll back the whole Confirm: the restore-
        ...
        # A git stage/commit/push failed AFTER the DB commit. Robust rollback -> the
        # restore-point undoes the committed DB write + the backend reverts the db-export
        # CSVs: both byte-identical, the durable mirror (the remote) never advanced ->
        # nothing lands. (A local git commit that landed before a push failure now dangles
        # over the restored files; the next Confirm re-stages from the restored state.)
        log.warning("confirm git step failed (entity=%s, stage=%s): %s -- restoring the "
                    ...
                    exc.stage, exc)
        _robust_rollback(handle, csv_revert)
        # The maintainer-facing detail is GIT-FREE (git is invisible to the maintainer).
        # The git stage + the raw git stderr (exc) are the OPERATOR's diagnostic and stay
        # in the log.warning above; they NEVER reach the rendered detail. git_stage is a
        # structured field for the frontend's own logic, not a maintainer-rendered string
        # (the frontend shows `detail`, never git_stage).
        return _failed_response(
            entity_label,
        ...
        # restore + FAILED. Never leave the DB ahead (the robust-rollback guarantee).
    ...
    # DB-side restore-point is discarded with the now-committed handle). "Saved <entity>
...
def _run_confirm_batch(body, *, author):
    """Run the synchronous atomic BATCH Confirm transaction -- N version-row UPDATEs as
    ONE all-or-nothing transaction. This drives the SAME transaction spine as _run_confirm
    (resolve tag -> write onto ONE held handle -> commit -> export -> integrity -> ONE git
    commit; robust rollback on any failure), but the write step composes ALL N edits onto
    ONE DeferredCommit via the data-core's update_version_rows_batch -- so one row failing
    the shared validator rolls back the WHOLE batch (nothing partial lands, at batch
    scale). It does NOT weaken _run_confirm (the single-mutation path is untouched); it
    ADDS the batch path alongside, reusing _resolve_author / _robust_rollback / CsvRevert
    / _staged_rel_paths / git_commit -- the same robust-rollback + exact-path staging
    discipline.

    `author` is (name, email) from the request context. Returns the FastAPI response dict
    (the same saved/busy/failed shapes _run_confirm returns -- ONE result for the whole
    batch). ALL-UPDATE: no new-row approval gate (a batch re-verify never creates a
    row)."""
    config = load_config()
    author_name, author_email = author
    ...
    #    validates the COMBINED prospective DB state (every edited row -- the per-row gate
    #    at batch scale) BEFORE any DB open; one invalid row leaves NOTHING written and
    #    returns NO handle (the all-or-nothing guarantee at the validator stage).
    handle = None
    try:
    ...
    try:
        ...
        _robust_rollback(handle, csv_revert)
        # The maintainer-facing detail is GIT-FREE; git_stage is the structured operator
        # field (same contract as _run_confirm's git handler).
        return _failed_response(
            entity_label,
...
def _batch_commit_message(n_rows, version):
    """The batch commit message body (git is invisible to the maintainer; this is the
    durable record a reviewer reads in the private repo's history). Names the batch + its
    row count + the resolution version."""
    return (f"maintainer-tool: batch save {n_rows} version-row UPDATE(s) "
            ...
            f"maintainer tool batch Confirm (all-or-nothing; 3 db-export CSVs, "
            f"exact-path staged; the DB is the local originator, not committed).\n")
...
def _staged_rel_paths():
    ...
    (never a broad add): ONLY the three derived-export CSVs at data/db-export/. The
    retired seeds dir is gone -- the maintainer tool never wrote it and now nothing reads
    it. The DB is the ORIGINATOR -- it is amended locally at config.out_dir (data/) but is
    NOT git-tracked; only its derived CSV export is the git record (whose rejected
    alternative was DB-tracked-as-binary). Staging the DB is wrong: it contradicts the
    originator/derived-record split, and the real checkout gitignores the DB, so `git add`
    of it is rejected and rolls the whole transaction back. The data/db-export/ CSVs are
    the DB's git-tracked diff record -- the only files committed."""
    return [
        # The derived CSV record -- the export target (the DB is the local originator,
        # never committed).
...
def _robust_rollback(handle, csv_revert):
    """The POST-commit robust rollback -- the two-half undo run on ANY failure after
    data_core.commit(handle): (1) data_core.restore(handle) -- the SCOPED restore-point
    ...
    semantics); (2) csv_revert.revert() -- the backend reverts the data/db-export/ CSVs
    from the pre-edit copy (the CSV-revert SPLIT: the CSVs are a backend FILE artifact,
    not the data-core's). After both, the DB + db-export CSVs are byte-identical to before
    the Confirm -- "on failure nothing lands" holds literally. The DB restore runs FIRST
    (it is the authoritative state); the CSV revert second. discard() reaps the kept-copy
    snapshot dir after the revert (else a failed Confirm leaks a tempdir; the success path
    discards at the end of _run_confirm)."""
...
def _commit_message(entity_label, version):
    """The commit message body. Git is invisible to the maintainer (they read "Saved
    <entity> <version>", never a hash); this is the durable record a reviewer reads in
...
def _failed_response(entity_label, detail, *, git_stage=None):
    ...
    rejection (the held txn rolled back) AND a post-commit failure (the restore-point
    rolled the DB rows + sqlite_sequence back + the backend reverted the db-export CSVs --
    both byte-identical to pre-Confirm). The page renders "nothing landed" + the cause (the
    write-failure state). `committed` is False and `retry` is False in both cases -- the
    robust rollback makes "on failure nothing lands" literal, so there is no half-landed
    state for the maintainer to retry past."""
...
def _lock_busy_response(entity_label, detail):
    ...
    renders "Save blocked -- files locked, Retry"."""
...
def confirm_update_version(
    ...
    """Confirm a re-verify / full-column UPDATE: the atomic transaction."""
...
def confirm_create_version(
        ...
        x_kcdx_author_name: Optional[str] = Header(default=None),
        x_kcdx_author_email: Optional[str] = Header(default=None)):
    """Confirm a new version for an existing entity (adding a new row requires explicit
    maintainer approval; the addition commits here; the GUI gated the new-row marker at
    the preview)."""
    author = _resolve_author(body.author_name, body.author_email,
                             x_kcdx_author_name, x_kcdx_author_email)
    return _run_confirm(
...
def confirm_create_entity(
        ...
        x_kcdx_author_name: Optional[str] = Header(default=None),
        x_kcdx_author_email: Optional[str] = Header(default=None)):
    """Confirm a brand-new entity (adding a new entity requires explicit maintainer
    approval; the addition commits here)."""
    author = _resolve_author(body.author_name, body.author_email,
                             x_kcdx_author_name, x_kcdx_author_email)
    return _run_confirm(
...
def confirm_supersede(
        ...
        x_kcdx_author_name: Optional[str] = Header(default=None),
        x_kcdx_author_email: Optional[str] = Header(default=None)):
    """Confirm a supersession: an UPDATE -- no new-row approval required."""
    author = _resolve_author(body.author_name, body.author_email,
                             x_kcdx_author_name, x_kcdx_author_email)
    return _run_confirm(
...
def confirm_deprecate(
        ...
        x_kcdx_author_name: Optional[str] = Header(default=None),
        x_kcdx_author_email: Optional[str] = Header(default=None)):
    """Confirm a deprecation: an UPDATE -- no new-row approval required."""
    author = _resolve_author(body.author_name, body.author_email,
                             x_kcdx_author_name, x_kcdx_author_email)
    return _run_confirm(
...
def confirm_edit_notes(
        ...
        x_kcdx_author_name: Optional[str] = Header(default=None),
        x_kcdx_author_email: Optional[str] = Header(default=None)):
    """Confirm a notes edit: an UPDATE to the existing names row -- no new-row approval
    required."""
    author = _resolve_author(body.author_name, body.author_email,
                             x_kcdx_author_name, x_kcdx_author_email)
    return _run_confirm(
...
def confirm_batch(
        ...
        x_kcdx_author_name: Optional[str] = Header(default=None),
        x_kcdx_author_email: Optional[str] = Header(default=None)):
    """Confirm a BATCH of version-row UPDATEs (the bulk re-verify / close-intervals
    worklist) as ONE atomic transaction: all-or-nothing (one row failing the validator
    rolls back the WHOLE batch -- nothing partial lands), one git commit. ALL-UPDATE --
    no new-row approval required (a batch re-verify never creates a row; a genuine
    new/variant row is the per-row create flow)."""
    author = _resolve_author(body.author_name, body.author_email,
                             x_kcdx_author_name, x_kcdx_author_email)
...
# there is no held txn to roll back -- Cancel just acknowledges (on Cancel, nothing
# lands -- which is already true, because nothing was started).
# ---------------------------------------------------------------------------
@router.post("/cancel")
def cancel():
