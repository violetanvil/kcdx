"""app.routes_delta -- the field-delta endpoint (D8 + D12, design §7 / §10).

The confirm surface (s06) calls this to render the plain-language FIELD DELTA: given
the SAVED record + the prospective EDITED record, it returns the changed fields as
`field: old -> new` (D8 -- the human's acceptance signal; the literal CSV diff is not
exposed). For a NEW version it also returns the D12 "nothing changed" verdict (a new
version identical to its source steers the maintainer to re-verify, not duplicate).

THIN CALLER (D13 / R3 / design §5 law 6): the backend computes NOTHING. It deserializes
...
header order, so s06 lists the changed fields in a stable layout on every call (design §7
layout stability). A JSON object's key order is NOT guaranteed across the wire (a client's
...
WHY ONE ENDPOINT (the field delta + the D12 verdict together)
-------------------------------------------------------------
s06's "new row" state shows the field delta AND the nothing-changed steering at once. One
POST gives the frontend both in a single call: the delta is always returned; the D12 verdict
is folded in as an optional `nothing_changed` field, present only when the caller flags
`is_new_version=true`. Two endpoints would force s06 to make two round-trips for one screen.
"""
...
# address_names header (design D8 -- field_delta's field_order arg, US-8 lifecycle UPDATE).
...
class FieldDeltaRequest(BaseModel):
    ...
    requests the D12 nothing-changed verdict (default false -- an UPDATE does not need it).
...
def field_delta(req: FieldDeltaRequest):
    """The saved-vs-prospective field delta (D8) + the optional D12 verdict.
    ...
    returns "nothing_changed": <bool> from is_new_version_nothing_changed (D12 steering).
    ...
    constant. Computes nothing itself (D13/R3)."""
    ...
    if req.is_new_version:
        # D12: only meaningful for a NEW version (identical-except-valid_from -> steer the
