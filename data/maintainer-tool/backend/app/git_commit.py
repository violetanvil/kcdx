"""app.git_commit -- the Confirm transaction's git plumbing (the backend's OWN concern,
NOT data-core rule logic).

WHAT THIS OWNS (and what it does NOT)
-------------------------------------
The git commit + push half of the Confirm transaction: stage the three data/db-export/
CSVs by EXACT PATH (the DB is the local originator, NOT git-tracked; only its derived
CSV export is the git record), author the commit with the request-context identity, and
push to the PRIVATE remote with the env-injected credential. This module stages whatever
exact-path list the caller hands it; the caller (routes_confirm._staged_rel_paths)
decides the set. This is plumbing the backend owns -- it computes nothing the data-core
owns (validate / SQL / delta / round-trip are the data-core's; staging+committing+pushing
the resulting files is the backend's). It runs `git` via subprocess with `git -C
<checkout>` (NEVER `cd` -- a parked CWD silently breaks the session).

THE GIT DISCIPLINE (the load-bearing contract)
----------------------------------------------
  - EXACT-PATH STAGING: `git -C <checkout> add -- <csv> <csv> <csv>` -- only the three
    ...
    committed). NEVER `-A` / `.` / `-u` / `--all`: the index is shared across parallel
    chats, so a broad add sweeps another chat's in-flight files into this commit (the
    documented contamination vector). Staging only the named files is what keeps
  ...
  - PUSH TARGETS `private` ONLY: the everyday push target (the comprehensive private
    ...
    tree and leaks it; the public remote is reached only via a separate publish step.
    This module pushes exactly `git -C <checkout> push <PRIVATE_REMOTE> HEAD` -- the named
    private remote, the current HEAD, nothing else.
  - LIVE index.lock -> EVENT-DRIVEN on git's OWN exit, never reap, never poll: a
    ...
    (event-driven is the default). The lock is NEVER deleted (reaping a live lock = two
    writers in the index = corruption).
...
----------------------------------------------
  ...
  - The push CREDENTIAL is ENV-INJECTED: KCDX_PUSH_TOKEN (a documented env var). When
    ...
    without the operator's auth). When present, it is wired into the push via an
    ephemeral credential (an `http.extraheader` Authorization, never written to disk,
    never logged). This module reads os.environ; it provisions no credential.
...
rollback (the restore point), a git failure after the DB commit triggers the backend
restore point so the DB + db-export CSVs roll back to their pre-Confirm bytes ("on
failure nothing lands" holds literally). git is the durable mirror, and a failure to
update it rolls back the local commit too -- DB and git stay in lockstep.
"""
import logging
import os
import subprocess

# The PRIVATE remote -- the everyday push target (a bare push goes to private; the public
# remote is reached only via a separate publish step). Confirm pushes HERE, never to the
# public remote. Named once so the push target is auditable.
PRIVATE_REMOTE = "private"

# The env var the operator wires the push credential to (the auth-ready seam). A GitHub
# token (PAT / installation token) the operator's deployment injects. ABSENT -> the push
# is skipped (the dev default -- commit locally, boot without auth). The app provisions NO
# credential; it reads this name.
...
# poll and NO reap. git's message is stable across versions ("Unable to create
# '.../index.lock': File exists"); match the lock-file token, case-insensitively, so a
# path/locale variant still classifies as the lock-contention case rather than a generic
# stage failure.
...
class GitCommitError(RuntimeError):
    ...
    retryable, the DB change is real); a push failure means the commit IS landed locally
    and a retry re-pushes the same content (git is idempotent)."""
...
class IndexLockBusy(GitCommitError):
    ...
    The Confirm surfaces this as "Save blocked -- files locked, Retry" (the save-result
    state); the lock is NEVER reaped (reaping a live lock corrupts the shared index).
    `.stage` is the git step that hit the lock (add / commit / push)."""
...
def _run_git(checkout_path, args, *, stage, env=None):
    ...
    (NEVER `cd` into the checkout -- a parked CWD silently breaks the session). A non-zero
    exit raises GitCommitError(stage) carrying git's stderr; the caller maps the stage to
    its recovery. A non-zero exit whose stderr is the index-lock signature raises
    IndexLockBusy(stage) instead -- EVENT-DRIVEN off git's OWN exit (no poll, no reap),
    surfaced as Retry."""
    ...
    if proc.returncode != 0:
        ...
        if _is_index_lock_stderr(stderr):
            ...
            # Log before raising -- name the stage + that the lock is held.
        ...
        # Every git failure is logged before it raises -- name the stage + git's own
        # stderr (NOT the token-bearing env). The push env is built per-call and never
        # logged (the credential stays out of logs, by construction).
...
def commit_and_push(checkout_path, rel_paths, *, message, author_name, author_email):
    ...
    to the PRIVATE remote -- the git half of the Confirm transaction.

    The order is deliberately stage -> commit -> push so the post-DB-commit window the
    caller composes around is minimal: the DB is already committed (the Confirm commit
    step) when this runs, so a failure HERE leaves the DB change real and is surfaced as
    ...
    #    audit trail records the same identity (the operator's login supplies it); a dev
    #    default is the caller's fallback. The message is read from stdin (-F -) so an
    #    arbitrary maintainer-facing body needs no shell escaping.
    ...
    # 4. Push to the PRIVATE remote -- the durable mirror. Skipped (dev default) when no
    #    env credential is present; the commit is already local + durable.
...
def _commit_with_message(checkout_path, message, author_name, author_email, env):
    ...
    context's. A nothing-staged commit would error -- but Confirm always stages a real
    DB+CSV delta, so an empty commit is itself a defect to surface, not to allow with
    --allow-empty."""
...
def _push_to_private(checkout_path):
    """Push HEAD to the PRIVATE remote with the env-injected credential. Returns
    (pushed: bool, skip_reason: str|None).

    NO credential in env -> SKIP (the dev default -- the app boots + commits locally
    without the operator's auth). The credential, when present, is injected as an
    ephemeral `http.extraheader` Authorization on the single push command (`git -c
    http.<remote-url>.extraheader=...`) -- never written to a config file, never logged.
    The push targets exactly `PRIVATE_REMOTE HEAD` -- never the public remote, never
    `--all`/`--mirror` (a hand push to the public remote leaks the private tree)."""
    token = os.environ.get(PUSH_TOKEN_ENV_VAR)
    if not token:
        # The dev default: no operator credential -> commit landed locally, push skipped.
        # This is the normal local/test posture, not a failure.
