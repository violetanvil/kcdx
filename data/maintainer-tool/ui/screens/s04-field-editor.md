...
edit (re-verify) and the full-column edit (general correction) are the same surface — what's
...
gutter + "was:" line + validation-error line always (stable layout). The grid scrolls if tall;
field positions are fixed across dirty/error transitions.
...
grid cell is removed — stable layout holds per SHOWN field, a hidden field reserves nothing). **Every
...
group tooltip). Tooltip content is SOURCED from the schema's documented column rules and comments
— domain facts, not invented.

## Contents
| Element | Component (Mantine) | Data bound | Intent emitted |
|---|---|---|---|
| `valid_from_version` | `field row (read-only)` | identity key | — (never editable; identity is read-only) |
...
| `verified_by` | `field row (editable)` → `text well`, **prefilled from the resolved identity, overrideable** | audit trio — the signer; on Confirm it is SENT as the request `author_name` (the git commit author) | `edit_field(...)` |
| `verified_date` | `field row (read-only)`, **shown only when the row is verified** (`last_verified_at_version` non-empty); system-set to today on verify | audit trio — a SYSTEM fact (when verification happened), never hand-typed | — (read-only, system-set) |
...
**Editable set:** everything on the row EXCEPT
`valid_from_version` (identity key, read-only) and the entity identity (`kcdx_id`,
...
re-find columns collectively. Content is sourced from the schema's documented column rules
(address kinds, survival columns, the audit-trio + evidence_kind rules) and the schema column
comments — not invented. The affordance is keyboard-reachable and the content available to a
screen reader (Mantine `Tooltip` aria; the trigger is a focusable button), via theme tokens.

**Dirty feedback (stable layout):** a changed field gets the `dirty` accent marker in its gutter
...
**Validation (single validator):** each field's validity comes from the shared validator (the
data-core, via the API), rendered inline in the reserved error line: an out-of-enum
`evidence_kind`/`kind`, a partial audit trio (the trio is all-set-or-all-null),
...
but the FE never surfaces a malformed-date entry path: `verified_date` is system-set,
read-only, never hand-typed, so the maintainer cannot author a bad date.)

**Edit-existing confirmation (confirm boundary):** opening an already-decided version for editing
...
**Verification context (advisory):** when the targeted version is not verified against a DLL
...
with the "I accept — save anyway" override carried into s06.

**The per-author static check.** When a **version-matching** DLL is linked
...
re-check). The badge's space is **always reserved** (no reflow when the verdict
appears/changes). The four verdicts (glyph+text, never color-alone):
...
- **Changed** — *"does NOT match the binary (`<what was observed>`)"* (the body hash differs /
  ...
  carries to s06.
- **Ambiguous** (callsite/anchor multiple hits) — the **warn-and-steer** verdict:
...
- **CannotCheck** — *"can't check this kind against the DLL (`<reason>`)"* (a `vtable_index`,
  ...
  transitive anchor-dependency case).

**No matching DLL linked → no badge** — the check is **unavailable**, the existing "Not verified
against a game DLL" advisory stands (the degraded state); authoring proceeds.
...
`content_hash` (the engine-computed BLAKE3 function fingerprint). That hash is
...
**evidence_kind from the check.** A passing static check (Unchanged) refines the
...
## Arriving from a failing report row — the per-field divergence diff
...
isolate a field); the per-field diff is the TOOL's, computed by re-running the per-author
static check above against the divergent build.

**The "What diverged" banner.** At the top of the editor, an advisory banner
(`Alert` — never blocks the edit) titled **"What diverged"** names that the maintainer
...
**'Actual' derives from the REPORT's DLL — the divergent build.** The diff derives the
...
**Per-field recorded-vs-actual (stable layout / non-color affordance).** For each KIND-RELEVANT
field the check covers (§"Field relevance by kind": `function` → `rva` + `signature`; `callsite` /
`instruction_anchor` → `survival_aob`; `string_anchor` → `survival_anchor_string`; `vtable_base` →
`survival_slot_count`), a field that diverged shows, inline in its reserved gutter (the same space
the dirty marker + "was:" line + verdict badge already reserve — no reflow): the **recorded** value,
the derived **actual** (what the linked build shows — the resolved site / body-hash result / `.text`
match count), and a **diverged marker** (glyph + text, never color-alone). This puts the diff
directly where the maintainer edits, answering "the actual field is clear showing what is different".
A kind-relevant field that did NOT diverge shows no marker (it matches the build). (Per-field
ATTRIBUTION is the build's thin new layer over `runVerdictCheck`, which returns one row-level verdict
per row; the attribution is unverified until a probe settles which kind-relevant field a row-level
divergence maps to before the inline diff lands.)

**The diverged-field box — red-boxed emphasis so the eye lands on the break.** A diverged
kind-relevant field is wrapped in a **diverged-field box**: an `error`-token
...
EXISTING reserved box, so it adds NO layout and shifts nothing (no reflow). It NEVER stands
alone as the divergence signal: the diverged glyph+text marker (above) is retained, so the cue is
glyph + text + box, never color-alone; the border + tint resolve to the `error` semantic
token, never a raw value. **Every diverged field is boxed equally** — when more than one
...
**The box persists through the edit until the fix is saved.** The diverged-field box
...
**The no-DLL-yet state.** On `[Fix ▸]` arrival before a suitable DLL is linked — the common
...
"no DLL → advisory, authoring proceeds" contract).

## Field relevance by kind
Each `kind` populates only certain columns; the rest stay NULL (the schema's address-kinds +
survival-columns rules are the AUTHORITY; the "Used by kind(s)" column is the
...
read-only (identity key). **`verified_date` is NOT in the always-shown set** — it renders
ONLY when the row is verified (`last_verified_at_version` non-empty), read-only and system-set;
an unverified row shows no `verified_date` cell at all.

**Per-kind used set** (the kind-specific fields shown; grounded in the schema's documented rules —
the map the build encodes in `frontend/src/editor/fieldModel.ts` `KIND_FIELD_RELEVANCE`):

| Kind | Fields shown (beyond always-shown) | Grounding |
|---|---|---|
| `function` / `function_variadic` / `function_no_sig` | `rva`, `signature` | a function entry has a verified signature; its survival datum is its body fingerprint, so NONE of the six survival columns apply (function kinds need no survival authoring) |
| `callsite` | `rva`, `offset`, `survival_aob`, `survival_expect_unique` | a call instruction, not a function entry → no signature; survival table: aob + expect_unique → callsite; offset = the consumer offset |
...
| `vtable_index` | `vtable_slot`, `survival_derives_from` | "no RVA; slot-only" → no rva, no signature; survival table: derives_from (→ its vtable_base) → vtable_index (its survival is DEFERRED) |
...
| `data_slot` | `offset`, `survival_rule`, `survival_derives_from`, `value` | a `.data` slot resolved by a derivation → no rva; survival table: rule + derives_from → data_slot; value = the authored datum |
...
- **View (no edits)** — all fields at saved values; `[Review changes]` disabled; read-only
  fields in the read-only treatment. Only the current kind's fields + populated strays render
  (kind-conditional, see §"Field relevance by kind").
...
- **Navigate-away while dirty — the unsaved-changes guard** — s04 is THE
...
- **Unverified / resolver-failure** — the advisory `warning banner` (`Alert`) + the override
  carried to save (verification is advisory).
- **Audit trio locked while unverified** — while the prospective
  ...
  verification fields fill in together." (text, not color-alone). This makes the all-or-null
  trio orphan unreachable by construction: a lone trio cell cannot be written (manually OR by the
  evidence_kind auto-suggest, which is suppressed on an unverified row). Setting the version runs the
  coupling (auto-fills the empty trio) and re-enables the inputs. `verified_date` is already
  read-only + hidden-when-unverified, so it is not in the locked set.
- **Check verdict states** (the `verdict badge`, when a version-matching DLL is linked):
  **no badge** (no matching DLL linked → degraded, the unverified advisory stands);
  **checking** (a brief loading state in the reserved badge region while the per-kind check runs
  over the DLL bytes — the region is reserved, no reflow); **Unchanged** ✓ / **Changed** /
  **Ambiguous** (+ `[show matches]` steer) / **CannotCheck** — each with its reserved detail
  ...
  advisory — `[Review changes]` is NOT gated on the verdict (only on validator
  validity); a Changed/Ambiguous verdict warns + carries the "I accept — save anyway"
  override to s06, it never disables save.
- **Arrived from a failing `[Fix ▸]` — no DLL linked** — the "What diverged" banner
  ...
  "actual" yet; the editor is fully usable (never blocked on the DLL).
- **Arrived from a failing `[Fix ▸]` — DLL linked, diff computed** — each diverged
  kind-relevant field shows recorded-vs-actual inline (diverged marker, glyph+text not
  color-alone) in its reserved gutter (no reflow) AND is wrapped in the
  **diverged-field box** (`error`-token border + faint tint around the field row) so the
  eye lands on the break; the marker is retained alongside the box (glyph + text + box, never
  color-alone), the box reuses the row's reserved space (no reflow), and the
  border/tint resolve to the `error` token. Every diverged field is boxed equally; the box
...
- **Arrived from a failing `[Fix ▸]` — no divergence found / cannot check** — the
...
- **Stable layout** — dirty marker, "was:" line, and error line reserve space always; no reflow.
- **Advisory verification** — unverified/resolver-failure is advisory + overridable, never a block.
- **Confirmed transaction** — editing an existing version crosses the confirm boundary; save is one
  transaction via s06.
- **Single validator** — every field's validity is the shared validator's verdict (via the API),
  rendered inline.
- **Read-only identity / non-color affordance** — `valid_from_version` read-only, conveyed by more
  than color.
- **Content back-stack** — navigating away from s04 while a field is dirty surfaces the
  unsaved-changes guard (Save/Discard/Cancel) before the navigation; nothing is saved or lost
  silently. s04 is the dirty-editor the back-stack's guard clause protects.
- **Semantic tokens only** — tokens only (the tooltip affordance, the stray flag, and the
  kind-conditional grid all use theme tokens — no raw hex/px). The `?` help affordance is a
  focusable button (keyboard + screen-reader reachable); the Mantine `Tooltip` wires the aria
  description.
...
- **Phone:** when reached from s02's selected row it renders inline within the detail
  ...
  error line keep their reserved space at every width (stable layout).
