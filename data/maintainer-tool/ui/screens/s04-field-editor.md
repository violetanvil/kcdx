...
## Purpose / when shown
The inline editor for one `address_versions` row, shown in the right pane below the
version table (s02) or from a compare column / history row (s03). The maintainer views the
row's full columns and edits the editable ones with live, layout-stable dirty feedback and
inline validation. The audit-trio edit (Job 2) and the full-column edit (general
correction) are the same surface — what's editable is the full row minus the identity key.

## Region & position
The right pane, below the version area. A vertical field list; each editable field reserves
its dirty-marker gutter + "was:" line + validation-error line always (law 1). The list
scrolls if tall; field positions are fixed across dirty/error transitions.

## Contents
| Element | Component | Data bound | Intent emitted |
|---|---|---|---|
| `valid_from_version` | `field row (read-only)` | identity key | — (never editable, law 7) |
| `module` | `field row (editable)` → `dropdown` | `module` (FK to `module_seed`) | `edit_field('module', v)` |
| `kind` | `field row (editable)` → `dropdown` | `kind` (the nine-kind enum) | `edit_field('kind', v)` |
| `rva` | `field row (editable)` → `text well` (mono) | `rva` | `edit_field('rva', v)` |
| `signature` | `field row (editable)` → `text well` (mono) | `signature` | `edit_field('signature', v)` |
...
| `verified_date` | `field row (editable)` → `text well` | audit trio (`YYYY-MM-DD`) | `edit_field(...)` |
| `evidence_kind` | `field row (editable)` → `dropdown` | audit trio (5-value ranked enum) | `edit_field(...)` |
| six survival columns | `field row (editable)` ×6 | `survival_aob` … `survival_expect_unique` | `edit_field(...)` |
| Revert-field | `ghost button` per dirty field | — | `revert_field(name)` |
| `[Review changes (N)]` | `primary button` | enabled when ≥1 dirty AND valid | `review_changes()` → s06 |
...
**Validation (law 6):** each field's validity comes from the shared validator
(`validators.py`), rendered inline in the reserved error line: a malformed `verified_date`
shape, an out-of-enum `evidence_kind`/`kind`, a partial audit trio (the trio is
all-set-or-all-null — `policy.md`), `last_verified_at_version < valid_from_version`, an
unresolvable `module` FK. The error names what's wrong; `[Review changes]` stays disabled
while any dirty field is invalid. No rule is reimplemented in the GUI.
...
**Verification context (law 4):** if no DLL is linked (s07), a `warning banner` notes
*"No game DLL linked — can't verify this row against a game version."* The edit still
proceeds; the warning is advisory. A resolver failure surfaces the same way with the
"I accept — save anyway" override carried into s06.
...
- **Unlinked / resolver-failure** — the advisory `warning banner` + the override carried
  to save (law 4).
...
- **Law 4** — unlinked/resolver-failure is advisory + overridable, never a block.
- **Law 5** — editing an existing version crosses the confirm boundary; save is one
  transaction via s06.
- **Law 6** — every field's validity is the shared validator's verdict, rendered inline.
