...
per-row verdicts — every curated DB row actively attempted in the running game).
The report is produced by the `kcdx_verify_all` console command the maintainer runs after loading
a save. The screen presents every reported row + its **7-state verdict** and the
...
on the failing rows — each in one batched, confirmed transaction (a mutation is one atomic,
confirmed transaction, here at batch scale) — and jump to fix a failing row. The report is read
**client-side via the File API** (the maintainer picks the report JSON in-page; no backend read
seam). The screen does NOT verify anything itself — it consumes verdicts the engine already
produced.

## Region & position
The main content area (right pane on wide; a drilled-in full view on phone), peer to s02/s03 —
NOT an overlay (a 157-row review session is a working surface, not a dim-and-dismiss overlay; the
navigation shell persists, only an overlay layer covers content). Reached from s01's `[Import
verification report]` affordance — a **top-level destination that RESETS the content back-stack**
to a fresh root at s08. The app shell persists around it. A `[Fix ▸]` **PUSHES** s02/s04 onto the
stack as a state-carrying frame, so `‹ back` from the fix returns to THIS s08 worklist with **the
ingested report intact** — the parsed pass/fail rows, the block split, and the scroll position all
restored, **no re-import** (the report is client-side only, so the state-carrying frame is what
preserves it). At s08's own stack root there is no `‹ back` (the navigator is the way out on wide;
on phone `‹ back` returns to the navigator home).
...
| Worklist table | `version table row` ×N (one per reported row) | `kcdx_id` · `name` (mono) · resolved version · matched `address_version` id · `live verdict badge` · `proof-rank chip` · detail | `select_row(kcdx_id)` (drills to fix) |
...
| No-action rows | `version table row` (no checkbox) | the 4 no-action verdicts — `not_applicable` / `cannot_check` / `skipped` / `error` | — (shown, no action) |
...
| `[Verify N rows]` | `button` (primary, verified block) | the selected verified rows | `bulk_verify(selected)` → s06 batch confirm |
| `[Close intervals · N rows]` | `button` (primary, failing block) | the selected failing rows | `bulk_close_intervals(selected)` → s06 batch confirm |
| A failing row's `[Fix ▸]` | `button` (subtle) | the failing row + its divergence `detail` | `fix_row(kcdx_id)` → PUSHES s02 / s04 onto the back-stack, CARRYING the row's `detail`; `‹ back` returns to this worklist, report intact |
| `‹ back` (content-pane, depth > 1) | `back affordance` | the stack | `nav_back()` → the screen that pushed s08; at root → navigator |

**The verdict (per row) — the 7-state enum + the proof rank.** The in-game sweep runs
...
both glyph+text never color-alone, with the matched `address_version` id shown for an
attributed row. The states:
...
- **`skipped`** — a precondition wasn't met THIS run (the live-exercise tier needed a loaded save
  ...
  names exactly why.
- **`error`** — the in-game verification sweep itself faulted on this row (distinct from `failed`:
  the ROW may be fine, the TEST blew up).
...
live-exercise tier — NOT a runtime "does the code still work" body hash (the hash is on-disk). The
static `Ambiguous` verdict is an s04 author-time outcome, NOT a report token — deliberately absent
from the report enum; s08 carries the live-pass verdicts only.

**Three blocks — two action, one no-action (the verdict→block mapping).** The worklist splits
into:
...
- a **no-action / informational block** — `not_applicable` / `cannot_check` / `skipped` / `error`
  ...
  here" — a version gap, a deferred kind, a precondition miss, a sweep fault). Rendered as a
  **`collapsible section`** (collapsed by default — it is informational, not the work surface), the
  rows fully visible + auditable when expanded, never silently dropped (the maintainer is final
  authority; never a silent success or silent loss).

**Report-vs-DB reconciliation — an already-acted-on row shows no-further-action.** The
...
action:** when a report is imported, the tool runs the `/save/reverify-batch` preview
...
existing skip). Nothing is hidden (the maintainer is final authority; never a silent loss — the
already-resolved rows are shown, just not actionable).

**Two batched bulk actions, each ONE batched confirm (a mutation is one atomic, confirmed
transaction, here at batch scale).** The verified block and the failing block each have their own
bulk action + s06 batch confirm. Neither auto-writes — the maintainer reviews the whole batch delta
and confirms once → ONE atomic transaction (validate → write → export → commit + push); both are
all-UPDATE so the new-row approval gate does NOT apply. The no-action block has no bulk action.
...
- **Verify-all** (verified block, `[Verify N rows]`) — every selected verified-block row's
  ...
  today`, `verified_by → <injected identity>`, `evidence_kind → <by the row's proof rank>` —
  ...
  `address_version` row (the gap-pass extension). A row already covered
...
- **Close-intervals** (failing block, `[Close intervals · N rows]`) — every selected failing
  ...
  to the last version it passed, because the sweep disproved validity beyond it. No
  ...
  individually via `[Fix ▸]` (a corrected/variant row via the author flow, per-row maintainer
  approval — never in the batch).

**Close → needs-action.** A close-intervals action that ORPHANS an entity — retracts
...
**The Fix flow carries context and returns.** `[Fix ▸]` is a round-trip,
...
recorded-vs-actual, inline at the field to edit** — re-derived in-browser against the
...
it cannot isolate a field), so the field-level diff is the TOOL's, computed by s04's
existing per-kind check. The s08 row's job is to carry the `detail` + the trigger; the per-field diff
renders on s04 (see [`s04-field-editor.md`](s04-field-editor.md) §"Arriving from a failing report
row"). And `[Fix ▸]` **PUSHES a state-carrying frame onto the content back-stack** — the s08
frame stores the parsed report (the worklist + block split + scroll), so `‹ back` from the fix
**restores the report exactly, with no re-import** (the report is client-side only, and
...
/ Cancel) — the fix is never silently lost or silently committed on the way back to the
report.

**The standing needs-action view.** Lifecycle completeness is a standing, tool-wide
...
- **Loading (ingesting)** — while parsing the picked report: the `ingest progress bar` —
  *"Parsing report… `N` / `M` rows"* (determinate; the row count is known from the file).
  Occupies the worklist's region and resolves in place to the populated worklist (stable layout).
...
- **Partial report (`complete: false`)** — the sweep finalized after stopping early (a mid-run
  death; the v3 report carries `complete: false` + `rows_expected` > rows-actual, the partial
  ...
  (system-caused, advisory copy). The `<rows-actual>` present rows render normally and stay
...
- **Error (a row's kcdx_id is unknown to the current DB)** — a report row references an id the
  ...
  silently dropped (never a silent loss).
- **Disabled** — each action block's bulk action is disabled (not hidden) when zero rows in that
  ...
  Verified-block rows carry the verify checkbox; failing-block rows carry the
  close-intervals checkbox AND a `[Fix ▸]` (the close retracts the interval; the fix authors the
  correction — distinct actions); no-action-block rows carry NEITHER (shown, no action).
- **Already acted on (report-vs-DB reconciliation)** — a re-imported row whose recommended
  ...
  *"already current"*), NO checkbox, NOT in any batch. It is excluded from the actionable
  ...
  empty-delta confirm). Shown, never hidden (never a silent loss); the maintainer sees a
  partly-acted re-import as true remaining work plus the already-resolved rows, never a wall of
  done rows framed as new.
- **Orphaned by a close (needs action)** — a close-intervals action that leaves an entity
...
- **Edge content** — a report with **0 failing** → the split defaults to Verified, the summary
  ...
  partial banner + the action bar stay fixed (stable layout); a long `name` /
  detail wraps within its cell without pushing siblings (stable layout).
- **Returned from a `[Fix ▸]`** — after a `[Fix ▸]` pushed s02/s04 and the maintainer
  ...
  empty state** (the report's client-side state was carried in the frame, not re-fetched). An
  actioned row (a confirmed fix) shows its updated verdict/value in place (a background result
  updates in place).
- **Back affordance** — at stack depth > 1, a `‹ back to <destination>` control sits
  top-left of the content pane in reserved space (no reflow); at s08's root (after the
  reset-from-s01 import) there is no `‹ back` (the navigator is the way out).
- **Unsaved-changes guard** — if a `[Fix ▸]` excursion left the s04 editor dirty and the
  maintainer navigates back to s08 (or elsewhere), the Save / Discard / Cancel confirm (the
  `overlay surface`) fires first; nothing is saved or lost without an explicit choice.

## Links in / out
- **In:** s01 `[Import verification report]` (`import_report` → the File API picker → this
  screen). (There is no auto-navigation to s08 — the maintainer chooses to import.)
- **Out:** `bulk_verify` / `bulk_close_intervals` → s06 (batch confirm overlay) → on confirm, one
  ...
  Retry"*), returns to the worklist with the actioned rows updated IN PLACE (the result
  updates status in place, never re-navigates); a failing row's `[Fix ▸]` → PUSHES s02
  (then s04 to author the corrected `rva` / `signature`, possibly a new/variant version row — per-row
  maintainer approval) onto the back-stack, and `‹ back` → THIS worklist with the report intact; at
  s08's stack root, `‹ back` → the navigator.

## Applicable interaction laws
- **Stable layout** — the summary header + action bar never reflow as rows scroll or verdicts
  update; the ingest progress occupies the worklist's reserved region.
- **Persistent shell** — s08 is a content-area screen, NOT an overlay; the shell persists; only the
  s06 batch confirm (an overlay) dims over it.
- **User-driven navigation** — importing, selecting rows, confirming, and `[Fix ▸]` are all user
  actions; the re-verify result updates the rows' status IN PLACE and never auto-navigates,
  auto-selects, or auto-opens a row.
- **Content back-stack** — `[Import verification report]` from s01 RESETS the stack to
  ...
  (the report is client-side only). A `[Fix ▸]` that left s04 dirty surfaces the
...
- **Advisory verification** — every imported verdict is advisory; a `failed` row (failing block), a
  no-action row (`not_applicable`/`cannot_check`/`skipped`/`error`), and a partial report are all
  SURFACED, never a block; bulk verify/close applies only the maintainer's explicitly selected rows,
  through the confirm (no silent bypass); no DLL/report content is uploaded (the report is read
  client-side).
- **Atomic confirmed transaction** — bulk re-verify is ONE atomic, confirmed transaction with the
  batch field-delta shown; a failure rolls back fully; the git commit/push stays invisible plumbing
  ("Re-verified N rows", never the git mechanics).
- **Single validator** — the audit-trio writes the re-verify performs are validated by the shared
  validator (reached through the API); s08 renders the verdict, it never reimplements a rule.
- **Non-color affordance** — the `live verdict badge` + the `proof-rank chip` + the block state are
  glyph+text, never color-alone; the disabled batch action is conveyed by more than color.
- **Semantic tokens only** — every color/space/size in s08 resolves to a semantic token; no raw
  value.
...
- **Phone (drill-down):** s08 is a full-screen drilled-in view reached from s01's import action;
  the back-stack drives the drill-down `‹ back` — at s08's root it returns to the
