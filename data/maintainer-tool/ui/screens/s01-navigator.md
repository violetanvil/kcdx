...
## Region & position
The navigator of the app shell (`../design.md` §"App shell"). **Wide:** the left pane,
fixed minimum legible width; below that width the divider stops and the detail pane scrolls
(not the navigator collapsing — law 2). **Phone:** the full-screen home view. Vertically:
...
| Status filter | `filter control` (`Select`/`SegmentedControl`) | all / active / deprecated / superseded (status derived per `policy.md` §"Status is NOT an authored column") | `set_status_filter(value)` |
...
`status_unverified`), color + glyph (law 7 — never color-alone). The list is fetched from
the backend API (the data-core's curated set); the frontend holds no SQL/rule logic (law 6).

## States & variants
- **Populated** — the filtered, scrollable list; the selected row uses `surface_selected` +
  `body_strong`; hover uses `surface_hover`.
- **Empty (no DB/seeds resolved)** — the API reported the backend could not resolve the
  reference DB/seeds at its configured checkout path (`../design.md` §8 / D18). Copy:
...
## Applicable laws
- **Law 2** — the navigator persists as the navigation shell; only the overlay layer covers
  content.
- **Law 3** — selecting/tapping a row is a user action that drives navigation; a background
  event (load completing, a status re-derivation) updates the list/chips in place, never
  auto-selects a row.
...
- **Phone:** the full-screen home view; tapping an entity **drills** to s02 full-screen (a
  `‹ back` returns here, selection + scroll position preserved — law 1/3). `+ New entity`
  opens s05 as a full-screen sheet.
