...
- **In-bulk, in-game, at startup (D25/D28).** A kcdx **test-suite plugin** runs both D25
  checks at engine startup over **every** DB row — the **on-disk version-applicability hash**
  (does the running build's on-disk body at this `rva` match the DB's `content_hash` → valid
  for this build, or diverged → avoid) + the **loaded-image reachability check** (does the
  address resolve into live `.text`) — and writes a **JSON report** (per row: resolves+works /
  dead / wrong-target / cannot-check, with the D25 meanings — version-match + reachability, NOT
  a runtime "does it still work" body hash). I **import that report** into the tool → a
  worklist: passing rows → one-click bulk re-verify (the audit trio auto-fills, `evidence_kind`
  from the check — D29); failing rows → flagged to fix.
...
- The in-game batch plugin reports every row's verdict (on-disk version-applicability +
  loaded-image reachability — D25) to a JSON report; importing it
...
**Batch mutation (bulk re-verify — US-11/D32).** The spine also commits a BATCH: the bulk
re-verify (s08, the verification report worklist) applies N audit-trio UPDATEs as ONE atomic
transaction with ONE batched field-delta confirm (the per-row `old → new` deltas shown together
under one confirm). It reuses the spine at batch scale — the same validator gate (each row
validated), the same deferred-commit + robust rollback (**all-or-nothing — one row failing rolls
back the whole batch**), one git commit/push. The batch is all-UPDATE (re-verify never creates a
row), so the new-row approval gate (law 8/AP18) does not apply. "One mutation = one transaction"
reads as "one confirmed UNIT = one transaction" — a single edit OR an explicitly-selected batch.
...
| D32 | The save spine supports a BATCH mutation (bulk re-verify is N rows in ONE atomic confirmed transaction) | The save spine (validate → write → export → round-trip → field-delta confirm → atomic commit + push) — designed one-mutation-per-transaction (D5/D16, law 5) — also supports a **BATCH mutation**: the bulk re-verify (s08, the verification report worklist) commits **N audit-trio UPDATEs as ONE atomic transaction** with **one batched field-delta confirm** (the per-row `field: old → new` deltas — `last_verified_at_version` / `verified_date` / `verified_by` / `evidence_kind` — shown together under one confirm). The batch reuses the existing spine at batch scale: the same shared validator gate (R3/law 6, each row validated), the same deferred-commit transaction + D21 robust rollback (**all-or-nothing — one row failing rolls back the WHOLE batch**, nothing partial lands), the same single git commit/push. The batch is **all-UPDATE** (re-verify never creates a row), so the new-row approval gate (law 8 / AP18) does NOT apply. This is the functional contract s08's batched-confirm UI builds on; it makes "one mutation = one transaction" read as "one CONFIRMED UNIT = one transaction," where a unit is a single edit OR an explicitly-selected batch. | Each row a separate transaction (155 sequential confirms + 155 commits — brutal UX + 155 git commits for one review session; defeats "bulk"). One approval with no per-row delta shown (faster, but breaks law 5's field-delta-before-write — the maintainer wouldn't see what each row's audit trio becomes). Keep the batch behavior only in the s08 screen spec, the TRD silent (an executor reading law 5 literally builds the batch path wrong, or a reviewer flags the batch as a law-5 violation — the drift the §E rectifying pass exists to close). *(Settled 2026-06-05 — surfaced by `/ui-design`'s bulk-re-verify design, approved by the user as a TRD change via the §E rectifying pass.)* |
