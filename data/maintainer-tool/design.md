...
> auto-increment reset). The derived CSV export writes to **`data/db-export/`** (curated,
> git-text — D20) **+ `data/db-export-bulk/` (the bulk corpus, Git LFS — D38)**.
> `create-version`-at-a-new-game-tag now works (a direct INSERT bypasses the
> seed-rebuild's baseline gate). **`run_rebuild` now rebuilds BOTH DBs from those tracked
> CSVs, NOT from `data/seeds/` (retired) or the Ghidra dump (expert-only regeneration) — D38.**
> **Scope:** the Address Library maintainer sub-system — `data/maintainer-tool/`,
> `data/db-export/` + `data/db-export-bulk/`, and the `data/refdata-extractor/python/` toolchain that links them.
...
> `policy.md` (the column-level authoring invariants); `fingerprint-per-kind.md`
> (the survival design).
...
| **Bootstrap seeds** | **RETIRED (D38).** Formerly the three files under `data/seeds/`, the one-time from-dump genesis. Now deprecated — nothing reads them; the data is archived at `data/seeds - deprecated/` pending deletion. The genesis role moved to the tracked CSV export (`data/db-export/` + `data/db-export-bulk/`), which `run_rebuild` now reads. |
| **The derived export (CSV record)** | the CSVs the maintainer tool exports from the DB on every committed save AND that `run_rebuild` reads as the rebuild genesis (D38), in two halves split by the `kcdx_id IS NOT NULL` curated/bulk seam: **`data/db-export/`** = the curated rows, plain git-tracked text (the diff/review record — D1/D19/D20); **`data/db-export-bulk/`** = the bulk corpus (the `kcdx_id`-NULL discovery rows + the DEV-only `statements`/`referenced_vars`/`call_edges` tables), tracked via **Git LFS** (D38). The DB is the originator (out of git, D1); these are its deterministic, lossless, round-trip-verified export. |
...
```
REBUILD (from the tracked CSVs — D38; the seeds + the from-dump genesis are RETIRED):
  data/db-export/*.csv  (curated, git)                  --run_rebuild-->  reference.sqlite
  + data/db-export-bulk/ (bulk: statements/refs/edges + kcdx_id-NULL rows, Git LFS)  -->  reference-dev.sqlite
  (the ~1.3 GB Ghidra dump is NOT a routine input — kept expert-only to REGENERATE
   the bulk CSVs when the dump itself changes, e.g. a new game version's disassembly)
...
AUTHORING (every maintainer edit, the inverted path — D19):
  tool --direct INSERT/UPDATE-->  reference.sqlite + reference-dev.sqlite   (validated, deferred-commit txn)
                                          |
                                          v  (deterministic export on every committed change — both halves)
  GIT-TRACKED RECORD:  data/db-export/*.csv (curated, git-text — D1/D20 diff layer)
                     + data/db-export-bulk/ (bulk CSV, Git LFS — D38)
  (round-trip oracle: rebuild-from-CSV → DB → re-export → byte-identical, over BOTH halves — D38)
```
The DB is the originator (out of git — D1). `run_rebuild` builds both DBs from the **tracked
CSV export** (`data/db-export/` curated + `data/db-export-bulk/` bulk-via-LFS — D38), NOT from
`data/seeds/` (retired) or the Ghidra dump (expert-only regeneration). Every edit writes the DB
directly and re-exports the derived CSV record; the rebuild reads it back — the same CSVs are both
the genesis and the history (D38).

What the inversion changes:

- **The CSVs survive** as a derived export — the curated half at **`data/db-export/`** (D20),
  the bulk half at **`data/db-export-bulk/`** under Git LFS (D38), both private by default (the
  publish-public allowlist is opt-in, so a new `data/` subdir is not published unless added).
  `policy.md`'s file-format rules (UTF-8, `QUOTE_MINIMAL`, `#`-comments, header-literal column
  order) still describe the exported shape, and a reviewer still reads a human-readable CSV diff
  on the curated half. `data/seeds/` is **retired** (D38) — the genesis role moved to the tracked
  CSV export, which `run_rebuild` now reads.
...
- **`db_editor.py`** — apply a validated, atomic edit transaction to the DB (the
  ...
  **Mechanism (D19, settled 2026-06-03 — supersedes the D13 seed-rebuild bridge):**
  ...
  exports the committed DB → the **derived CSV record** (`data/db-export/` curated + `data/db-export-bulk/` bulk-via-LFS, diff-preserved — D20/D38). `run_rebuild` now rebuilds both DBs from that
  tracked CSV export (the seeds + the from-dump genesis are retired — D38; the dump is kept
  expert-only to regenerate the bulk CSVs). This is the
...
- **The engine survival checker** (kcdx engine, `src/survival.cpp` extended) — the **authority**
  (D27): the per-kind verification pass, extended from function-hash-only to all 9 kinds. It
  runs, per row, the **strongest applicable active attempt** and returns a structured response —
  the model is "do something to every row, report what it found, cap the verdict honestly at
  what the attempt proved" (D25, D36). The methods, weakest→strongest: existence/resolution →
  on-disk version-applicability hash → loaded-image reachability → safe-read exercise → observed
  live execution. The **version-applicability hash is on-disk** (not the loaded image — relocated
  + kcdx-detoured) + the **reachability check** reads the loaded image (does the address resolve
  into live `.text`); the **live-exercise tier** (observed execution) is the new top rung (D36).
  Lives in the kcdx tree (not the maintainer-tool), governed by kcdx's own rules; the
  maintainer-tool design references it as the cross-impl authority.
- **The in-game verification plugin** (kcdx `test-plugins/`) — a dev-mode-gated plugin whose ONE
  job is to drive the engine checker over the curated USER set and emit the JSON verification
  report (D28). Triggered by a **console command (`kcdx_verify_all`) run after the maintainer
  loads a save** (D33-revised) — the live-exercise tier needs a loaded world; it is NOT a
  boot-automatic sweep. The sweep **streams a per-row line to the console** as each row's attempt
  completes (so a 157-row pass never reads as a hang) and emits one structured response per row
  (D36). A standing batch-sweep producer; governed by kcdx's `test-suite.md`. (The checker's own
  synthetic unit-tests — cap-84 dispatch, cap-85 cross-impl agreement — stay boot-time self-tests,
  no game state needed; only the bulk curated sweep moves to the command.)
...
**The version selector's source includes the linked Bin's resolved version — even when it is
NEW (D43).** The top version selector is populated from TWO sources, not one: the server-known
`game_versions` tags (the dropdown) AND **the version resolved from the linked Bin folder**
(the WHGame.dll `.rdata` scan, D15/D30). When a linked Bin resolves to a version the DB does
not know — a build newer than any `game_versions` tag, with no `address_versions` row covering
it — that resolved version **auto-becomes the selected version at the top**, marked *"from your
linked Bin: `<version>` · new"*, **without the maintainer hand-typing a version string**. This
is the maintainer-side disassembler-test (`cornerstones.md`): the tool reads the version from
the artifact the maintainer already linked; it never makes the human supply what the folder
carries. The auto-selected new version is **a first-class selectable that drives authoring** —
it is the version a new-row authoring/verify action targets, feeding the existing D30
link-to-create flow (author a row at `<version>` → the static per-kind check runs against the
linked DLL → AP18 confirm on the new row). The new version is **ephemeral until a row is
authored at it**: it does not enter the server's `game_versions` tag list on the folder-link
alone — only the AP18-confirmed authoring of a row at it commits the new version's existence
(a `game_versions` tag is a deliberate curated fact, born when a row commits at it, never a
folder-link side effect — the AP18/law-8 deliberate-addition discipline, D43). On a phone or
any host with no game install, there is no Bin to link → the dropdown + newest-row default
(D10) is the path, unchanged.
...
- **In-bulk, in-game, on a console command after save-load (D25/D28/D33-revised/D36).** A kcdx
  **test-suite plugin** — dev-mode-gated, triggered by a **console command (`kcdx_verify_all`) I run
  after loading a save** (D33/D36 — the live-exercise tier needs a loaded world; NOT a boot sweep) —
  over the **curated USER set** (D33). Per row it runs the **strongest applicable active attempt**
  (D36's 5-rank ladder — observed execution → safe-read → reachability → on-disk hash → resolution)
  and **attributes each result to the `address_version` row whose fingerprint the swept bytes match**
  (D34). It **streams a per-row line to the console** as each attempt completes (D36 — never reads as
  a hang) and writes a **v3 JSON report** (per row: `kcdx_id`, resolved version, the **7-state
  verdict** + `method_rank` + `invoke_attempted` + `invoke_skip_reason` + detail, and the matched
  `address_version_id`; the verdict is the CEILING of the strongest method that ran —
  `verified_working` ONLY from observed execution, else `passed_not_verified` / `failed` /
  `not_applicable` / `cannot_check` / `skipped` / `error`, per D36). I **import that report** into the tool → a worklist with
  ...
  **close-intervals** (retracts each failed row's `valid_through` to its `last_verified_at_version`,
  ...
  (D32). The `valid_through` written here is an **authored, tool-auto-filled** column (D40 — the
  audit-trio pattern: the re-verify auto-fills it, I confirm it, and I can hand-correct an interval
  in the s04 single-version editor; an interval validator gates both). A failed row needs no "failed"
  marker — not advancing `last_verified_at_version` leaves
...
- The in-game batch plugin (dev-mode, console-command-after-save-load, curated set — D33/D36)
  runs the strongest applicable attempt per row (D36's rank ladder) and reports every row's
  **7-state verdict + `method_rank`** PLUS the matched `address_version_id` (D34) to a v3 JSON
  report; importing it drives the two-block worklist (D35) — `verified_working`/`passed_not_verified`
  → the verified block; `failed` → the failing block; `not_applicable`/`cannot_check`/`skipped`/`error`
  → shown, no action —
...
**US-12 — Navigate and go back without losing my place (the in-app back-stack).** As a maintainer,
I move between the entity detail, the verification worklist, and the needs-action view, and when I
drill into something to fix it I can go `‹ back` to exactly where I came from — my ingested report
still showing its pass/fail rows, my needs-action list still scrolled where I left it — so I am
never dumped at the app root or forced to re-do work to get back. (§10 D42.)
**Acceptance:** navigation is an in-app back-stack governing the content area (no URL router); the
shell (navigator + content + overlay) persists (`ui/design.md` laws 2/3). A resolve-action (an s09
row's resolve, an s08 `[Fix ▸]`) pushes a frame; a top-level destination (a navigator entity
selection, `[Needs action]` / `[Import report]` / `[+ New entity]`) resets the stack to a fresh
root (so `‹ back` never detours through an abandoned task). Each frame carries the screen's full
in-memory STATE, so `‹ back` restores it exactly — critically, **`‹ back` to s08 after a `[Fix ▸]`
shows the SAME ingested report (worklist + block split + scroll), with no re-import** (the report is
client-side only, D31 — verified by drilling into a failing row, fixing or cancelling, and `‹ back`
landing on the intact report). On wide, the navigator stays always-live and a destination-labeled
`‹ back` ("‹ back to Needs action") shows in the content pane at stack depth > 1; at root there is
no back (the navigator is the out). On phone the content-stack drives the drill-down `‹ back`,
returning to the navigator home at root.

**US-13 — Never lose (or silently commit) unsaved edits when I navigate.** As a maintainer, when I
start editing a row or a lifecycle field and then navigate away — switch entities, hit a top-level
view, or go `‹ back` — the tool stops me with a clear choice so I never lose work I meant to keep,
and never silently save work I meant to abandon. (§10 D44.)
**Acceptance:** any navigation away from a dirty editor (the s04 field editor or s02 lifecycle with
pending edits not yet through the s06 confirm) is intercepted by a confirm overlay — *"You have
unsaved changes to `<entity/row>`. [Save changes] / [Discard changes] / [Cancel]"* — BEFORE the
navigation happens. **Cancel** stays put (nothing lost); **Discard** drops the edits then navigates
(loss only after the explicit click); **Save** runs the normal save spine (validate → field-delta
confirm → atomic transaction, law 5) then navigates. Nothing saves OR is lost without an explicit
choice — verified by editing a field, attempting each navigation type, and confirming the overlay
fires and that neither a silent save nor a silent discard occurs. Auto-save-on-navigate is
explicitly excluded (it would silently commit + push an abandoned edit and trip AP18 on a new row).
...
`last_verified_at_version` — the last version it passed (D35). The per-row edits are NOT computed by
the frontend — the **data-core resolves them from the report** (D39): the FE sends the report rows to
a `/save/reverify-batch` preview endpoint, which calls the `reverify_resolver` module (it reads the
matched row by `matched_address_version_id` for verify-all, or the interval-containing row of
`kcdx_id`+resolved-version for close-intervals, and computes the trio + proof-rank `evidence_kind` +
the `valid_through` extend/retract), returns the field-deltas for the batched confirm (no write), and
the FE then posts the SAME computed edits to `/confirm/batch` to transact. The `valid_through` the
resolver computes is written to the AUTHORED `valid_through` column (D40 — on the seed surface +
`EDITABLE_VERSION_COLUMNS`, the audit-trio pattern: tool-auto-filled here, hand-overrideable in the s04
single-version editor; an interval validator gates both — `valid_through >= valid_from`, no overlap,
open-row uniqueness). Both reuse the spine at
batch scale — the same validator gate (each row validated), the same deferred-commit + robust rollback
...
**The in-app navigation model (the back-stack — US-12/D42).** The tool is a single-page app with
NO URL router; navigation between content screens is governed by an **in-app back-stack**. The app
shell is unchanged (`ui/design.md` laws 2/3): a persistent navigator (the left pane on wide; the
home view on phone) + a content area that swaps between **peer content screens** (s02 entity detail,
s08 verification worklist, s09 needs-action) + an overlay layer (s05 create, s06 confirm). The
back-stack governs the **content area** only:

- **Frames carry full screen STATE, not just identity (D42).** Each stack frame stores the screen
  AND its live in-memory state: s08's ingested report (the parsed pass/fail worklist + block split +
  scroll), s09's section toggles + scroll, s02's selected version + expanded sections + selected
  version row. `‹ back` **restores the screen exactly as it was left** — it does not re-mount a fresh
  copy. This is load-bearing for the s08 report (US-12): the verification report is read CLIENT-SIDE
  and never persisted server-side (D31), so an identity-only frame would land `‹ back` on the empty
  "import a report" state and force a re-upload; the state-carrying frame is what keeps the ingested
  report intact across a [Fix ▸] excursion (the behavior s08's spec already promises — "the report's
  client-side state survives the Fix excursion").
- **A resolve-action PUSHES; a top-level destination RESETS (D42).** A context-carrying resolve
  action (an s09 row's resolve, an s08 `[Fix ▸]`) PUSHES a frame, building the fix-flow chain
  (s09→s02→s04). A deliberate **top-level destination** — selecting an entity in the always-visible
  navigator, or a top-level entry (`[Needs action]`, `[Import report]`, `[+ New entity]`) — **resets
  the stack** to a fresh root at that destination. So `‹ back` always walks the way the maintainer
  came to the CURRENT thing, never detouring through an abandoned earlier task.
- **The back affordance (D42).** On wide, the navigator is always live (selecting an entity there is
  never on the stack); a **`‹ back` control sits at the top of the content pane whenever the stack
  has depth > 1**, labeled with its destination ("‹ back to Needs action", not a bare arrow) so the
  maintainer always knows where it returns; at stack root there is no back (the navigator is the
  "out"). On phone the same content-stack drives the drill-down `‹ back`; at the phone root, `‹ back`
  returns to the navigator home view.

**The unsaved-changes guard (every navigation away from a dirty editor — US-13/D44).** Navigating
away from a screen with unsaved edits — a stack-reset to a new top-level destination, selecting
another entity, or a `‹ back` — is **intercepted by a confirm overlay** before the navigation
happens: *"You have unsaved changes to `<entity/row>`. [Save changes] / [Discard changes] /
[Cancel]"*. **Cancel** stays on the screen (nothing lost); **Discard** drops the pending edits and
navigates (the loss is explicit, never silent); **Save** runs the normal save spine (validate →
field-delta confirm → atomic transaction) then navigates. Nothing is ever saved OR lost without an
explicit maintainer choice — the guard forecloses BOTH failure modes: the maintainer is never
surprised by lost work, AND never tricked into thinking a discard silently committed (or that a
navigate-away silently saved). The guard is HARD on silent loss (a dirty editor cannot be navigated
away from without the choice surfacing) and complements law 5 (a mutation is one confirmed
transaction): auto-save-on-navigate is rejected (it would silently commit + git-push an edit the
maintainer meant to abandon, and would trip AP18 on a new row). "Dirty" = the s04 field editor or
the s02 lifecycle editor holds pending edits not yet through the s06 confirm.
...
- **Verification-context states** — a picked version (the dropdown default), or "checked
  ...
  DLL (D9/D10/D15).
- **Navigation back** — a content screen reached via a resolve-action (stack depth > 1) shows a
  `‹ back` affordance labeled with its destination ("‹ back to Needs action"); `‹ back` restores the
  prior screen's full state (the s08 ingested report intact, no re-import — D42); at stack root there
  is no back affordance (the navigator is the out).
- **New version from a linked Bin** — the top version selector shows the Bin-resolved version as the
  selected version, marked *"from your linked Bin: `<version>` · new"* when it is not in
  `game_versions` (D43); it drives authoring at that version, and is ephemeral until a row commits at
  it.
- **Unsaved-changes guard** — navigating away from a dirty editor (s04 field editor / s02 lifecycle
  with pending edits) surfaces the Save / Discard / Cancel confirm before the navigation; nothing
  saves or is lost without an explicit choice (D44).
...
- **Distribution (R9 — web app, D14/D18):** a **Docker image** — a Python (FastAPI/Flask)
  ...
  checkout (the repo with the tracked CSV export — `data/db-export/` + `data/db-export-bulk/`
  under Git LFS, the rebuild genesis per D38 — + the git-ignored reference DBs) lives on a
  **mounted volume at a configured path** the container reads/writes/commits; the image carries
  only the app code. The operator provides the volume + the push credential (below) **+ `git lfs`
  for the bulk corpus (D38)**. The exact image packaging
...
| D29 | A passing check determines `evidence_kind` (verification IS the audit evidence) — the in-game check's tier is keyed by its D36 proof rank | A passing verification check **auto-fills `evidence_kind`** by WHAT the check was — tying verification directly to the audit trail (the record of HOW a row was verified, not a guess): the browser static AOB-uniqueness check → `pattern_scan`; a maintainer eyeballing Ghidra with no automated check → `maintainer_ghidra` (the manual default already built); the **in-game check → keyed by the row's D36 proof rank** — because D36 split the in-game pass into two proof tiers, a single `live_production` would over-claim a row that never executed. The mapping: a **rank-1 `verified_working` row** (OBSERVED live execution — the function fired + passed through this session) → **`live_production`** (it truly ran in the real game); a **ranks-2–5 `passed_not_verified` row** (the in-game STATIC pass — on-disk version-applicability hash + loaded-image reachability + safe-read; the in-game test plugin checked it, but did NOT observe it execute) → **`live_test_plugin`** (the existing rank-2 tier — the in-game verify-all sweep IS the test-suite plugin, D28/D33, so "the in-game test plugin verified this row" is exactly what `live_test_plugin` records; rank-2 sits honestly between `live_production` (observed executing) above and the static browser/manual tiers (`pattern_scan`/`maintainer_ghidra`) below). **`live_test_plugin` thus covers the in-game test plugin's STATIC pass** (the plugin ran the check; it just did not observe execution — that is the `passed_not_verified`/rank-2-5 case); a `verified_working` row the plugin observed executing earns the higher `live_production`. No new `evidence_kind` enum value is added (the frozen-schema guarantee holds — `live_test_plugin` is an existing enum member already used by in-game-plugin-verified rows), and the existing `live_test_plugin` rows keep their tier on re-verify (no downgrade). All editable. This composes with the just-built audit-trio auto-fill (setting `last_verified_at_version` auto-completes `verified_date`=today + `verified_by`=injected identity + `evidence_kind`) — the check refines `evidence_kind` from the default to the tier the check actually establishes. **A passing bulk re-verify writes `last_verified_at_version` → the swept version on the matched row (D34) + the auto-trio (with `evidence_kind` per the row's proof rank as above); AND if the swept version sat in a GAP or beyond the matched row's interval, it EXTENDS that row's `valid_through` forward to the swept version** (the check proved the swept version belongs to that row, so the row's coverage grows to include it — D34). A row already covered (`last_verified_at_version >= swept version`) is skipped — nothing to add. | `evidence_kind` stays a manual pick decoupled from the check (the maintainer chooses the tier; no auto-attribution — but then a real passing check and a guess record the same evidence label, losing the "verified by HOW" signal the trio exists to carry). A single `live_production` for EVERY in-game-passing row regardless of rank (the pre-D36 framing — over-claims a `passed_not_verified` static pass as live execution, the exact "verified by HOW" signal-loss this decision prevents, now that D36 distinguishes observed-live from statically-passed). Mapping the in-game static pass to **`pattern_scan`** (the WEAKEST tier, coined for the BROWSER AOB scan — it under-claims a check the in-game plugin actually ran, mislabels the live-image-reading reachability/safe-read checks as a static byte-scan, AND silently DOWNGRADES the existing `live_test_plugin` rows on re-verify; `live_test_plugin` is the honest existing fit). A NEW `evidence_kind` value for the in-game static pass (unnecessary enum churn against the frozen-schema guarantee — `live_test_plugin` already means "the in-game test plugin verified it"). *(Settled 2026-06-04; the `valid_through`-extension-on-gap-pass added 2026-06-05 — D34; the in-game `evidence_kind` split by D36 proof rank added 2026-06-09 — `verified_working`→`live_production`, `passed_not_verified`→`live_test_plugin` — surfaced by the s08 `/ui-design` reconciliation, the static-tier value corrected from `pattern_scan` to the more-honest existing `live_test_plugin` by the design-soundness gate.)* |
...
| D33 | The in-game sweep is dev-mode-gated, runs on a console command AFTER save-load, over the curated USER set only (**trigger REVISED by D36**) | The batch verification plugin (D28) is a **dev-mode-gated kcdx test plugin**: it runs ONLY in `dev_mode` (self-skips otherwise, like every `cap-NN`/`comp-NN`), and is triggered by a **console command (`kcdx_verify_all`) the maintainer runs AFTER loading a save** (D36) — NOT once at engine boot. **Why the trigger reversed (D36):** the live-exercise tier (D36 rank-1 observed execution) needs a loaded world — many curated functions only fire during play, so a boot-time sweep would see zero fires and could never reach `verified_working`. Running after save-load is what makes the top rung reachable. A player launching normally never runs it (no `dev_mode`, no console gesture; the agent enables `dev_mode` + launches per `agent-builds-and-deploys`). It sweeps the **curated USER set only** — the `address_versions` rows that carry a `kcdx_id` (the maintained entities, the ~157-row scale the s08 worklist is designed for); the DEV bulk discovery rows (the ~321k `kcdx_id`-NULL rows) are NOT swept (no trio, unusable worklist scale). The sweep **streams a per-row line to the console** as each attempt completes (D36) so a 157-row pass never reads as a hang. | Startup-automatic boot sweep (the ORIGINAL D33, now superseded — a boot sweep runs before any save loads, so the live-exercise tier sees no fires and every hookable row caps at `passed_not_verified`; the whole point of D36's top rung is lost). A console trigger with NO save-load precondition (run from the menu — live-exercise rows self-skip to `skipped`, defeating the live tier). Always-on every launch incl. production (against the dev-only-tests convention). Sweeping the full DEV set (unusable scale). *(Settled 2026-06-05; trigger REVISED 2026-06-09 — D36: console-after-save-load replaces boot-automatic, because the live-exercise tier needs a loaded world. The curated-USER-set scope is unchanged.)* |
| D34 | The sweep ATTRIBUTES each result to the matched `address_version` row; a gap-pass extends that row's interval | The check does not just say pass/fail per `kcdx_id` — it **resolves WHICH `address_version` row the swept bytes belong to** by matching them against each candidate row's fingerprint (`content_hash` for a function, the per-kind datum otherwise). The report carries the matched row id per result (`matched_address_version_id`). This is what lets the importer handle an **uncovered version**: when the swept version falls in a GAP between an entity's intervals (e.g. id 1 `valid_from 1.1, valid_through 1.3`; id 2 `valid_from 1.5`; swept at **1.4**), the byte-match attributes 1.4 to whichever row it matched (say id 1), and a passing bulk re-verify **extends that row's `valid_through` forward** to the swept version (id 1 `valid_through 1.3 → 1.4`) — the check proved 1.4 belongs to id 1, so id 1's coverage grows to include it. A row whose interval already covers the swept version (or whose `last_verified_at_version >= swept version`) is skipped. The write stays an UPDATE (no new row), since a passing check by definition found the bytes UNCHANGED — there is nothing new to describe, only coverage to record. | Match only against one nearest/expected row (can't attribute a gap version to the right row when the gap sits between two candidates). Create a new `address_version` row for the swept version (wrong — a passing check found NO change, so a new row would just duplicate the matched row with a different label; the schema is one row per interval-of-unchanged-facts, not one per version). Stamp only `last_verified_at_version` and never touch the interval (leaves the row's interval claiming it doesn't cover a version the check proved it does). *(Settled 2026-06-05 — the user's "auto attribute it to the correct address version that it worked for"; gap-pass extends `valid_through` to the swept version. The `valid_through` write target is the AUTHORED column D40 establishes — D40, 2026-06-10.)* |
| D35 | The worklist is two reviewed blocks; a failure CLOSES the over-claimed interval to the last passing version | The import shows a **diff the maintainer reviews, never an auto-write** — a **verified block** (all `resolves_works`) and a **failing block** (all `wrong_target`/`dead`), each with its own bulk action + batched field-delta confirm (D32). **Verify-all** (verified block) writes the trio + the D34 `valid_through` extension. **Close-intervals** (failing block) writes, per failing row, **`valid_through` → that row's `last_verified_at_version`** — retracting an over-claimed interval (e.g. id 2 `valid_from 1.5, valid_through null` that fails at 1.6 closes to `valid_through 1.5`) back to the last version with positive evidence, because the sweep DISPROVED validity beyond it. A failure needs **no persisted "failed" marker and no new schema field**: not advancing `last_verified_at_version` already leaves the row UNVERIFIED at the new version by the existing query-time derivation (`policy.md` §"Status is NOT an authored column"); the interval-close keeps the row's `valid_through` honest. The maintainer then **fixes** a failed function individually via [Fix ▸] → the author flow (a corrected RVA/signature, possibly a new/variant version row via the create flow + AP18 — per-row, not in the batch). | A persisted `failed_at_version` field or a `verification_failed` `evidence_kind` tier (unnecessary — UNVERIFIED-by-derivation is already the durable signal; overloads `evidence_kind`, a passing-only quality ranking; adds schema churn against the frozen-schema guarantee). A failing block as pure read-only triage with no interval-close (leaves the DB claiming validity the sweep disproved until each row is hand-fixed — the honest interval lags the evidence). Close `valid_through` to "the version just before the failing sweep" (overclaims — that version may never have been checked; `last_verified_at_version` is the only version with positive evidence). *(Settled 2026-06-05 — the user: a failure's `valid_through` "should just go back to the last version it was checked for where it passed", recorded as a bulk close-intervals action. The `valid_through` close writes the AUTHORED column D40 establishes — D40, 2026-06-10.)* |

| D36 | Verify = test EVERY row actively + report what it found; the verdict is the CEILING of the strongest method that ran (the 7-state enum + the 5-rank proof ladder + the live-exercise tier) | The sweep's promise: **every curated row gets its strongest APPLICABLE active attempt and a real structured response — no row is ever a passive "couldn't determine."** Each row's response is a **triple + a flag**: `{ verdict, method_rank (1–5), detail, invoke_attempted (bool), invoke_skip_reason (null \| unsafe_to_call \| uncontainable \| not_a_callable_kind) }`. **The verdict enum (7 states):** `verified_working` (observed executing correctly, this session — the top rung, no doubt), `passed_not_verified` (the strongest applicable attempt PASSED but cannot prove execution — only bytes/resolution/wiring), `failed` (an attempt the row should pass returned wrong — diverged bytes / dead resolve / wrong target / exercise threw), `not_applicable` (the running build's version isn't covered by this row — gap / no matching version; the version-check attempt RAN and found non-coverage), `cannot_check` (the attempt ran but the row lacks the inputs the check needs — no fingerprint / a deferred kind like `vtable_index`), `skipped` (a precondition wasn't met THIS run — needs a loaded save, run from menu; module DLL absent — the only "didn't run," and it names exactly why), `error` (the harness ITSELF faulted on this row — distinct from `failed`: the ROW is fine, the TEST blew up). **The 5-rank proof ladder (what each METHOD can prove "without a shadow of a doubt"), weakest→strongest:** (5) existence/resolution → (4) on-disk version-applicability hash → (3) loaded-image reachability → (2) safe-read exercise (read a live value with zero mutation — a cvar read, a read-only vtable walk) → (1) **observed live execution** (the function fired in the running process AND passed through correctly — kcdx's own hook-chain fire-count + correct pass-through, OR kcdx's own production call that already ran). **The honest rule: a verdict is the CEILING of the strongest method that ran. ONLY rank-1 (observed execution) can award `verified_working`; ranks 2–5 cap at `passed_not_verified` no matter how green the test.** This is the load-bearing point: passing a hash/reachability/read check is real evidence but NOT proof-of-working, so it cannot claim the top rung — it is "passed tests," not "verified working." The report carries BOTH the verdict and the rank that produced it. **rank-1 has two observed sub-paths, both zero-invoke-risk** (kcdx never mints a synthetic call to a foreign function — the GAME calls it, we observe): HOOKED (the `engine.*` chain set — `lua_pcall`, `lua_newstate`, `ccrypak_*`, `serialization`, `CGame_Update`, `SaveGame`/`LoadGame` — fire-count > 0 + correct pass-through) and CALLED-by-kcdx-in-production (`ICVar_GetIVal/GetFVal`, `IConsole_ExecuteString/AddCommand` — kcdx already invoked them this session). An uncallable foreign function (the `lua_pcall`-class row) still gets an ACTIVE attempt — its strongest applicable method is the static check (rank 4) — and a real response (`passed_not_verified` on a hash+reach pass, `failed` on a mismatch) with `invoke_attempted: false, invoke_skip_reason: "unsafe_to_call"`; the "couldn't safely invoke" fact is a structured property of the response, NEVER a verdict that means "gave up." The per-kind ceiling MATRIX is §11.6. (The rank-1 `verified_working` set is small — the ~10–15 kcdx-hooked/called rows whose observed fire the sweep confirms at runtime, not a pre-baked count — and most curated rows honestly land `passed_not_verified`; that is the design working, not a coverage gap.) **Maps onto D35's two-block worklist:** `verified_working` + `passed_not_verified` → the verified block (verify-all); `failed` → the failing block (close-intervals); `not_applicable` / `cannot_check` / `skipped` / `error` → shown, no action. **The report schema bumps v2 → v3** (adds `method_rank`, `invoke_attempted`, `invoke_skip_reason`; the 7-verdict enum). | A passive survey that marks hard rows "not_exercisable / cannot_check" without attempting anything (the cop-out this decision kills — the user: "it should be literally doing something for each item, and we get a response for every row"). A single `verify` verdict with no proof-rank (then a hash-match and an observed-fire read the SAME `passed`/`verified`, collapsing two very different confidence levels — the user: "if passing tests isn't enough to verify, it should enum as passed tests, not verified working"). Awarding `verified_working` from a passing static check (a green hash is NOT proof the code runs — the exact over-claim the ceiling rule forbids). A general runtime "will this call corrupt something before I call it?" pre-flight (NOT reliably constructible — the corruption is a silent successful write to shared state with no inverse, the KI-0001 class, invisible to SEH and to any snapshot; a false-safe corrupts the session, strictly worse than not invoking — so rank-1 comes from OBSERVING the game's own call, never from a guarded synthetic call). Collapsing the 7 verdicts to 3 (loses `failed`-vs-`error` — a crashed harness misreads as a bad row — and `not_applicable`-vs-`cannot_check` — a version gap misreads as missing data; each of the 7 drives a distinct worklist action). A boot-time trigger (D33 — the live-exercise tier needs a loaded world). *(Settled 2026-06-09 — the user: bucket every attempt by what it can prove without a shadow of a doubt; do something active for every row; only observed-execution earns "verified working," everything else is "passed tests.")* |

| D37 | The verification report is flushed INCREMENTALLY (append-per-row JSONL → finalized v3 JSON), never bulk-written at sweep end | The sweep writes each row's result to disk the INSTANT that row's attempt completes — it does NOT accumulate all rows in memory and emit one JSON document at the end. Mechanism: **append-per-row to a line-delimited sink (JSONL)** — one row-result object per line, written + flushed as the row resolves (the same per-row cadence as the console stream, D36); when the sweep finishes, a **finalize pass wraps the accumulated lines into the v3 JSON document** (`schema_version` + `game_version` + `summary` + `rows[]`). **Why incremental, not bulk:** the live-exercise tier (D36 rank-1) drives real game code, and a row's attempt CAN fault/hang the session (the `error` verdict exists for exactly the catchable case; an uncatchable hang/crash is still possible). A bulk write-at-end loses the ENTIRE report on any mid-sweep death; per-row flush means a sweep that dies at row N leaves a durable, complete-up-to-N JSONL the maintainer can still ingest (or a recovery wraps it into a partial v3). The v3 schema (the cross-repo contract) therefore defines BOTH shapes: the per-row JSONL line (one `rows[]` element) AND the finalized document, plus a partial-report signal (a `complete` flag / `rows_expected` so the FE consumer knows a report stopped early vs. swept the whole set). The console stream (D36) and the JSONL flush share the per-row tick — the row is printed AND durably written together, so what the maintainer watches scroll IS what is on disk. | Bulk-write the whole v3 JSON once at sweep end (the default-but-wrong shape — a mid-sweep hang/crash, which the live-exercise tier makes a real risk, loses every row's result; the user: "we cant bulk flush at the end"). Whole-document re-serialize after EVERY row (always a valid v3 file, but O(N²) serialization + a full write per row; the JSONL append is O(1) per row + one finalize). Periodic batched flush every N rows (bounds loss to a batch, not zero — weaker than the per-row durability the live-exercise risk demands). In-memory only with no disk until end (the bulk-flush this decision forbids). *(Settled 2026-06-09 — the user: "we must ensure we have the data flushing incrementally. we cant bulk flush at the end." JSONL-append→finalize chosen over per-row-full-rewrite + batched-flush.)* |

| D38 | Seeds RETIRE; BOTH reference DBs rebuild from tracked CSVs (curated → git, bulk → Git LFS); the dump retires to an expert-only regeneration tool (**supersedes D20**) | The bootstrap seeds (`data/seeds/`) are **fully deprecated and retired** — nothing reads them; the data is archived at `data/seeds - deprecated/` (deleted once no reference survives). The from-dump `run_rebuild` genesis is **replaced by a rebuild from committed CSVs** so neither DB is ever reconstructed from the ~1.3 GB Ghidra dump in normal operation. **The DB stays the authoring source-of-truth, OUT of git (D1 holds — `reference.sqlite` + `reference-dev.sqlite` remain git-ignored derived artifacts).** What changes is the GENESIS and the TRACKED CSV scope, split by the existing **`kcdx_id IS NOT NULL` curated/bulk seam**: **(1) the CURATED half** (the `kcdx_id`-bearing rows, ~157 — the human-authored, reviewable facts) exports to **`data/db-export/*.csv`, plain git-tracked text** (D1's diff/review layer, unchanged + already exported on every save); **(2) the BULK half** (the `kcdx_id`-NULL discovery rows + the DEV-only tables `statements` [~5.24M rows], `referenced_vars`, `call_edges`) exports to a tracked **CSV bundle under Git LFS** (a new location, e.g. `data/db-export-bulk/`) — git-addressable + versioned, but the heavy content lives in LFS storage, not the git object store as diffable text (line-diffing millions of machine-discovered `FUN_<rva>` rows has no review value; LFS is the right store for data this size while honoring "track as CSV"). **`run_rebuild`'s genesis changes** from `Ghidra dump + data/seeds/*.csv → both DBs` to `data/db-export/ (curated, git) + data/db-export-bulk/ (bulk, LFS) → both DBs`. **The dump retires to an expert-only one-off** — kept ONLY to *regenerate* the bulk CSVs when the dump itself changes (a new game version's fresh disassembly), never a routine rebuild input. **The exporter extends to capture BOTH DBs losslessly** (today it filters to the curated subset — `csv_exporter` §"the WHERE makes the exporter correct against the DEV DB too"); the completeness bar is a **round-trip oracle extended over the bulk**: rebuild-from-CSV → DB → re-export → byte-identical for both the curated and bulk halves (the existing `csv_integrity` round-trip, widened from the curated 157 rows to the full corpus). Both DBs are reproduced by `clone + git lfs pull + run_rebuild-from-CSV`, with zero dump dependency. | Keep D20's seeds-as-frozen-genesis + the from-dump rebuild (the user fully deprecated seeds — "we shouldn't be using the seeds at all anymore"; and "we don't want to have to reconstruct" the DEV DB from the dump). Commit the DEV bulk as raw CSV-text in git (~GB of 5.24M-row text — git chokes on multi-million-row diffs, slow clones, churn on every discovery re-run; LFS is the right store). Commit the `reference-dev.sqlite` BINARY directly (contradicts D1's DB-out-of-git; unresolvable binary merge conflicts; worse bloat than LFS-CSV). Ship the bulk only via the release page (no version control on it — a manual per-release upload that drifts from the committed curated CSV; "track" became "distribute" — weaker than the user's "track as CSV"). Track ONLY the curated half + keep the dump as the bulk genesis (does NOT meet "don't reconstruct" — the dump stays a required rebuild input). *(Settled 2026-06-10 — the user: "seeds is deprecated... we shouldn't be using the seeds at all anymore"; "we don't want to have to reconstruct. we should be tracking the dev db as csv(s) as well." The curated→git / bulk→Git-LFS split + the dump-retires-to-expert-only + the lossless round-trip-over-bulk bar settled via `senior-architect-consult`; Git LFS chosen over the release-page artifact for version control over the bulk. **Amended by D40 (2026-06-10):** `valid_through` moves from the curated derived-overlay (`address_versions_derived.csv` / `_AV_DERIVED_COLS`) to the AUTHORED curated seed CSV (`address_versions_seed.csv`) — a deliberate edit to the authored-vs-derived seam, with a deliberate round-trip-oracle re-baseline; it stays in the curated→git half, only its sub-classification (derived → authored) changes.)* |

| D39 | The bulk re-verify writes are RESOLVED + computed by the DATA-CORE from the report, not the frontend — a new `reverify_resolver` + a `/save/reverify-batch` preview seam | **The data-core computes the re-verify edit-specs; the frontend sends report rows + the maintainer's confirm, never pre-computed edits.** This is what D28 ("the maintainer imports that report file into the tool, which **computes the writes**") + D34 (the importer extends the matched row's interval using `matched_address_version_id`, a DB row id) already mandate — the FE cannot build the edits, because the v3 report row carries `kcdx_id` + resolved `version` + `verdict` + `method_rank` + `matched_address_version_id` (a DB integer id; NULL on a non-verified-block row), but NOT `valid_from_version` / `valid_through` / `last_verified_at_version` (the write-path identity + the current values the deltas need), and only the data-core speaks DB ids + owns the interval logic (law 6, sole writer). Three units: **(1) a new data-core module `reverify_resolver.py`** — takes the report rows + verdicts and produces the per-row edit-specs in the `{kcdx_id, valid_from_version, edits}` shape `/confirm/batch` already consumes. **Verify-all** (`verified_working` / `passed_not_verified` rows): reads the matched row BY `matched_address_version_id` → its `{kcdx_id, valid_from_version}` (the write-path key) + its current `valid_through` / `last_verified_at_version`; computes the **audit trio** (`last_verified_at_version` → the report version, `verified_date` → today, `verified_by` → the injected identity — D17a), the **proof-rank-keyed `evidence_kind`** (a rank-1 `verified_working` row → `live_production`; a ranks-2–5 `passed_not_verified` row → `live_test_plugin` — D29-rev), and the **D34 gap-extension** (`valid_through` → the report version when the swept version sat beyond the matched row's interval; a row already covering it is skipped — nothing to add). **Close-intervals** (`failed` rows — `matched_address_version_id` is NULL by the report's attribution invariant): resolves the target row DETERMINISTICALLY as the `address_version` row of `kcdx_id` whose interval **contains** the resolved `version` — a `failed` row's version was definitionally covered (a build version in a GAP is `not_applicable`, NOT `failed`, and `not_applicable` is shown-no-action, never in the failing block), so exactly one row contains it; computes `valid_through` → that row's `last_verified_at_version` (the D35 retract). **(2) a new backend preview endpoint `/save/reverify-batch`** — mirrors `routes_save.py`'s preview-only contract: it calls `reverify_resolver`, returns the per-row **field-deltas** (`field: old → new`) for the FE to display (law 5 — the batch field-delta list shown before commit), and **writes nothing, opens no transaction** (Save-previews / Confirm-transacts, D16). **(3) the existing `/confirm/batch` (the 6.2 endpoint) transacts UNCHANGED** — the FE posts the resolver-computed edits the preview returned; ONE atomic transaction, all-or-nothing rollback (D32 / D21). The FE flow (s08): report rows → `/save/reverify-batch` → render the batch field-delta list → maintainer confirms once → POST the same edits to `/confirm/batch`. The **write MECHANISM is unchanged (D19)** — the same `_apply_one_db` batch path + deferred-commit txn + export-on-save; only the edit-spec SOURCE moved from the FE to `reverify_resolver`. The `evidence_kind` of a `verified_working` vs `passed_not_verified` row, and the gap-extension vs the close-retract, are visible in the field-delta list before anything lands (law 5). **The `valid_through` extend/retract the resolver computes is written to the AUTHORED `valid_through` column D40 establishes** — the resolver computes it (this decision holds), D40 settles WHERE it lands (the authored column, not a derived value), that it is hand-overrideable in the single-version editor, AND that landing it requires the data-core write-path extension D40 names (the `_seed_action_rows` slot + the `_apply_one_db` `valid_through`-emitting UPDATE branch reconciled with `_UPDATE_PRESERVE_COLUMNS`) — NOT a mere column reclassification. | The FE computes the edits client-side (the blocker this corrects — the FE has no `valid_from_version` / `valid_through` / `last_verified_at_version` and no DB-id→key mapping; pushes interval/gap logic into the FE against law 6; needs N read calls per batch). A single `/reverify` endpoint with a `dry_run` flag (one endpoint, but departs from the established two-endpoint Save-previews/Confirm-transacts pattern + recomputes the resolve twice unless cached). Resolve in the backend route handler instead of a data-core module (mixes HTTP concerns with interval/gap domain logic; not headless-testable without the FastAPI layer; against data-core-owns-DB-logic). Surface `valid_from_version` in the report or the read API for the FE to map (unneeded plumbing — the resolver already has DB access, so the `matched_address_version_id` → `{kcdx_id, valid_from_version}` lookup is a trivial server-side query; a contract/schema change to avoid a query the data-core can already do). Extend the v3 report to carry the current row values (touches the FROZEN cross-repo report contract + the in-game producer for data the data-core already holds). *(Settled 2026-06-10 — surfaced building s08 step 6.3: the 6.2 `/confirm/batch` edits-precomputed shape wrongly assumed the FE computes the gap-detection + write targets, contradicting D28/D34; Option A (data-core resolves) chosen, then the concrete seam — a `reverify_resolver` data-core module + a `/save/reverify-batch` preview endpoint mirroring `routes_save.py` + the unchanged `/confirm/batch` transact — settled via `/design`. Phase-06 re-plan consequence: a new BE step lands the resolver + preview endpoint before the s08 FE wiring (6.3), whose FE changes from "compute edits" to "send report rows + display the returned deltas".)* |

| D40 | `valid_through` is an AUTHORED + tool-auto-filled column (the audit-trio pattern) — it graduates from the derived-overlay to the authored seed surface, hand-overrideable | **`valid_through` follows the exact authored-AND-auto-filled pattern the audit trio already established (D17a/D17b/D29): it is an AUTHORED column the maintainer owns via the confirm, AND the tool auto-fills its value from the re-verify report.** This resolves how the D34 gap-extension / D35 close-retract `valid_through` write is expressed: `valid_through` becomes a maintainer-editable, seed-tracked column — NOT a derived/maintainer-generated value written by a side-channel. Three settled facts: **(1) AUTHORED — on the seed surface.** `valid_through` is added to `EDITABLE_VERSION_COLUMNS` + the authored seed CSV (`ADDRESS_VERSIONS_CSV_HEADER`, as the tag form `valid_through_version` → resolved to the `game_versions.id` FK exactly as `valid_from_version` resolves) + the seed reader (`read_address_versions_seed`); it is a human-reviewable curated fact, shown in the confirm field-delta like any authored cell. **It MOVES out of the bulk derived-overlay (`_AV_DERIVED_COLS`) into the authored seed** — a deliberate edit to the D38 authored-vs-derived seam (the value was already versioned + round-tripped via the overlay; this re-homes it to the authored side, where a maintainer-owned curated fact belongs). **(2) AUTO-FILLED — the common path.** The re-verify (D39's `reverify_resolver`) computes the value (D34 gap-extension → the report version; D35 close → the matched row's `last_verified_at_version`) and the maintainer confirms it — the maintainer never hand-types an interval on the common path (the disassembler-test spirit applied to data authoring: declare intent — run the sweep, confirm the batch — the tool does the interval bookkeeping). **(3) HAND-OVERRIDEABLE — the `verified_by` model (D17a), NOT the `verified_date` model (D17b).** `valid_through` is also editable by hand in the single-version field editor (s04) — a maintainer can hand-correct a mis-attributed interval or close one the sweep didn't cover, the same way `verified_by` is hand-overrideable for an on-behalf/correction. An **interval validator** gates the edit (auto-filled or hand): `valid_through >= valid_from` (the close is not before the open), no overlap with the entity's other intervals, the open-row uniqueness (one `valid_through IS NULL` per kcdx_id). **Two reconciliations land with the seam move:** the **round-trip oracle re-baselines** (deliberate + inspected — the only delta is `valid_through` moving overlay→authored-seed + any closed-interval values, the same kind of deliberate re-capture as the `char`→`i8` drift), and the **`validate_db_shape.py` "all baseline-open (`valid_through IS NULL`)" invariant** is reconciled so a legitimately-closed interval (satisfying the interval validator) is VALID, not flagged as a baseline violation (a D35 close produces the DB's first closed interval). The SCHEMA is unchanged (D22) — `valid_through` is already a stored typed cell (`schema.py` `("valid_through","INTEGER")`), no new column. But the WRITE PATH is NOT a no-op reclassification — it is real new write-path work the seed-surface addition alone does NOT deliver: today `_seed_action_rows` (`import_to_sqlite.py`) carries no `valid_through` slot, the trio-only re-verify UPDATE writes only the 4 trio columns, and the full-column US-5 UPDATE **deliberately EXCLUDES** `valid_through` via `_UPDATE_PRESERVE_COLUMNS` (`import_to_sqlite.py:2324`) — an exclusion that exists to stop a full-column UPDATE from CLOBBERING (NULLing) an already-closed `valid_through` back to open (`build_curated_row` mints `valid_through=None`, so an unguarded UPDATE would re-open a closed interval → two open rows → the open-interval-uniqueness index `ix_av_open_unique` trips). So the build must: (a) reclassify the column (seed CSV header + reader + curated exporter SELECT + `EDITABLE_VERSION_COLUMNS`, removing it from `_AV_DERIVED_COLS`); AND (b) **extend the data-core write path to actually emit `valid_through` on a re-verify UPDATE** — add the `valid_through` slot to `_seed_action_rows`, emit it in the `_apply_one_db` UPDATE for the re-verify/interval edit, and reconcile that emission with `_UPDATE_PRESERVE_COLUMNS` (the re-verify `valid_through` write sets a non-null ordinal on an existing row — the opposite of the collision the exclusion guards, but the two must be made to coexist explicitly, e.g. the interval edit is a distinct edit shape the preserve-set does not strip). The write still runs through the EXISTING `_apply_one_db` direct-write + deferred-commit spine (D19 — the mechanism is reused, law 6 holds; the data-core stays sole writer); what is new is the `valid_through`-emitting UPDATE branch, not a parallel writer. The interval validator (above) gates it. The engine consumes `valid_through` for resolution (`src/refdb.cpp` — `[valid_from, valid_through]` is the covering interval), so the value must round-trip through a rebuild — it already does (via the overlay today), now via the authored seed. | Keep `valid_through` a derived-overlay column written by a side-channel (the FE/resolver writes it off the authored surface — but then the confirm delta shows a value the seed CSV doesn't carry, splitting "what's reviewed" from "what's authored"; and "it should be authored" is the user's settled intent). Make `valid_through` auto-fill-only, never hand-editable (the `verified_date`/D17b model — rejected: an interval boundary is a curated fact a maintainer may legitimately need to hand-correct, like an `rva` or `signature`; the `verified_by`/D17a hand-overrideable model fits, the system-only model does not). Graduate it to authored WITHOUT the oracle re-baseline (impossible — the round-trip oracle is byte-identical *because* the seam is where it is; moving a column across the seam necessarily re-baselines it; doing so deliberately + inspected is the principled move, NOT oracle-gaming — the trio columns live on the authored seed the same way). Add a NEW schema column for the authored interval (unnecessary — `valid_through` is already a stored cell; D22 flat-final holds, this is a reclassification not a migration). *(Settled 2026-06-10 via `senior-architect-consult` — the user reframed "what actually needs to be versioned and carried through": the consult found `valid_through` was ALREADY versioned (in `_AV_DERIVED_COLS`, round-tripped) and the audit trio (D17a/D17b/D29) is the exact authored-AND-auto-filled precedent. The user settled: "yes it should be authored, but the tool should also auto fill it" + hand-overrideable like `verified_by` + confirmed the overlay→authored-seed move. This supersedes D39's "the resolver writes `valid_through` as a derived value" framing — the resolver still computes it (D39 holds), but writes it to the authored column D40 establishes.)* |

| D41 | The tool guarantees ENTITY-LIFECYCLE COMPLETENESS — a standing "needs action" view surfaces every version-relative incomplete-lifecycle entity, and the report worklist reconciles against current DB state (already-acted-on rows show no-action) | **No entity is ever left silently incomplete at the current game version.** The maintainer tool adds a version-relative lifecycle-completeness layer ABOVE the write-time structural checks (policy.md / validators.py HARD ERRORs catch *structural* integrity — NULL ids, duplicate tuples, supersession-pair XOR, acyclic supersession, `last_verified < valid_from`, audit-trio all-or-nothing, FK closure — but NOT *semantic* completeness at a specific version V, which is a query-time property the importer cannot know). Three settled facts: **(1) A STANDING "needs action" view — the incomplete-lifecycle surface.** A tool-wide view/filter lists every entity whose lifecycle is incomplete at the current game version V, reachable any time (NOT tied to a report import), catching gaps from ANY flow (a close-intervals orphan, a hand-edit, a pre-existing never-verified row). The set of "needs action" shapes — structurally-valid-but-semantically-incomplete-at-V states the write-time checks do not catch — is BROAD (the user: "we need to ensure the lifecycle is complete"): the **uncovered-at-V orphan** (no `address_versions` interval covers V — `valid_from <= V <= valid_through` OR `valid_through IS NULL` — AND `is_deprecated = 0` AND `superseded_by IS NULL`; no row is AUTHORITATIVE for V, so the entity is UNVERIFIED at V — the engine still resolves it, but only by best-matching to a nearest stale closed row with a warning (`src/refdb.cpp` `PickBestVersionRow` returns a best-match unless the entity has zero rows), which is exactly the incomplete-lifecycle state needing a maintainer decision; a strict subset of policy.md §"Status is derived" rule-4 UNVERIFIED — specifically the no-covering-interval case, distinct from a covered-but-stale row), the **never-verified** row (`last_verified_at_version IS NULL` — authored but never signed off), and the broader version-relative integrity set (extensible: a `deprecation_replacement` / `superseded_by` pointing at a nonexistent or itself-incomplete entity, a supersession with no successor row covering V). Each is surfaced with its resolution path (author a successor row for V, OR mark deprecated, OR supersede). **(2) REPORT-vs-DB RECONCILIATION — the tool derives "already acted on" from `(report version + current DB state)`.** A re-imported v3 report is reconciled against the live DB per row: the data-core reads the current DB state for the report row's `(kcdx_id, version)` and derives whether the report's recommended action is ALREADY reflected. This mirrors verify-all's EXISTING skip (`reverify_resolver`: `last_verified_at_version >= swept → return None`, no edit); **close-intervals gains the symmetric skip** — a `failed` row whose covering interval is already closed to `last_verified_at_version` (`valid_through == last_verified_at_version`, non-NULL) is already-done → NO edit-spec (the gap that produced the silent no-op confirm: the close-intervals path emitted a no-op edit with an empty field-delta because it lacked verify-all's skip). NO report-schema change and NO engine change — the report keeps its current shape (the engine's binary assessment, produced BEFORE any maintainer action, so it cannot carry a future action's state); the DB is the live source of truth for "already acted," and the report's `kcdx_id` + `version` are the keys the tool reconciles against. **(3) The s08 worklist REFLECTS reconciliation + the close→needs-action flow + the Fix-flow completeness.** An already-resolved re-imported row MOVES to a "no further action" state (mirrors the existing no-action block: surfaced, not actionable, no checkbox, not in any batch — the spec's "everything surfaced, nothing silent" stance); the actionable verified/failing blocks show only rows still needing the action (a re-imported partly-acted report shows true remaining work). A close-intervals action that ORPHANS an entity (retracts `valid_through` below V with no successor) FLAGS that entity as needs-action (it appears in the standing view) — surfaced, never silent; close-intervals stays ONE atomic transaction (the lifecycle is completed as a SEPARATE deliberate act — author successor / deprecate / supersede — never a forced multi-step). The three branched s08 Fix-flow gaps land with this revision: **`[Fix ▸]` carries the report row's divergence `detail`** (e.g. "on-disk body hash mismatch: build diverged from the recorded version") to the s04 editor so the maintainer sees WHAT to fix; **`[Fix ▸]` gains a return path** back to the report/worklist (the imported report state survives the Fix excursion — no one-way dead-end, since s08 and s02/s04 are peer content screens and a naive navigate-away drops the client-side File-API report state); **an applied row shows the resulting value** (the new `valid_through`), not just "✓ applied". The reconciliation computes at the EXISTING reconciliation point — `reverify_resolver` / the `/save/reverify-batch` preview (D39), which already reads per-row DB state (`_saved_cells` reads `target["valid_through"]`); recognizing the already-done state is re-classifying the already-computed diff, not new plumbing. | A separate per-DB-read "already acted" stamp in the report (the engine stamps each row's sweep-time DB interval state into the report — rejected: the stamp is true only AT SWEEP TIME and goes stale the moment the maintainer acts, AND it is an engine/Phase-4-5 change for data the data-core already holds live; the user: the report should carry what's needed, resolved to "the DB derives it from the report version + current DB state" — the live DB, not a stale stamp). Block stale re-imports entirely (warn/refuse a report whose actions are partly applied — rejected: a maintainer mid-session who closed some rows could not re-open the same report to finish the rest without a full game re-sweep; the worklist must reconcile, not block). Keep the no-op close as a benign idempotent write (leave close-intervals emitting a no-op edit + an empty-delta confirm — rejected: it asks the maintainer to confirm a batch that changes nothing, the silent-no-op shape, and does NOT solve the "frames everything as new" complaint — only the symptom). Scope "needs action" to only the post-close orphan this session (an inline prompt, no standing view — rejected: misses pre-existing orphans + orphans from other flows; the user chose the broad standing view). Hide already-done rows with a toggle (rejected: violates the s08 "everything surfaced, nothing silent" principle — surfacing-by-default is the repo's stance). Fold the lifecycle work into the engine (re-sweep so a closed row never re-reports `failed` — rejected: requires a fresh game sweep per reconciliation, can't reconcile a re-imported OLD report, and is a much bigger engine change; the already-acted-on knowledge is a tool/DB concern, not an engine one). *(Settled 2026-06-12 via `/design` — surfaced during live acceptance of Phase 6 (6.3): re-importing a report showed an already-closed interval as a fresh failing row, and confirming produced a no-op edit with an empty field-delta. The user reframed the root: "the report should carry the information needed for the tool to understand if it's already been acted on" → resolved to "can't the DB derive that based on the report version it ran on and the current DB state" (the live-DB derivation, verified computable); then "we just need to make it clear if an item was closed but has no successor and has not been marked as deprecated/superseded by — needs to be visible as 'needs action'" → the broad standing lifecycle-completeness view ("yes, we need to ensure the lifecycle is complete"). The already-acted display = a "no further action" state; the close→flow surfaces the orphan, resolved separately; the three s08 Fix-flow gaps folded in. This EXTENDS D39 (the resolver gains the close-intervals already-done skip) and D40 (the authored `valid_through` is what the close writes / the orphan check reads) — neither is superseded.)* |
| D42 | The frontend has an in-app BACK-STACK navigation model (no URL router); frames carry full screen STATE; a resolve-action pushes, a top-level destination resets | **Navigation between content screens is an in-app back-stack, and `‹ back` restores a screen exactly as it was left — not a fresh re-mount.** Each frame stores the screen AND its live in-memory state (s08's ingested report + block split + scroll; s09's toggles + scroll; s02's selected version + expanded sections). A context-carrying resolve-action (an s09 row's resolve, an s08 `[Fix ▸]`) PUSHES a frame (building the s09→s02→s04 fix chain); a deliberate top-level destination (a navigator entity selection, `[Needs action]` / `[Import report]` / `[+ New entity]`) RESETS the stack to a fresh root, so `‹ back` never detours through an abandoned task. The app shell (persistent navigator + swappable content area + overlay layer) is unchanged (`ui/design.md` laws 2/3); the back-stack governs the content area only. On wide, the navigator stays always-live (a navigator selection is never on the stack) and a `‹ back` labeled with its destination appears in the content pane at stack depth > 1; on phone the same content-stack drives the drill-down back. The state-carrying frame is REQUIRED for the s08 report (D43-adjacent): the report is client-side only (D31), so an identity-only frame would force a re-upload on `‹ back`. | A URL router (react-router) with real routes (adds shareable/bookmarkable URLs + browser-back, but a routing dependency + bundle cost against the thin-shell framing, and a URL-back model the screen specs don't reference = drift); OR identity-only frames that re-fetch state (fails the s08 client-side report — `‹ back` lands on the empty import state, the exact bug). |
| D43 | The version selector's source includes the linked Bin's resolved version, even when NEW — it auto-selects, drives authoring, and is ephemeral until a row commits at it | **A linked Bin folder whose WHGame.dll resolves to a version the DB does not know auto-becomes the selected version at the top, marked "from your linked Bin · new" — the maintainer never hand-types a version string.** The selector is populated from TWO sources: the server-known `game_versions` tags AND the Bin-resolved version (the `.rdata` scan, D15/D30). The resolved-new version is a first-class selectable that DRIVES authoring (it is the version a new-row authoring/verify action targets, feeding the D30 link-to-create flow), and is EPHEMERAL until a row is authored + AP18-confirmed at it — a `game_versions` tag is a deliberate curated fact, born when a row commits at it, never a folder-link side effect (the AP18/law-8 deliberate-addition discipline). This is the maintainer-side disassembler-test (`cornerstones.md`): the tool reads the version from the artifact the maintainer linked, never making the human supply what the folder carries. On a phone / no-install host, the dropdown + newest-row default (D10) is the path. | Make the maintainer hand-type a new version tag (the disassembler-test FAILURE — shoving the tool's resolving job onto the human; the "you have to manually type it in" defect); OR auto-register the new version into `game_versions` on folder-link (a curated DB fact created by a side effect — undoes AP18; a linked-then-abandoned beta build pollutes the tag list). |
| D44 | Navigating away from a dirty editor is intercepted by a Save / Discard / Cancel confirm — nothing saves OR is lost silently | **Any navigation away from a screen with unsaved edits (a stack-reset, an entity switch, a `‹ back`) surfaces a confirm overlay before the navigation: "You have unsaved changes to `<entity/row>`. [Save changes] / [Discard changes] / [Cancel]."** Cancel stays (nothing lost); Discard drops the edits then navigates (the loss is explicit, never silent); Save runs the normal save spine then navigates. The guard forecloses BOTH failure modes — the maintainer is never surprised by lost work, and never tricked into thinking a discard committed or a navigate-away silently saved. It is HARD on silent loss (a dirty editor cannot be navigated away from without the choice) and complements law 5 (one confirmed transaction). "Dirty" = the s04 field editor or s02 lifecycle editor holds pending edits not yet through the s06 confirm. | Auto-save-on-navigate (silently commits + git-pushes an edit the maintainer meant to abandon — the opposite surprise; also trips AP18 on a new row); OR hard-block navigation while dirty (same safety, more friction — the confirm gets the same no-silent-loss guarantee with one click). |
| D45 | A `[Fix ▸]` arrival on a failing row shows the PER-FIELD divergence (recorded vs actual) inline at the field to edit — re-derived IN-BROWSER against the report's DLL, NOT from the engine report | **When the maintainer opens s04 via a `[Fix ▸]` on an s08 `failed` row, the editor surfaces WHICH recorded `address_versions` field diverged from the running build, recorded-vs-actual, inline at the field — re-derived client-side, with no engine or report-schema change.** This sharpens D41's Fix-flow `detail`-carry: the engine report carries only a whole-row `verdict` + a prose `detail` (e.g. "on-disk body hash mismatch"), because **the in-game sweep computes ONE whole-body-hash verdict per entity and CANNOT isolate which DB field diverged** (`src/survival_verify.cpp` `MapStaticVerdict` — a single `Status` from a whole-body `content_hash` compare + a `.text` reachability range test; the per-field facts — `rva` / `signature` / the survival columns — are DB columns, not separately-verified sweep outputs). So "show the actual field that's wrong" is computed by the TOOL, not reported by the engine. Five settled facts: **(1) DIFF SOURCE — the in-browser per-kind check, reused.** The diff re-runs the EXISTING s04 per-kind static check (`frontend/src/editor/verdictCheck.ts` `runVerdictCheck` → `extractSurvivalCheck` → `checker`) over a linked DLL, per the kind-relevant field set (s04 §"Field relevance by kind"): a `function` row diffs `rva` (→ resolved site) + `signature` (→ body-hash compare); a `callsite` / `instruction_anchor` diffs `survival_aob` (→ `.text` match count); a `string_anchor` diffs `survival_anchor_string`; a `vtable_base` diffs `survival_slot_count`. NO new check is written — the existing per-kind machinery already produces these verdicts in-browser (the no-upload invariant, D26). **(2) 'ACTUAL' DERIVES FROM THE REPORT'S DLL — the build that diverged.** The diff derives "actual" from the SAME running-game DLL the in-game sweep ran against (the report carries `game_version`; the maintainer links/picks that running build) — NOT a version-matching DLL. A `failed` row failed BECAUSE the build diverged from the recorded version, so the divergent build IS where "what's actually there now" lives; deriving from a version-matching DLL would diff against the wrong build (often unavailable on the exact path that motivated the feature). This widens the existing s04 check (documented against a version-matching DLL) to the divergent-DLL case — **whether each kind's existing check behaves correctly against a divergent DLL is a checkable claim, `assumes correct-against-divergent-DLL — unverified, probe before building`** (a probe/spike against a real divergent-DLL fixture is the first build step, `results-driven.md`). The probe's scope ALSO covers per-field ATTRIBUTION: `runVerdictCheck` (`verdictCheck.ts`) today returns ONE row-level `CheckResult` per row, not a per-field verdict (a `function` row runs one body-hash compare keyed off the recorded `content_hash`), so mapping that single verdict to the specific diverged field (`signature` vs `rva` for a `function` row) is a thin new attribution layer the build adds — `assumes the row-level verdict can be attributed to the diverged kind-relevant field — unverified, probe before building`. The probe confirms both the divergent-DLL behavior AND the per-field attribution before the inline diff is built. **(3) NO-DLL-YET STATE — recorded values + a non-blocking prompt.** On `[Fix ▸]` arrival before a suitable DLL is linked (the COMMON case — the divergent build is new, so no version-matching DLL exists): s04 shows every RECORDED field value immediately + the engine's prose `detail` + a non-blocking prompt "Link the running-game DLL to see what diverged" (prefilled from the report's `game_version`); the per-field "actual" column fills in once the DLL is picked. NEVER block the editor (law 4 — the existing s04 "no DLL → advisory, authoring proceeds" contract holds; a maintainer who wants to hand-correct an rva they already know is never gated behind a file pick). **(4) PROMINENCE — inline per-field + the summary banner.** Each diverged kind-relevant field shows its recorded-vs-actual INLINE, directly where the maintainer edits it (recorded value, the derived "actual", a diverged marker glyph+text never color-alone — law 7), reusing s04's reserved-space dirty/verdict gutters (law 1); PLUS the existing E5 "What diverged" top banner (`COPY.divergenceTitle`) is EXTENDED to name the diverged field(s) at a glance. The inline-at-the-edit-point is the literal ask ("the actual field you would edit should be very clear showing what is different"); the banner is the glance-level overview. **(5) STATES.** no-DLL-linked (recorded + prompt, fact 3) · DLL-linked-and-diffed (inline recorded-vs-actual + banner, fact 4) · no-divergence-found (the check PASSES against the new build — surfaced honestly, "no field diverged against the linked build", not a silent empty) · check-cannot-run (a per-kind CannotCheck, e.g. `vtable_index` deferred, or a function row with no recorded `content_hash` — honest, advisory, never a faked pass). FRONTEND-ONLY (the gitignored FE repo); no `kcdx_verify_all` change, no v3→v4 report-schema bump. | Extend the in-game sweep to compute per-field divergence + emit a v4 report (the engine RE-DERIVES which field diverged from the live binary — rejected for NOW: real disassembler-tier engine RE per row (reverse-derive the signature/rva from the live body), a hard task that pushes the engine's job up a tier (the disassembler-test risk), AND a `schema_version` bump both repos re-parse against the frozen-v3 guarantee; effort is not a cornerstone, but the engine-side capability is genuinely larger — deferred as a separate later feature for the no-DLL case, NOT built here). Derive "actual" from whatever DLL is already linked on s02 (rejected — may diff against a version-matching-but-WRONG build, showing a misleading/empty diff exactly when the build diverged; the common `[Fix ▸]` case is precisely when no version-matching DLL exists, so the diff would be most often unavailable — weak on the path that motivated it). Block the editor until a DLL is linked (rejected — turns an advisory check into a gate, violates law 4 + the existing s04 degraded-state contract). Top banner only / inline-only (rejected — the banner-only loses the at-the-edit-point clarity the user asked for; the inline-only drops the glance-level "what diverged" overview the E5 banner already established). *(Settled 2026-06-14 via `/design` — surfaced during the Phase-3 milestone UAT of the s08 Fix-flow: the `[Fix ▸]` banner carried only the engine's whole-row prose reason, so "it doesn't nicely show me what is wrong"; the user sharpened it to "the actual field you would edit should be very clear showing what is different" (recorded-vs-actual). The engine-capability fact — the sweep computes one whole-body hash, cannot isolate a field — was confirmed by reading `src/survival_verify.cpp`; the in-browser re-derive is the smallest correct surface that actually shows recorded-vs-actual. EXTENDS D41 (the Fix-flow `detail`-carry gains the per-field diff) and reuses D26 (no-upload in-browser check) / the s04 per-kind check (D24–D31); supersedes nothing.)* |
| D46 | Report-vs-DB reconciliation ALSO fires on IMPORT, not only on a batch-action round-trip — the s08 worklist classifies every actionable row against current DB state at import time | **On importing a v3 report into s08, the tool reconciles every actionable block's rows against current DB state BEFORE the worklist's first paint — so a re-imported, partly-acted report shows already-acted rows in "no further action" without the maintainer first opening a batch action.** This EXTENDS D41 fact 2: that decision built the report-vs-DB reconciliation but wired its only invocation to the `/save/reverify-batch` preview that runs when a batch action opens — "the EXISTING reconciliation point, not new plumbing." A plain re-import-and-look (the maintainer re-opens a report to see remaining work, takes no new batch action) never triggered that preview, so the already-acted map stayed empty and an already-closed row re-classified into its actionable block from the report verdict alone (KI-0023). The fix invokes the SAME `/save/reverify-batch` preview on import, once per actionable block (failing → close-intervals, verified → verify-all), and feeds the returned `already_acted` rows into the worklist before first render. The FE still READS the classification, never re-derives it client-side (D41 fact 2 holds — the resolver/preview owns the already-done predicate); only the INVOCATION POINT is added (also on import, not just on batch-open). No new endpoint, no new resolver logic. The import-time reconciliation degrades gracefully (a per-block preview failure is logged and that block is left un-reconciled — rows stay actionable, the pre-fix behavior — never a thrown error blocking the worklist, never a silent empty worklist; the import already succeeded, reconciliation is layered on top, AP14 fail-loud-but-don't-drop). | Reconcile only on a batch action (the as-built D41 behavior — rejected: KI-0023 is exactly the re-import-and-look path, which never opens a batch; the spec's s08 §"Already acted on" promises the already-done row shows "no further action" on a re-import, which this path could not deliver). Add a dedicated `/reconcile` classify-all endpoint (rejected — unnecessary new plumbing; the `/save/reverify-batch` preview already returns the per-row `already_acted` classification + reason, so the import path reuses it per action). Compute already-done client-side in the FE from loaded entity data (rejected — the FE holds only an id→name map, not per-version interval state; computing the `valid_through == last_verified_at_version` predicate client-side re-implements the data-core's interval logic in the FE, the D39-rejected law-6 violation). Narrow the spec to "reconciliation only after you act" (rejected — trades the UX cornerstone for implementation effort; the already-done row still shows as failing on a pure look, the exact failure D41 was authored to remove). *(Settled 2026-06-15 via `/debug KI-0023` Gate-A `architect-review` + user decision — surfaced during the verification-engine Phase-6 milestone UAT: a close-intervals batch landed correctly, but re-importing the same report showed the acted row broken again. The architect-review confirmed (cold) the maintainer's expectation is correct — the s08 screen spec §"Already acted on" promises "No further action" for an already-closed `failed` row, which the as-built batch-only reconciliation could not deliver on the re-import-and-look path. EXTENDS D41 (reconciliation's invocation point) and reuses D39's `/save/reverify-batch` preview; supersedes nothing. FE: `46d3c09`.)* |

| D47 | A diverged field on the s04 `[Fix ▸]` arrival is wrapped in a RED-BOXED emphasis (an `error`-token border + faint tint around the field row), so the maintainer's eye lands on the break — refining D45's per-field divergence render | **When s04 is reached via an s08 `[Fix ▸]` on a `failed` row, a DLL is linked, and the per-field diff has computed (D45), each diverged kind-relevant field is wrapped in a `diverged-field box`: an `error`-token border (1–2px) + a faint `error`-tinted row background around the whole field row.** This sharpens D45's "PROMINENCE" fact (4): D45 placed the recorded-vs-actual diff inline at the field with a glyph+text diverged marker + the "What diverged" banner; the maintainer's report was that the eye still scans the gutters to find the broken field. The box is the visual locator. Four settled facts: **(1) TREATMENT — border + tint, no reflow.** The box reuses the field row's EXISTING reserved box (input + dirty-marker gutter + "was:" line + verdict-badge space), so it adds NO layout and shifts nothing (`ui/design.md` law 1); the border + tint resolve to the `error` role token (`#f87171` dark / `#dc2626` light), never a raw value (law 9). **(2) NEVER COLOR-ALONE — composes with the marker.** The diverged glyph+text marker (D45) is RETAINED — the divergence cue is glyph + text + box, so the box is reinforcement, never the sole signal (law 7); a maintainer who cannot distinguish the border color still reads the marker. **(3) EVERY DIVERGED FIELD BOXED EQUALLY.** When more than one kind-relevant field diverges (a `function` row where both `rva` and `signature` diverge), each gets its own identical box — no priority ordering, no single "primary" highlight; the "What diverged" banner continues to name the full diverged set at a glance. A kind-relevant field that did NOT diverge shows no box. **(4) PERSISTS THROUGH THE EDIT until saved.** The box marks the DIVERGENCE FACT (this field diverged from the linked build), which holds until the correction commits — so the box persists while the field is dirty (the existing dirty marker layers ON TOP, in its own reserved gutter — both visible, no reflow), and clears only after the save lands and the row is no longer divergent. It never clears on the first keystroke (a half-typed or wrong correction must keep the "this was broken" signal until a saved, re-checked row says otherwise) — the static divergence result drives the box, the dirty marker is the orthogonal edit-in-progress signal, the two compose. FRONTEND-ONLY (the gitignored FE repo); reuses the existing `error` token (NO new token); no engine / report-schema / data-core change (the diff is already computed in-browser, D45). | Accent gutter bar only — thicken/red-fill the existing dirty-marker gutter, no full box (rejected: less prominent — the eye still scans somewhat; the user chose the full row box for immediate locating). Clear the box on first edit — dim to the normal dirty treatment once the maintainer starts typing (rejected: the divergence cue vanishes before the fix is confirmed, so a half-typed or wrong correction loses the "this was broken" ground-truth signal; the user chose persist-until-saved). Box every diverged field PLUS lead the "What diverged" banner with a count ("2 fields diverge: rva, signature") (rejected: the user chose equal boxes with the banner naming the diverged set at a glance, without a count — the box set itself conveys the multiplicity, a count is redundant banner copy). Box + auto-scroll the first diverged field into view on arrival (considered — most prominent, but adds an on-mount scroll behavior; the user chose the box treatment without the scroll for this cycle). Color-only border with no marker (rejected outright — color-alone violates law 7). A new dedicated "danger-box" token (rejected — the `error` role already carries exactly this "validation error / write failure / blocked" semantics; a new token is token-sprawl against law 9's single-source intent). *(Settled 2026-06-15 via `/design` — Step 1b of the maintainer-tool-completion-sequence, surfaced as a UX-enhancement request during the verification-engine Phase-6 acceptance: "make WHAT broke more visible — a red box around the diverged element." A UI design revision; cornerstone UX (immediate visual locating of the break at zero layout cost, holding laws 1/7/9). EXTENDS D45 (sharpens its prominence fact) and reuses the `error` token + the D45 in-browser per-field diff; supersedes nothing.)* |
...
PySide6 desktop-`.exe` delivery (D14–D18 — the tool is now a hostable web app), the
`survival` sibling-table shape (D22 folds it into `address_versions`), the static-only
4-token verdict vocabulary (D25 → D36's 7-state enum + rank ladder), the boot-automatic
sweep trigger (D33 → D36's console-after-save-load), and the seeds-as-frozen-genesis +
from-dump rebuild model (D20 → D38: seeds retire, both DBs rebuild from tracked CSVs —
curated git + bulk Git LFS — the dump retires to expert-only).**
...
**The `kind` enum (`policy.md` §"kind") is the closed set of things the
...
### 11.6 The per-kind verification matrix (D36) <a name="11-6-verification-matrix"></a>

The verdict-ceiling each `kind` can reach, and its default `invoke_attempted` posture — the
data-side companion to D36's enum + rank ladder. The verdict per row is the ceiling of the
strongest method that actually ran; a `failed` at any rank overrides the ceiling downward.
Counts are the curated USER set as of 2026-06-09 (157 rows / 9 kinds).

| `kind` | n | Strongest applicable attempt | Ceiling verdict | `invoke_attempted` |
|---|---|---|---|---|
| `function` — kcdx-HOOKED | subset | **rank 1** — observe hook fired this session + correct pass-through | **verified_working** | false (game calls, we observe) |
| `function` — kcdx-CALLED in prod | subset | **rank 1** — kcdx's own production call already ran + returned correctly | **verified_working** | false (kcdx already called it) |
| `function` — safe-read getter | small | **rank 2** — safe-read the live value (zero mutation) | passed_not_verified | true (`safe read`) |
| `function` — foreign, uncallable | bulk of 125 | **rank 4** — on-disk hash + reachability | passed_not_verified | false (`unsafe_to_call`) |
| `function_no_sig` | 10 | **rank 4** — hash + reachability (no ABI to call) | passed_not_verified | false (`not_a_callable_kind`) |
| `function_variadic` | 2 | **rank 4** — hash + reachability | passed_not_verified | false (`unsafe_to_call`) |
| `callsite` | 4 | **rank 3/4** — AOB uniqueness + reachability (Ambiguous if multi-hit, D31a) | passed_not_verified | false (`not_a_callable_kind`) |
| `string_anchor` | 1 | **rank 4** — `.rdata` literal present + xref | passed_not_verified | false (`not_a_callable_kind`) |
| `instruction_anchor` | 1 | **rank 3/4** — resolver-chain derivation | passed_not_verified | false (`not_a_callable_kind`) |
| `data_slot` | 4 | **rank 5** — derivation check (no content hash, D25) | passed_not_verified | false (`not_a_callable_kind`) |
| `vtable_base` | 4 | **rank 3** — N-qword table-shape + each entry resolves into `.text` (read-only walk) | passed_not_verified | false (`not_a_callable_kind`) |
| `vtable_index` | 6 | **deferred** — slot index, no live-vtable resolution path yet | **cannot_check** | false (`not_a_callable_kind`) |

The rank-1 membership (WHICH specific functions reach `verified_working`) is the kcdx-hooked +
kcdx-called set — the `engine.*` chain entries (`lua_pcall`, `lua_newstate`, `ccrypak_fopen`,
`ccrypak_adjustfilename`, `serialization`, `CGame_Update`, `SaveGame`/`LoadGame`) + the
production-called accessors (`ICVar_GetIVal/GetFVal`, `IConsole_ExecuteString/AddCommand`) — on the
order of ~10–15 rows, the exact membership being a runtime fact the sweep discovers (which hooks
actually fired this session), not a pre-baked count. The honest headline: a small `verified_working`
set (the observed-execution rows), the bulk landing `passed_not_verified` (actively
hash+reach-checked, capped honestly), the 6 `vtable_index` rows `cannot_check`, and any divergence
flipping a row to `failed`. Every row gets an active attempt and a real response.
