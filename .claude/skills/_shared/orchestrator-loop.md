...
- **Source work-item** — the tracked work-item this step lands, as `<doc path> → <ledger step id>`, OR `none` for an untracked one-off. When set, the step's diff MUST flip that ledger row to `DONE` with the Commit cell `(landed)` (NOT a hash — §C item 3 explains why; the hash backfills one commit later); the ledger contract is `docs/outstanding-work/README.md` §"Status ledger". `none` skips the write-back entirely.
...
3. **Source-ledger row updated?** If the caller-injected **Source work-item** is set (not `none`), the diff MUST flip that ledger row to `DONE` with the Commit cell `(landed)` — NOT a hash — in the source doc named, per `docs/outstanding-work/README.md` §"Status ledger". Absent for a tracked step → step failed (same class as a missing test plugin); re-task for it. `none` skips this check.

   **Do not write a hash into this commit.** A commit cannot contain its own hash; `git rev-parse HEAD` before committing yields the PRIOR commit's hash (the stale-hash bug). The cell stays `(landed)` and the real short hash backfills one commit later: at the START of the NEXT step's §A brief, the manager injects "also replace the prior row's `(landed)` with `<the hash captured at that step's §C-commit>`" into the authorized scope, so the next step's commit carries both its own `(landed)` and the prior row's now-known hash. The plan's FINAL step leaves a trailing `(landed)`; the manager lands a tiny follow-up `/commit` that backfills it (capture this step's hash after §C-commit, edit the cell, commit).
...
- **A tracked step's source-ledger row flips to `DONE` + `(landed)` in the step's own commit** per §C item 3, when the caller injected a Source work-item. The hash is NOT written here (a commit can't contain its own hash) — it backfills one commit later. The commit message is not the completion record — the ledger row is. `none` (untracked one-off) skips it.
