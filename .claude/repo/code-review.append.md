...
- **Immutable-review-files guard** — the system-registered `guard-immutable-docs.py` makes the `.claude/skills/code-review/<branch>/<hash>/` findings paths immutable to working agents; the heredoc-write requirement is enforced, not convention.
