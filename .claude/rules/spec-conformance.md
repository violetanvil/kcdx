...
- **A totalizing invariant governs the gaps under it — an unstated case defaults IN-scope, never back to the replaced system.** When the settled design states a TOTALIZING invariant — a takeover, a full replacement, "X owns ALL of Y", "every Z is now ours" (kcdx took over the mod loader, then init, now the filesystem — design §1 "kcdx IS the engine filesystem; every file operation is kcdx's") — that invariant is the load-bearing intent, and a case the design describes only on its happy path (the common/indexed/expected input) does NOT thereby fall outside it. The unstated edge case (the miss, the write, the non-common input, the long tail) defaults INTO the invariant's scope: kcdx owns it too. A design section detailing only the happy path is an incomplete DESCRIPTION of a complete invariant, not a scoping-down of it. Reading that silence as "this case isn't ours" — and recommending it be handed back to the system being replaced (coexistence, fall-through to the engine/legacy path, "the engine still does that one") — is the defect this clause names. Surface the gap (above), and when you do, the recommendation **leans toward the invariant** (kcdx handles it), never toward the replaced system — unless the design's OWN stated deferrals/scope (a §"deferred/out", an explicit exception) put the case out, in which case cite that, not the section's silence. Effort never narrows it (`cornerstones.md` — implementation effort is never a valid reason to weaken a cornerstone or cut scope; the correct answer that just costs more is still the answer).
