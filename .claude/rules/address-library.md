...
- **A NEW seed row requires explicit user approval before it lands (AP18).** Adding an entity/version row to `address_names_seed.csv` / `address_versions_seed.csv` grows the Address Library DB — a new curated game-binary target the project commits to maintaining across versions. STOP and get the user's explicit sign-off on the specific entity BEFORE writing the row; an addition that lands without it is unauthorized. This gates only the seed-row ADDITION, NOT the always-on resolve-by-name expectation (AP1) and NOT an UPDATE to an existing row (re-verify / deprecate / supersede). Warn-only `guard-seed-approval.ps1` flags the addition at author-time; the review gates (`code-review` / `step-review`) carry the hard check. Full authoring law: [data/seeds/policy.md](../../data/seeds/policy.md) §"DB additions require explicit approval".
