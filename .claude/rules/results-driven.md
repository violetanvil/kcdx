...
**Probe-archive hygiene (applies outside `/debug` too).** A `// === DIAGNOSTIC (PROBE X)` engine edit is **disabled in place** (compile-disabled `#if 0` block with the four-line archive header per `debug/SKILL.md` §3d) before the next probe, unless the next probe explicitly builds on it. Never stack two LIVE probe sites; ARCHIVED `#if 0` sites are inert at compile time and don't count (the rule is no-two-live, not no-two-on-disk; `debug/SKILL.md` §2f; `guard-probe-stack.ps1` enforces). **Reverting / deleting the probe is forbidden** — the investigative wiring is the cheapest jumping-off point for the next investigation into the same subsystem (CLAUDE.md hard rule). An archived probe IS commit-eligible (it ships dormant alongside its known-issue's Resolution; only LIVE probes stay uncommitted). Running a probe loop here without entering `/debug` does not exempt the edit from the archive discipline — if the loop runs more than two probes or the investigation turns hard, switch to `/debug` for the active-instrumentation tracking.
