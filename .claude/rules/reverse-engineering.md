...
1. **Check the Address Library** — `data/address-library/seed.csv` + `src/address_library.cpp::kEntries[]`. If an ID exists, use its address; if its prose/`signature` already carries the ABI fact, cite it. A row may answer *partially* (args known, return unknown — the cap-21/cap-22 IConsole case): descend only for the missing piece.
