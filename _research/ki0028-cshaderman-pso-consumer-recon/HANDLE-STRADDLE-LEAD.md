...
**Net after PROBE T + PROBE U — the filesystem-OBJECT layer is FULLY exonerated.** Every kcdx OUTPUT is correct (bytes/handle/sizes/enum) AND kcdx owns the one CCryPak object (vtable + global) end-to-end. The black-geometry divergence is NOT in the object kcdx swapped, NOT in what it serves, NOT in the handle, NOT in a second object. The swap's effect must therefore be a **side-effect of the SEAT ITSELF on engine state OUTSIDE the CCryPak object**: the engine's ORIGINAL CCryPak construction does work on the wider engine/render state (registers itself, sets a global, initializes a sub-object, signals a readiness gate) that kcdx's vtable-only swap does NOT reproduce — OR a timing/order effect of WHEN kcdx seats vs when the engine expects the filesystem ready. The vtable swap replaces DISPATCH but runs none of the original CCryPak's own constructor/init side-effects on the world. The next probe targets the seat's side-effects on engine state beyond the object.
