...
| [p1-ccrypak-construction-order.md](p1-ccrypak-construction-order.md) — PROBE P1 (CCryPak construction order vs first file call + ready-bracket) — answered STATICALLY (binary read, no launch) | `src/asset_overlay.cpp` + `src/mod_absorb/ctor_bracket.cpp` PROBE_P1 blocks (removed, no residue) | OUTCOME (c) — CCryPak built+published (id 158) BEFORE the first gEnv+0x50 file call AND the ModManager ready-bracket → swap seats at the construction site, not the ready-bracket | file-system-takeover §8 P1 |
| [p2-p4-seating-and-ki0019-persists.md](p2-p4-seating-and-ki0019-persists.md) — P2/P4 (seating-spike re-verification: swap seats + holds; KI-0019 still reproduces with the swap active) — live launch + cdb dump read | spike instrumentation in `src/fs_takeover/` (slot-36 marker; removed in a later step once the real slots prove seating) | P2/P4 PASS (swap live on `./system.cfg`, boots through 101 thunks); KI-0019 PERSISTS by design (only slot 36 kcdx-owned; read family thunked) → resolves at step 4.2 | KI-0019 (step 1.4, `3be161a`); file-system-takeover §9 |
| [vanilla-pak-format-confirmed.md](vanilla-pak-format-confirmed.md) — vanilla-pak format check (static on-disk head+EOCD read of real `<game>/Data/*.pak`) — no launch, no source residue | none (static read of on-disk paks; no in-source probe) | CONFIRMED — 8 vanilla paks all `PK\x03\x04`, GeomCaches.pak full read = standard PKZIP, `PK\x05\x06` EOCD, no zip64, STORED+DEFLATE → design §6 `assumes` discharged | file-system-takeover §6 (step 2.2) |
| [p3-off-vtable-handle-rep.md](p3-off-vtable-handle-rep.md) — PROBE P3 (off-vtable raw-handle access vs FOpen handle) — answered STATICALLY from primary-evidence recon (no launch, no source residue) | none (static read of front3/F5/step2-DS recon; no in-source probe) | OUTCOME 1 — no off-vtable access to a FOpen-class handle (read family dispatches on the handle tag through the vtable; the one off-vtable raw op is on an engine-minted pak-MOUNT handle, not FOpen) → kcdx handle-id is safe; read family must stay kcdx-owned | file-system-takeover §4.4 / §8 P3 (step 3.1) |
