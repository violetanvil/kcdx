...
**Next-unused-integer**, partitioned by range. The repo ships an
`address-library/database.csv` and a `NEXT_AVAILABLE_IDS.md` file that lists
the next ID to claim in each range. PR authors edit both files atomically.
...
| Community member finds a new site | PR against `address-library/database.csv` + `NEXT_AVAILABLE_IDS.md`. Reviewer checks AOB uniqueness in `.text` of the current build. Merge if uniqueness holds and the description is concrete enough to re-derive. |
