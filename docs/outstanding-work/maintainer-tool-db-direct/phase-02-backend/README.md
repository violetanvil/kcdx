...
| 4d data-core SCOPED restore-point (D21) — capture the touched rows (address_versions/survival/game_versions/address_names, both DBs) + sqlite_sequence + the data/db-export/ CSVs BEFORE the commit, inside apply_direct_edit; restore them on a POST-commit failure (the deferred ROLLBACK covers pre-commit). A few-KB capture, never the 1.3GB DEV DB; a data-core capability (D13/law 6 — write semantics) | DONE | 8c9758a |
| 5 Confirm transaction (D16/D19/D21) — ONE synchronous request: open the 4a txn → 4c DIRECT-write → commit DB → export to data/db-export/ (D20) → cheap integrity check → git commit/push → success/failure; ROBUST rollback on any failure (deferred ROLLBACK pre-commit; the 4d SCOPED restore-point post-commit, incl. PK reset); EVENT-DRIVEN index.lock (git's exit, no poll); auth-ready seams (D17, + dev default); reuses the kept step-5 WIP git/auth machinery (drops its full-file snapshot → calls 4d) | DONE | 75442e0 |
...
4d. [step-4d-data-core-scoped-restore-point.md](step-4d-data-core-scoped-restore-point.md)
