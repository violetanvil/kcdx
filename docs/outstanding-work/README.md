...
- [asset-replacement/](asset-replacement/README.md) — **active, not started.** kcdx's asset-replacement surface — one `assets/` folder, explicit replacement declarations (per-asset sidecar or code), navigable `kcdx.plugin.<author>.<plugin>.*` cross-plugin references, transparent per-class staging. The most-touched mod-authoring surface for TCs. Re-decomposes the restructure Phase 8.5 scope against the settled design at [`restructure/phase-08.5-asset-replacement/asset-design.md`](restructure/phase-08.5-asset-replacement/asset-design.md); the production FOpen hook + the `[entrypoints].assets` overlay map are already built (the landed foundation). 3-phase plan (probe-gated resolution → author surface → regression); manipulation deferred. Phase tree authored by /plan; canonical phase-grain ledger in [asset-replacement/README.md](asset-replacement/README.md).
