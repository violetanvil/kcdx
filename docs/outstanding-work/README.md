...
- [asset-replacement/](asset-replacement/README.md) — **active, NEEDS RE-PLANNING against the corrected seam.** kcdx's asset-replacement surface — one `assets/` folder, explicit replacement declarations (per-asset sidecar or code), navigable `kcdx.plugin.<author>.<plugin>.*` cross-plugin references, transparent per-class staging. The most-touched mod-authoring surface for TCs. **Canonical design:** [`../design/asset-replacement.md`](../design/asset-replacement.md) — kcdx OWNS resolution by REPLACING `CCryPak::AdjustFileName` (slot 1, id 152), independent of `sys_pakPriority`, seam installed in the already-shipping ready-bracket. The existing tree was decomposed around the superseded FOpen-redirect mechanism and must be re-run through /plan against the canonical doc before /execute; the `[entrypoints].assets` overlay map (`2588b33`) is valid landed foundation. 3-phase shape (probe-gated resolution → author surface → regression); manipulation deferred.
