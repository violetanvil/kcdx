...
- [maintainer-tool-db-direct/](maintainer-tool-db-direct/README.md) — **active, not started.** Build the maintainer tool: DB-direct authoring of the Address Library with CSV auto-export — the COMPLETE six-job tool (create entity, re-verify, supersede, deprecate, create version, edit-any-version + compare), built to the settled TRD + UI design layer. 4-phase plan (headless data-core → PySide6 GUI spine → full jobs → PyInstaller .exe). Phase tree authored by /plan; canonical phase-grain ledger in [maintainer-tool-db-direct/README.md](maintainer-tool-db-direct/README.md).
