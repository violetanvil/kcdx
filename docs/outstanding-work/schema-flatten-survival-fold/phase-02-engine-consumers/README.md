...
| Step | Status | Commit |
|---|---|---|
| 4 engine SELECT + decode + ResolveResult — src/refdb.cpp DecodeVersionRow reads the folded columns; ResolveResult (refdb.h) gains the fields (append-only); the survival pass reads av columns, not the sibling | NOT STARTED | — |
