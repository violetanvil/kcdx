# Step 3.5 — subsume the asset_overlay.cpp two-hook seam
...
kcdx's vtable slots (slot 1 = the index lookup, slot 36 = the kcdx open, and the
read serve — all from the step 3.2 open+read cutover). The overlay-map logic the index
...
**Dependencies.** Step 3.2 (the open+read cutover — slot-1/open + the read serve,
the full replacement) + step 3.4 (the table finalized). The seam is removed only
after its full replacement is live. This is the last real-slot step's natural
successor.
