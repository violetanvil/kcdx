...
point: the CCryPak **construction site** (`CSystem_pCryPak_construct_store`, id
158, RVA `0x9B3C0C` — hook the store/just-after so kcdx owns the object the
instant the engine publishes the pointer into gEnv+0x50), NOT the late
`ModManager_ctor` ready-bracket. P1 (step 1.3, RESOLVED — static binary read,
outcome (c)) found the engine's first `*(gEnv+0x50)` file call (`0x1807A723A`)
PRECEDES the ready-bracket (`0x1807A76FE`), so the ready-bracket would miss it.
This proves the two load-bearing seating mechanisms on a tiny, fully
...
(write the kcdx vtable pointer into `[pCryPak+0x00]` at the seating point — the
CCryPak construction site `CSystem_pCryPak_construct_store`, id 158, per P1's
outcome (c)); and the P2/P4 probe instrumentation. This is the spike: it changes no resolution behavior
...
swap + the RESOLVED P1 seating point — the construction site, id 158), §4.3 (the
per-slot declarative table — the reversibility property is load-bearing: the table
is the single point of slot ownership), §8 P1 (RESOLVED, the seating anchor) + §8
P2 + P4 (the probe outcome maps). The executor builds to those sections, not this
summary (`.claude/rules/spec-conformance.md`).
...
swap is wired) — SATISFIED: P1 resolved (static binary read, outcome (c)); the
seating point is the construction site (id 158). Per
`.claude/rules/incremental-delivery.md` the probe (1.3) landed before the step
that rests on its outcome (1.4).
