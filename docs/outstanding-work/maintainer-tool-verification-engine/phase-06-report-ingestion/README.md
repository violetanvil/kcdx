...
| [6.3 [FE] The two batch actions — send rows → `/save/reverify-batch` → s06 batch confirm → `/confirm/batch`](step-3-fe-bulk-reverify-batch-confirm.md) | DONE | FE:d8771ff — the s08 two batch actions (verify-all + close-intervals), each its own s06 batch confirm, each one all-or-nothing transaction. Per D39 the FE computes NO edit-specs (`bulkReverify.ts` relays the resolver's `edits` by reference; the displayed deltas come from the `/save/reverify-batch` preview verbatim); on confirm relays the SAME edits to `/confirm/batch`. In-place applied-marking (law 3), rollback on a `failed` body (D21), disabled gates (law 7). Gate: `tsc -b && vite build` exit 0; vitest 34 files / 478 passing; step-review commit-step. Milestone UI acceptance pending (s08 new screen). |
