...
| 2.6d — the cross-row derivation DAG (user-approved 2026-06-07): for an instruction_anchor/data_slot row, resolve its dependency row (via `derives_from`), run THAT row's check against the same matched DLL, and pass its verdict as `opts.dependencyResult` so a dependent is correctly marked transitively CannotCheck when its anchor did not survive (the checker already supports it; only the editor-side dependency-row lookup is built here) | FE (`npm run build` + Vitest) | NOT STARTED | — |

The parent **2.6 row** in [`README.md`](README.md) flips to DONE when all four sub-steps land and
