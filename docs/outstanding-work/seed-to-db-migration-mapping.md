...
**Context.** The reference DB becomes the authoring source-of-truth for verified function facts; `kEntries[]` / `seed.csv` become a maintainer-generated cache projected from the DB's verified overlay. Bootstrap direction is `seed.csv → DB → generated seed.csv/kEntries[]`. The 139 current `seed.csv` rows are the only verified source; they seed the overlay. The generator (my side) then round-trips overlay → `seed.csv`/`kEntries[]` byte-equivalent.
