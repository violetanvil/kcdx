...
kcdx force-loads WHGame.dll from its own DllMain and builds the ONE Lua VM itself via
the FIX A symbol shim; the engine's `CScriptSystem::Init` VM-creation is intercepted
so it ADOPTS kcdx's state and never creates its own. One compiled Lua body, one
sentinel set — the dual-Lua sentinel hazard dies by construction. Delivers the
...
| First step | a keystone PROBE, not a build — observe `CScriptSystem::Init` + `lua_newstate` under force-load before wiring the interception | user 2026-06-05 · design §4 |
...
- **Loader-lock budget** — kcdx.dll DllMain (registration + force-load + VM build + before_game apply) targets <200ms, hard cap <500ms; over budget = cut work, never ship slow.
