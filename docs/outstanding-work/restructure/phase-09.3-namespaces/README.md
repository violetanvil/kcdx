...
| [6 — multi-region trampoline-pool expansion](step-6-multi-region-trampoline.md) | DONE | (landed) — scope corrected (the pool was ALREADY multi-region; the real delta is proactive per-reservation 80% expansion + an N-region-per-anchor cap + a teaching exhaustion error). Built into `Allocate()`; rel32-reach reuse preserved exactly; engine self-test cap-93 (expansion + exhaustion rows). Gated step-review PROCEED |
