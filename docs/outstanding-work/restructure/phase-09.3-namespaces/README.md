...
| [7b — `kcdxStatementInterface` (C++ static-bytes mirror)](step-7b-cpp-statement.md) | DONE | 60c178c — new interface (QI 14, append-only): `ReplaceWith` (static op) + `InsertBefore`/`InsertAfter` (register-and-defer, apply path unwired on both surfaces) + handle queries; `kcdxOp` (11 kinds) / `kcdxLocator` (12 kinds) value structs mirror the Lua constructor sets 1:1; `kcdxStatementOptions` with `targetRef`. Single-sourced seams: `QueueStatement` (one queue path, Lua-transparent extraction) + `BuildOpView` (one op-classification table). cap-96 (4 rows). docs/cpp statement NYI→Built + op/locator stubs corrected to the value-struct reality. Gated step-review PROCEED (re-review after the clobbered cap-96 README rows were restored) |
| [7c — `kcdxHookInterface` insert methods + `targetRef` opts field](step-7c-cpp-hook-insert.md) | DONE | fceb276 — `InsertBefore`/`InsertAfter` appended after the v≥2 marker (same 3-param shape as the existing sub-verbs; the required `statementLocator` + the `targetRef` ref-as-target arrive via two fields appended to `kcdxHookOptions`); `kcdxHookInterface_Version` 2→3 (a v2 requester is still served — prefix offsets unchanged); the C++ inserts set the SAME `insertPending` flag the Lua insert path sets, so the SAME apply-handler defer fires (no parallel mechanism); one `targetRef` precedence gate at the installer funnels covers all 8 verbs; `ConvertLocator` promoted + `CollapseTargetRef` extracted as shared seams (statement behavior unchanged). cap-97 (3 rows: insert-defers / insert-requires-locator / targetref-hook). docs/cpp hook.md insert entries + targetRef docs. Gated step-review PROCEED (one MINOR plan-ref token fixed) |
