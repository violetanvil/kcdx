...
| 8.5 — Asset replacement (pak overlay) | **SUPERSEDED 2026-06-04** — re-planned + spun out to the standalone `asset-system/` tree (`docs/outstanding-work/asset-system/`). This stub's 5-step decomposition WILL NOT BE BUILT as written; the live work is asset-system Phases 1–2 DONE (the two-hook seam + the full Lua+C++ `kcdx.assets.*` surface), Phase 3 regression + the Phase-11 in-game serve remaining. |
...
### Phase 8.5 — Asset replacement (kcdx absorbs pak mods) — **SUPERSEDED 2026-06-04 → `asset-system/`**

> **SUPERSEDED.** This phase was re-planned and spun out into the standalone
> **`docs/outstanding-work/asset-system/`** tree (a richer 3-phase decomposition
> than the 8.5a–8.5e stub below). Live status there: **Phases 1–2 DONE** — the
> two-coordinated-hook seam + the full Lua+C++ `kcdx.assets.*` /
> `kcdxAssetInterface` author surface; **Phase 3** (regression) + the **Phase-11**
> in-game serve (the boot-cache KI-0005 gap) remain. The `2026-05-28 audit`
> snapshot below is the historical record of where the work stood at the spinout;
> the `asset-system/` ledger is authoritative.

**Status (2026-05-28 audit — historical, pre-spinout):** 8.5a is DONE — the production asset-overlay hook is installed on `CCryPak::FOpen` via `hook_chain::AddCEngine` (`src/asset_overlay.cpp`, pass-through body), resolving by name `CCryPak_FOpen` (kcdx_id 131); the diagnostic FOPEN probe was archived to `_research/probe-archive/fopen-override.md` and removed. 8.5b/c/d/e are NOT BUILT — no `[entrypoints].assets` parse, no overlay map, no `kcdx.assets.*` Lua surface (grep `src/lua_bind*.cpp` for `assets` returns nothing), no `kcdxAssetInterface`, no `cap-XX-asset-replace` plugin. The pak-mod ABSORB path (mod-loader takeover landing pak mods into MOUNT verbatim) is a SEPARATE feature already complete — that is NOT this phase. This phase is OVERLAY: a kcdx plugin shipping a single loose file that OVERRIDES a pak-resident asset by virtual path.

The "kcdx replaces pak mods" section above is this phase's scope. Sub-phases:

- **Phase 8.5a**: hook the game's pak resolver. **DONE** — the production asset-overlay hook is installed on `CCryPak::FOpen` via `hook_chain::AddCEngine` (`src/asset_overlay.cpp`, pass-through body this step), resolved by name `CCryPak_FOpen` (kcdx_id 131, verified ABI). The diagnostic FOPEN probe was archived to `_research/probe-archive/fopen-override.md` and removed from `src/`.
...
**Empowered C++ wrapper for `kcdx.bytes` — SHIPPED (verb landed as `Write`/`TryWrite`, not `Replace`).** The raw `kcdxBytesInterface::Register(&opts)` is the always-available floor (Phase 3 sub-2, DONE); the empowered helper layers on top in `include/kcdx/Kcdx.h`, peer to the existing `kcdx::hook::*` helpers. It landed during the 9.3/9.5 wrapper work as:

- `kcdx::bytes::Write(K, "name", replacement, opts=nullptr)` (Floor 1 — fire-and-auto-log) and `kcdx::bytes::TryWrite(K, "name", replacement, opts=nullptr)` (Floor 2 — returns the handle for programmatic branching). Required args (target name + replacement bytes) positional; optional knobs threaded via a `kcdxBytesOptions* userOpts` pointer (name / description / original / module / offset / idempotent / context / anchorString / `[advanced]` pattern / addressId / targetSymbol). Auto-threads `owningPlugin = K.self`. Wraps `K.bytes->Register(&opts)`. Header-only.

> **Verb rename — settled 2026-06-12.** The original spec named the verb `kcdx::bytes::Replace(...)` with a designated-initializer trailing-struct opts. The as-built surface is `Write`/`TryWrite` with a `kcdxBytesOptions*` opts pointer — the **same 3-floor naming + opts idiom the shipped `kcdx::hook::*` wrapper uses** (Floor 1 fire-and-log + Floor 2 `Try*`), so a C++ author sees ONE consistent wrapper convention across hook + bytes (consistency = predictability, cornerstone #1). User-accepted as the settled shape; the `Replace`/designated-init spec is superseded.

`docs/cpp/bytes.md`'s common-path lead leads with the empowered `kcdx::bytes::Write` form; the raw `K.bytes->Register(&opts)` form is the labeled raw-floor drop-down per the 3-floor model (`docs/cpp/wrapper.md`) — DONE. The test row shipped as `test-plugins/cap-63-cpp-bytes-wrapper/` (the wrapper floor; cap-39 covers the raw floor — both surfaces under permanent regression). Direction was user-locked in the 2026-05-28 `/senior-architect-reply` thread for the hook docs flip; this is the bytes-side mirror.
