...
| P5 Cycle 1 | Extract ALL probe code from `src/` → `_research/probe-archive/` (9 inline `#if 0` strips + 2 whole-file dead probes removed + bugsplat PROBE Z block + cap-43 struck); KEEP fopen + bugsplat live infra; sweep stale refs | /execute | DONE-pending-commit (build GREEN; step-review PROCEED after 2 HALT-and-fix rounds — orphaned cap-43 + dangling/wrong fopen comments caught; PROBE Q intact; zero-loss archive) |
| P5 Cycle 2 (deferred → /execute) | `deprecated-toml-token-cleanup` 70-fix deletion sweep (6 open questions); `docs/outstanding-work/hardcoded-address-audit.md` deletion-hygiene survivors (describes the now-removed probe files as live) | /execute | DEFERRED |
