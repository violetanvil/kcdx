# Phase 1 — The exporter-over-bulk + the tracked CSV genesis + the widened oracle

**Intent:** build the D38 export/genesis machinery — extend the exporter to capture the bulk half
losslessly to `data/db-export-bulk/` (under Git LFS), repoint `run_rebuild` to rebuild both DBs from
the tracked CSVs (the dump → expert-only), and widen the round-trip oracle over the bulk so the
completeness bar (rebuild-from-CSV → DB → re-export → byte-identical, both halves) is a standing
green gate. This is the core of the migration; Phase 2 then moves the backend + toolchain seed-path
constants onto it. Gated by the data-core pytest (`data/refdata-extractor/tests/`).

**Design authority:** `data/maintainer-tool/design.md` **D38** + the revised §body dataflow. Build
to D38's curated/bulk seam (`kcdx_id IS NOT NULL`) + the LFS-for-bulk + the round-trip bar.

**Precondition:** Phase 0 (the probe) PASSED — the bulk round-trips CSV-losslessly. If P0 surfaced a
divergence, this phase does not start until the user reconciles the D38 model.

## Step-grain ledger

| Step | Status | Commit |
|---|---|---|
| [1.1 [CORE] Extend the exporter to capture the bulk half losslessly → data/db-export-bulk/](step-1-exporter-bulk.md) | DONE | 6af2d80 — new seeds_shared/bulk_exporter.py (export_bulk): the 3 DEV-only tables + kcdx_id-NULL av rows → data/db-export-bulk/, lossless (probe-verbatim encoder). Gate: pytest 3 passed + the real 1.3GB DEV DB exports with every CSV-record count == DB. Step-review PROCEED. **1.1-fix in 40d58b6** (landed with 1.3): the curated av rows' 8 dump-derived promote columns fell through both CSV halves → added the address_versions_derived.csv overlay (the authored-vs-derived split, Option A); explicit three-set partition + falsifiable coverage assert. |
| [1.2 [CORE] Git LFS tracking for data/db-export-bulk/ (.gitattributes + LFS init)](step-2-lfs-tracking.md) | DONE | 99a1aab — .gitattributes: data/db-export-bulk/*.csv → filter=lfs (curated data/db-export/ stays plain text). LFS already installed; gate: git check-attr confirms bulk→lfs, curated→unspecified. |
| [1.3 [CORE] Repoint run_rebuild genesis to the tracked CSVs; the dump → expert-only](step-3-rebuild-genesis.md) | DONE | 40d58b6 — run_rebuild_from_csv rebuilds BOTH DBs from the tracked CSVs (curated data/db-export/ + bulk data/db-export-bulk/) with NO dump; _merge_derived restores the curated rows' derived facts from the overlay by kcdx_id + restores the promoted av id; from-dump path retained expert-only (--rebuild-from-dump). Gate: 8/8 — full curated av round-trip byte-identical incl. merged promote columns + restored av_id + cascade. Step-review PROCEED (coverage-assert CONCERN fixed in-change). |
| [1.4 [CORE] Widen the round-trip oracle over the bulk (the completeness bar)](step-4-roundtrip-oracle.md) | DONE | 4adf95a — hash_real_tables drops the curated-only exclusion + content-hashes the full corpus (curated + bulk statements/referenced_vars/call_edges + kcdx_id-NULL av), excluding only non-semantic _dict_*/sqlite_sequence/USER-autoinc-id; round_trip_full rebuilds-from-CSV (run_rebuild_from_csv, NO dump) → re-export → byte-identical both halves. Falsifiable (injected bulk length divergence → RoundTripError, AP15). Mini-dump fixture, not 1.3GB. Gate: 4/4. Step-review PROCEED. |

## Phase verification gate

Phase 1 is done when: the exporter writes complete, lossless bulk CSVs to `data/db-export-bulk/`
(1.1); those CSVs are Git-LFS-tracked (1.2); `run_rebuild` rebuilds BOTH DBs from
`data/db-export/` + `data/db-export-bulk/` with no dump (1.3); and the widened round-trip oracle is
green over the full corpus — rebuild-from-CSV → DB → re-export → byte-identical for both the curated
and bulk halves (1.4). All gated by the data-core pytest. Build-green is necessary, not sufficient —
the round-trip oracle (1.4) is the real completeness proof (`.claude/rules/skeptical-expert.md`).
After 1.4 is green, the dump is retired as a routine input (the soundness carry-forward's gate is
satisfied).
