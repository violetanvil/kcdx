...
| [Phase 2 — The backend + toolchain migration off data/seeds](phase-02-backend-toolchain/README.md) | DONE | All 3 steps landed: 2.1 (076834e, import_to_sqlite SEED_DIR) + 2.2 (7f26259, backend config/csv_integrity/routes) + 2.3 (d19ad50, the 9 data-core test fixtures). The toolchain + backend + tests run off data/db-export, no in-scope data/seeds reference remains. Full data-core suite GREEN (177 passed) + backend suite GREEN (69 passed). The paused verification-engine 6.2 was unblocked + committed (42ebd79 + FE:17bfa12) as part of clearing the tree. Phase 3 (delete data/seeds - deprecated) proceeds. |
| [Phase 3 — Close the deprecation (delete data/seeds - deprecated)](phase-03-close-deprecation/README.md) | DONE | 735c30e — the 3 data/seeds CSVs + the untracked deprecated copy deleted; policy.md kept for the deferred governance sweep. Full data-core + backend suite GREEN post-deletion (246 passed). The migration's DATA layer is fully off data/seeds. |
