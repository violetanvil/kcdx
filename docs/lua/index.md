...
- **consumer (of a behavior)** — a plugin that reads and sets a behavior it
  does not declare — the simple-modder role: a consumer names a behavior and
  a value, never a function name or an address.
...
- **apply boundary** — the single point, after all plugins have loaded, where
  ...
  settled value (never-set behaviors are skipped — they read as their
  declared `default`). See [behavior.md](behavior.md) §"The apply boundary".
...
| `kcdx.behavior.declare` / `.set` / `.get` / `.list` | named behaviors — declare a behavior your plugin owns (stamped `<author>.<plugin>.<bare>`; spec = description + default + implementation + optional revert); set a behavior's value at load (recorded last-wins, applied once at the apply boundary) OR toggle it post-load when the declarer shipped a `revert` (revert-then-implementation); read a behavior's current-or-default value; browse the registered behaviors with an optional prefix filter. | [behavior.md](behavior.md) |
