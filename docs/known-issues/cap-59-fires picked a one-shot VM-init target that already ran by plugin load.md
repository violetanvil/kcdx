...
- **Diagnostic archive:** PROBE α archived in-place in `src/hooks.cpp` at the `HookedUpdate` latch + at `HookedLuaPcall_Engine` via `#if 0` + 4-line header per CLAUDE.md hard rule. PROBE Z archived in `src/probes/bugsplat_ctor_probe.cpp` (verdict: codegen + branch_pool VirtualAlloc + dtor all loader-lock-safe; bugsplat site is migratable when its site lands).
