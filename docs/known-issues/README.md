...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0007](KI-0007-update-version-unique-constraint-500.md) | 2026-06-07 | maintainer-tool `/confirm/update-version` 500s on every version-row edit: `sqlite3.IntegrityError: UNIQUE constraint failed: address_versions.kcdx_id` at `_projected_update` (the full-column UPDATE). The real constraint is the PARTIAL unique index `(kcdx_id) WHERE valid_through IS NULL` — two open-interval rows collide. Pre-existing step-3c path (`f0dbf10`), NOT a 2.6 regression — OPEN |
