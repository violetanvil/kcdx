...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0028](KI-0028-fs-takeover-boot-no-render.md) | 2026-07-03 | Boot renders nothing (black screen, no 2D UI, no 3D scene) when the kcdx filesystem takeover is active (swap ON); boots to menu normally with the swap OFF (`kcdx-noswap` marker). The FS swap is the perturbation. Investigation RESET 2026-07-03 to a clean frame — the plan is to find the FIRST swap-ON-vs-OFF behavioral divergence from the swap point forward, via the neutral `FS_BOOT_TRACE` logging; no prior theory (the accumulated KI-0028/26/27 FS research + probes were boxed outside the repo). OPEN |
