...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0014](KI-0014-pdb-autoload-zero-functions-in-load-loop.md) | 2026-06-09 | Phase 9.3 step 3b PDB auto-load reports cap-90's `/DEBUG:FULL` PDB as zero-in-range-functions at the live launch (the FASTLINK-fallback WARN fired on a provably-FULL PDB; `cap-90-pdb-internal-address` red, `resolve found=false`). The engine integration otherwise works (the fallback fired correctly for ~36 FASTLINK plugins); a prior probe proved the same DbgHelp sequence enumerates a FULL PDB's internal. Key diff: the probe ran one-shot (cap-89 only); the feature runs in the real load loop after ~36 plugins. Probe A (raw-enumerate instrumentation) owed — OPEN |
