...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0019](KI-0019-ccrypak-fopen-reentrancy-av-on-inventory-open.md) | 2026-06-13 | ACCESS_VIOLATION on inventory open. Dump-read root cause (NOT re-entrancy — that was a text-log misread): null write of 0x0D=EACCES from `ucrtbase!get_osfhandle`→`invalid_parameter`, under `fseek` on a cross-CRT `FILE*` minted by asset HOOK 2 (`_wfopen_s`, kcdx's CRT) and fseek'd by WHGame's CRT during FSR2/DLSS init. The `fseek` SIBLING of KI-0006's cross-CRT `fclose` — same HOOK 2 hazard. The 3 Phase-10 probes are a RED HERRING (installed zero hooks; crash is non-deterministic). BUNDLED → KI-0006 / Phase 11 (FIX A collapses the dual-runtime), user-decided 2026-06-13 — OPEN (Phase-11-gated) |
