...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0018](KI-0018-cpp-declared-behavior-impl-not-fired-and-asbool-miscoerce.md) | 2026-06-12 | A C++-declared behavior's implementation never fires at the apply boundary, and the handle `AsBool` mis-coerces a recorded bool — a Lua-set value RECORDS on the C++-declared behavior (`Get` returns it — the shared registry works) but `impl ran=0` and `AsBool=0` while `Get=1`. Found in the P2 s1 launch (cap-102); a C++ Set of a *Lua*-declared behavior DOES fire (so the boundary path works — the break is specific to a C++-declared behavior's impl storage). 9/12 rows + all interface infra (wave-end gate, off-thread wall, early-stop wall) GREEN — OPEN |
