...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0028](KI-0028-fs-takeover-boot-hang-ui-render-init.md) | 2026-06-20 | Boot HANGS at UI/render bring-up (sound loads, no video, no input — killed via Task Manager, no crash dump) AFTER KI-0027's table-DB load succeeds. Dev log's last line = reading `cursor_green.dds` from Engine.pak; suite ran to `passing=320/343`, table-DB error gone, enumeration correct. A NEW downstream failure the KI-0027 fix unblocked the boot far enough to reach (the KI-0026→KI-0027→KI-0028 chain). FS-takeover enumeration is functioning; the render/UI frontend bring-up is wedged. `/debug KI-0028` owed — get ground truth on which thread is blocked + whether any kcdx slot is on its stack — OPEN |
| [KI-0027](closed/KI-0027-fs-takeover-table-glob-enumeration-unserved.md) | 2026-06-20 | Table-DB load failed ("Database system error - tables can't be loaded", `err_id=259`): kcdx's fs-takeover served single-file reads but NOT wildcard-glob directory enumeration. The table system globs `Libs/Tables/**/<table>__*.xml` to discover override patches; the glob dispatches through the CCryPak `FindFirst`/`FindNext`/`FindClose` triplet (slots 63/64/65) which thunked to the engine original, so kcdx served zero pak-resident entries. FIXED (fs-takeover Phase 5): kcdx owns the triplet over the unified set + the index arm honors the filename glob mask (`4befc07`). Live-verified — zero `err_id=259`, correct match counts, 320 tests pass. Boot now reaches UI/render init (KI-0028) — Closed 2026-06-20 |
| [KI-0026](closed/KI-0026-fs-takeover-metadata-slots-graphics-init-fatal.md) | 2026-06-16 | 0xC8 CryFatalError (code 200) in the engine's NGX/FSR2 graphics-init (`C_Game::CreateInstance`) on the first boot with the fs-takeover step-3.3 metadata/enum slots live — KI-0012 mod-mount-list signature, no kcdx frame in the 14-frame stack. PROBE M EXONERATED (5 mount stubs installed, 0 fired pre-crash). Last kcdx activity: ctor_bracket_complete (enabled_n=15) → slot-36 FOpen live → `loose_open_failed` on `engine/config/engine_core.thread_config` (errno=2) → fatal ~430ms later. Suspected: a kcdx metadata/open slot mis-serves an engine file graphics-init needs (the §6/Decision-C pak-long-tail gap) — UNPROVEN, `/debug` owed. Dump + bundle captured — OPEN |
| [KI-0025](closed/KI-0025-refdb-dangling-survival-derives-from-kcdx-id-12.md) | 2026-06-15 | reference-DB integrity: a new-entity `/confirm` validated clean at preview but FAILED the whole-DB integrity gate + rolled back (`survival_derives_from kcdx_id=12 has no curated address_versions row`), blocking ALL new-entity adds. TWO coupled defects (the stale-integer-version-id filing guess was FALSIFIED, PROBE A). (1) DATA: a batch UPDATE (`f15ae4f`) closed kcdx_id=12's SOLE interval (a LIVE gEnv anchor) → zero open rows → kcdx_id=9's `survival_derives_from=12` edge failed the open-row integrity lookup. (2) CODE (the real blocker, PROBE I): `_present_row_non_trio_differs` EAGERLY resolved an UNCHANGED dependent's derives-from edge through `_resolve_derives_from_av_id` (which demands `valid_through IS NULL`) just to COMPARE no-op cells — so the reopen couldn't self-heal (chicken-and-egg). Fix A (`79ea49a`): compare the edge in kcdx_id space (PK lookup, no open-row predicate) so a no-op comparison can't raise; + `check_live_entity_has_open_interval` validator. Repaired kcdx_id=12 (`8ec66e3`); ACCEPTANCE: the blocked new-entity add now SUCCEEDS — CCryPak_FOpenRaw kcdx_id=160 (`5527f2b`). Gate A (general-purpose substitute) Fix-A-over-masking; Gate B `land-fix`. Interval 6/6 + apply 22/22 green (2 pre-existing reds unrelated). Closed 2026-06-15 |
| [KI-0024](closed/KI-0024-maintainer-fe-audit-trio-orphan-on-direct-trio-edit.md) | 2026-06-15 | maintainer-tool FE: editing an audit-trio field (verified_by / verified_date / evidence_kind) on a row with a mismatched last_verified_at_version orphans the trio → the validator rejects (all-or-null + non-empty version). User-hit during Step 2 acceptance, "on random fields all the time." Root cause: FieldEditor's trio coupling fired in ONE direction only (`applyAuditTrioCoupling` runs only on the version-field edit) — every trio-cell-initiated path (a direct write, the D29 auto-suggest, EMPTYING a verified cell) bypassed it, and the error surface compounded it (raw validator string rendered verbatim + attributed by substring match to the populated fields, not the empty deficient one). Fixed both directions + the error surface: version-empty → disable trio inputs + gate D29 (FE 8557c11); version-set → setField rejects blanking a trio cell (re-type allowed; un-verify by clearing the version) + teaching error attached to the deficient field, raw string suppressed (FE 158dd3f). Single-sourced from AUDIT_TRIO_SEEDS; validity stays the API's (law 6). Gate A (1b + 2a) + Gate B `land-fix`; vitest 604/604; user-confirmed repro. Follow-ups surfaced: the general teaching-error layer (deferred) + the s05 CreateVersionForm having the same blank-trio shape. Closed 2026-06-15 |
| [KI-0023](closed/KI-0023-batch-close-intervals-reimport-shows-broken-again.md) | 2026-06-15 | maintainer-tool FE: a confirmed close-intervals batch showed the acted row broken again on re-import. Root cause (static-traced): the s08 already-acted reconciliation only ran on a batch-action `/save/reverify-batch` round-trip; a plain re-import reset `alreadyActed` empty and never re-classified, so `reconciledBlocks` had nothing to move out and an already-closed `failed` row re-bucketed into `failing` from the report verdict alone. A design gap (D41 scoped reconciliation to the batch preview), not a code bug. Fixed by on-import reconciliation: `handleFile` fires the existing preview per actionable block before first paint (FE reads the classification, never re-derives — D41 fact 2). FE:46d3c09 + 3 falsifiable tests (vitest 588/588); capture D46 (`592ea9d`); Gate A architect-review + Gate B root-cause-verifier `land-fix`; user-confirmed repro (15 rows under "No further action" on a plain re-import). Closed 2026-06-15 |
| [KI-0022](closed/KI-0022-maintainer-fe-fix-arrival-divergence-not-rendering.md) | 2026-06-14 | maintainer-tool FE: the D45 per-field divergence diff did not render on a real `[Fix ▸]` arrival (build-green-not-sufficient runtime wiring defect). Root cause (probe-pinned): `fixDivergence`/`fixGameVersion` were set by the `[deepLink]` applier but nulled by a separate `[kcdxId]` load effect — under StrictMode the reset's 2nd fire landed after the set, clobbering it (`verifyOpen` survived, no reset writer). Fixed by a single-owner divergence effect (FE:7d37417) + 2 falsifiable regression tests (StrictMode arrival + entity-switch stale-clear); Gate B `land-fix`; user-confirmed. Closed 2026-06-14 |
| [KI-0021](closed/KI-0021-maintainer-fe-new-entity-verified-row-unsavable.md) | 2026-06-14 | maintainer-tool FE: creating a NEW ENTITY with a verified first row was impossible to save — the audit-trio all-or-null validator demands verified_date, but CreateEntityForm rendered it read-only/system-set (🔒) and its single-cell setField never auto-filled it (CreateVersionForm prefills+defaults it; the new-entity form was missed), so the trio was structurally stuck incomplete and every save was rejected. Root cause (Gate-B-verified mechanism): setField wrote only the edited cell → setting last_verified_at_version left verified_date empty → validator demanded a value the read-only UI gave no way to supply. Fixed by wiring the canonical applyAuditTrioCoupling into setField+revertField (SET auto-fills today/maintainer_ghidra for empty trio cells, CLEAR clears all three — trio complete-by-construction). FE:9ff0fb5 + a falsifiable CreateEntityOverlay regression test (vitest 7/7, verified live; revert→test goes red). Gate B `land-fix`; the s04 sibling trio path user-confirmed during the Step 2 acceptance same day. The fix had landed on its own commit during the audit-trio pass; the doc sat stale-open until the completion-sequence reached it. Closed 2026-06-15 |
| [KI-0020](KI-0020-maintainer-fe-app-test-s09-timing-flake.md) | 2026-06-14 | maintainer-tool FE `src/App.test.tsx` s09 step-3.3 tests are non-deterministically flaky under parallel-worker load (0–2 red/run, varies). Probed: a real-timer timing race, NOT a logic defect — the tests pass 13/13 in isolation and the suite passes 572/572 on a clean run; only the 41-file parallel load surfaces it. Belongs to `maintainer-tool-lifecycle-completeness` step 3.3 (which authored these s09 tests), surfaced while gating the divergence-diff worker. **2026-06-15 (sequence Step 4): NON-REPRODUCIBLE** across 13 full-suite runs incl. forced contention (24 over-subscribed workers + 12 CPU spinners) — all green 632/632, zero `act()` warnings; probably already resolved by `7d37417` (KI-0022's single-owner StrictMode-stable s09 effect collapsed the same double-fire race) but UNVERIFIED (a flake that won't repro can't be proven fixed). Kept OPEN per user decision with a watch trigger (reopen when an s09 `App.test` next reds); no harden lands until reproducible to verify against — OPEN |
| [KI-0019](KI-0019-ccrypak-fopen-reentrancy-av-on-inventory-open.md) | 2026-06-13 | ACCESS_VIOLATION on inventory open — NOT FIXED, crash still reproduces. LEADING DIAGNOSIS (dump + source, NOT re-entrancy — that was a text-log misread): null write of 0x0D=EACCES from `ucrtbase!get_osfhandle`→`invalid_parameter`, under `fseek` on a cross-CRT `FILE*` minted by asset HOOK 2 (`_wfopen_s`, kcdx's CRT) and fseek'd by WHGame's CRT during FSR2/DLSS init. Residual UNCONFIRMED: HOOK 2 hit-vs-miss on the FSR2 file. The `fseek` sibling of KI-0006's cross-CRT `fclose`. The 3 Phase-10 probes are a RED HERRING (installed zero hooks; crash non-deterministic). Fix = the file-system takeover (`docs/design/file-system-takeover.md` §9; its own track) — kcdx owns every handle on its own CRT, eliminating the cross-CRT class structurally; closes with KI-0006 at the takeover plan's step 4.2 — OPEN |
| [KI-0018](closed/KI-0018-cpp-declared-behavior-impl-not-fired-and-asbool-miscoerce.md) | 2026-06-12 | cap-102's three C++ rows read an apply-boundary effect (the impl-fire flag, the applied AsBool value) at kcdxPlugin_PostGameLoad — one stop BEFORE RunApplyBoundary fires the impl (boundary runs post-PostGameLoad / pre-InputLoaded). NOT an engine defect: PROBE B/C proved the C-impl trampoline fires correctly with the right value. Fix = moved the boundary-dependent assertions to the InputLoaded handler (post-boundary), no src/ change. All 13 cap-102 rows GREEN. Fixed ff8d4d5 — Closed 2026-06-12 |
| [KI-0017](closed/KI-0017-boundary-time-statement-original-capture-empty.md) | 2026-06-11 | `kcdx.statement.replace_with` apply rejected `(0 vs N)` — the carrier `PatchEntry` never set `original`, so `patch::Resolve`'s length precondition rejected EVERY statement write; no statement had ever applied (the resolution layer exposes no instruction-bytes field, so the empty `original` was inevitable). Fixed `402d028` (fill `original` from the live resolved-VA read; AP14-clear — the guard is replacement-vs-site). Fixtures re-framed to assert the LIVE apply (`3421de2`); user-verified `wrote_bytes=3==byte_range_len` from inside the boundary. Residual fork-3 (statement carriers absent from the conflict engine — loud-but-unnamed clobber) → TD. — FIXED. Closed 2026-06-11 |
...
| [KI-0015](closed/KI-0015-find-eager-lua-materialization-hang.md) | 2026-06-10 | Phase 9.4 acceptance: boot HANG at `cap-99-truncates-loud`. Root-caused (AP17): `kcdx.find` eagerly materialized EVERY statement of EVERY one of the 500 capped records into nested Lua tables — a broad query (callers of `_Init_thread_footer`, 30,393 matches) builds ~131,691 statement + 266,737 capture sub-tables ≈ 400K nested Lua tables on the boot worker thread → memory/GC stall → watchdog timeout (no AV/minidump). The pure SQL runs ~1.5s; the stall is wholly the SQL→Lua eager materialization. Fixed `40236a9`: `find` returns lean function headers + a SQL-computed `statement_count`; statement bodies stay in SQL until `kcdx_dev_inspect` asks for ONE function. Re-verified 2026-06-10 (cap-99-truncates-loud PASS, lean shape). Closed 2026-06-10 |
...
- [closed/KI-0015-find-eager-lua-materialization-hang.md](closed/KI-0015-find-eager-lua-materialization-hang.md)
  — Phase 9.4 acceptance: boot hung at `cap-99-truncates-loud`. `kcdx.find`
  eagerly materialized the full statement list of every one of the 500 capped
  records — a broad query (30,393 matches) built ~400K nested Lua tables on the
  boot worker thread under GC pressure → memory/GC stall → watchdog timeout (no
  AV). A SEARCH tool returning the full body of every hit was the defect. Fixed
  `40236a9`: `find` returns lean function headers + a SQL-computed
  `statement_count`; statement bodies stay in SQL until `kcdx_dev_inspect` asks
  for ONE function. Re-verified at the 2026-06-10 acceptance launch. (The later
  hang the lean fix EXPOSED — the un-indexed dev-DB scan — was KI-0016.) Closed
  2026-06-10.
