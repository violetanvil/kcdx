...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0019](KI-0019-ccrypak-fopen-reentrancy-av-on-inventory-open.md) | 2026-06-13 | ACCESS_VIOLATION on inventory open — runaway re-entrant `engine.ccrypak_fopen` recursion (seq from 568978), null-arg-reg deref through ucrtbase CRT-heap. Surfaced by the Phase-10 probe launch but the 3 probes installed ZERO hooks (kcdx.hook nil) → almost certainly NOT probe-caused; leading frame = asset-system FOpen detour re-entrancy gap (HOOK 2 own-FILE* path, reworked post-KI-0012). Same hook/signature as KI-0012 but distinct trigger (inventory vs boot) + KI-0012's mount-list gate held. Probe A (remove probes + repro) pending — OPEN |
