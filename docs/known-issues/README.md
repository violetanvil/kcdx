...
| [KI-0024](KI-0024-maintainer-fe-audit-trio-orphan-on-direct-trio-edit.md) | 2026-06-15 | maintainer-tool FE: setting one audit-trio field (verified_by / verified_date / evidence_kind) directly on a row with an empty last_verified_at_version orphans the trio → the validator rejects (all-or-null + non-empty version). User-hit during Step 2 acceptance, "on random fields all the time." Root cause (static-traced): FieldEditor `setField` couples the version→trio direction (FIX 1: editing last_verified_at_version auto-fills/clears the trio) but NOT the trio→version direction — a direct trio edit falls through to a plain patch with no coupling, so the orphan state the version-path coupling was written to prevent is reachable the other way. Edit-editor sibling of KI-0021 (same audit-trio-coupling family). Fix = a design fork (auto-fill version+trio / lock trio until version set / symmetric trio-only couple) the user decides via `/debug` Gate A — OPEN |
