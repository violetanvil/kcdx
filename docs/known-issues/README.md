...
| [KI-0021](KI-0021-maintainer-fe-new-entity-verified-row-unsavable.md) | 2026-06-14 | maintainer-tool FE: creating a NEW ENTITY with a verified first row is impossible to save — the audit-trio validator demands verified_date, but CreateEntityForm renders it read-only/system-set (🔒) and never auto-fills it (CreateVersionForm DOES default it to today; the new-entity form was missed). User-hit. Fix = port the verified_date-today auto-fill to CreateEntityForm + a regression test, in the FE repo (FE:<hash>). DEFERRED by user decision until the divergence-diff feature (Phase 3.2) closes — OPEN |
