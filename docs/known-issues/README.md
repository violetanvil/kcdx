...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0023](KI-0023-batch-close-intervals-reimport-shows-broken-again.md) | 2026-06-15 | maintainer-tool FE: a confirmed close-intervals batch shows the acted row broken again on re-import. Ground-truth-confirmed the WRITE works (kcdx_id 12 `valid_through_version` empty→1.5.1164953, committed f15ae4f — interval closed in DB); the defect is RE-IMPORT/reconciliation. The acted row carries `matched_address_version_id: None` — candidate discriminator (resolver can't attribute a no-matched-id row to the closed DB row). Mechanism unknown; two candidate paths to probe. Blocks verification-engine Phase 6 acceptance — OPEN |
