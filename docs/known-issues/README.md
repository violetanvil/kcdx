...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0008](KI-0008-update-version-open-row-silent-noop.md) | 2026-06-08 | maintainer-tool: editing an OPEN-interval (current) version row silently NO-OPS — the value never persists, `/confirm/update-version` returns 200, nothing lands (not even in the in-flight txn). A CLOSED-row edit writes correctly; the OPEN-row edit's value is dropped before the apply. The seed-edit layer is correct (prospective seed → new value); the defect is downstream in the action-derivation. Pre-existing step-3c path, distinct from KI-0007's crash — OPEN |
