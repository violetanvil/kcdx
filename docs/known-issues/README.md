...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0025](KI-0025-refdb-dangling-survival-derives-from-kcdx-id-12.md) | 2026-06-15 | reference-DB integrity: a new-entity `/confirm` validates clean at preview but FAILS the whole-DB integrity gate + rolls back — `survival_derives_from kcdx_id=12 has no curated address_versions row`. Surfaced adding CCryPak_FOpenRaw (kcdx_id=160, slot 35) for fs-takeover step 3.2; the FOpenRaw row is clean+unrelated — the gate is whole-DB so ANY new entity now trips this PRE-EXISTING break. Probed (not theorized): kcdx_id=12 DOES have an address_versions row in reference.sqlite (kind=5 string_anchor 'exec autoexec.cfg', valid_from=1); kcdx_id=9 (instruction_anchor) has survival_derives_from=12; the mismatch is export/integrity-view vs DB, likely a stale integer version-id (last_verified_at_version=1 vs the 1.5.1164953 tag). Blocks ALL new-entity adds until repaired. Fix = an UPDATE to existing rows (GUI's job, out of /add-db-entity scope) — a maintainer decision; do NOT hand-edit CSV or force the write (the gate is correct) — OPEN |
