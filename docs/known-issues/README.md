...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0017](KI-0017-boundary-time-statement-original-capture-empty.md) | 2026-06-11 | `comp-20-declarer-statement` red — a `kcdx.statement.replace_with` registered AT the behavior apply boundary (9.5 P1 s3) drains but is rejected at apply: `original/replacement length mismatch (0 vs 3)` — the original-byte capture is EMPTY. One-variable control: cap-92 (same statement shape, same SaveGame target, LOAD-time registration) passes 4/4 in the same run. Hypothesis (unverified): boundary-time entries miss the original-capture/resolution stage load-wave entries get. Gates the design §9 declarer-leg acceptance; P2 s2's queued-set path drives the same machinery — OPEN |
