...
| ID | Opened | Summary |
|----|--------|---------|
| [KI-0027](KI-0027-fs-takeover-table-glob-enumeration-unserved.md) | 2026-06-20 | Table-DB load fails ("Database system error - tables can't be loaded", `err_id=259`): kcdx's fs-takeover serves single-file reads but NOT wildcard-glob directory enumeration. The table system globs `Libs/Tables/**/<table>__*.xml` to discover override patches; 292 such `__*.xml` opens all miss-original + ZERO `ForEachFile` enum-slot fires (slots 15/101 deferred THUNK). Boot now reaches the game-data layer (the deferred `ForEachFile`/`FindFiles` takeover surface is its first hard consumer). Distinct subsystem from KI-0026 (enumeration vs single-file read; 259 ≠ 0xC8) — surfaced after KI-0026's three fixes unblocked the boot past graphics-init — OPEN |
