...
## Reframe 17 (2026-07-03) — PROBE Z9 FALSIFIES the condvar-stall thesis: the shader-ready flag FLIPS (wall_ms=7140), the wait is PER-FRAME (13,981 enters) and completes; the 2 "identical" samples were a per-frame sampling artifact

PROBE Z9 (`producer_ready_probe.cpp`, run `kcdx-dev_2026-07-03_10-16-32.log`, swap-ON black arm) hooked the wait-loop entry `0x9ace14`, captured the wait object, and a watcher polled the ready-flag `[wait_obj+0x50]` + the producer singleton `[0x492b8c8]`. The theory under test was Reframe 16's "the producer never sets the ready-flag → Main is stuck."

- **`READY_FLAG_SET wall_ms=7140`** — the ready-flag `[wait_obj+0x50]` FLIPPED to non-zero 7.1 s in. The shader system BECAME READY on the black arm.
- **`READY_PROBE_VERDICT wait_entered=1 flag_ever_set=1 producer_installed=1 wait_enters=13981`** — the wait loop was entered ~14k times over ~2 min (per-frame cadence), the producer was installed, and the flag flipped. Per Z9's pre-committed map (`entered + flag_set => shader-ready DOES fire; the stall is DOWNSTREAM`).

**Both Reframe 15 ("Main is STUCK in a condvar wait") and Reframe 16 ("producer never signals ready") are OVERTURNED.** `0x9ace14`/`0x1c1e7e0` is a PER-FRAME shader-coordination wait that COMPLETES every frame — not a one-shot boot gate that hangs. The two byte-identical invasive `~0 k` samples caught this recurring per-frame wait mid-block; two identical samples are NOT proof-of-stuck for a per-frame frame (they prove the frame recurs at the sampled depth). This is the exact per-frame sampling trap §13 warns about and PROBE M already killed for `0x869c39` — I re-fell into it by reading "2 identical invasive samples" as "stuck." The CShaderMan condvar wait is EXONERATED as the wedge.

**Corrected frontier (returns to Reframe 13's form — do NOT theory-hop):** what is genuinely still proven is unchanged — geometry buffers ARE created (Z8: geo_buf=262) but no index buffer is ever BOUND (ia_set_ib=0, draw_indexed=0) on a live, presenting (~310fps, frames black) game; the swap is the differentiator. What is FALSE now is "Main is stuck." The open question is again: on a presenting game with geometry created, WHY is no IB ever bound? The condvar axis is a dead end (per-frame, completes). Next observation must catch WHERE indexed-draw recording is abandoned — instrument the render-item/draw-submission leaf that should call `IASetIndexBuffer`, swap-ON vs swap-OFF (is the geometry culled/skipped before binding, or is the bind call never reached?), NOT another wedged-stack frame (they recur per-frame). Recon + the falsification detail: `_research/ki0028-mainthread-condvar-wait-recon/FINDINGS.md` §"PROBE Z9".

## Reframe 16 (2026-07-03) — STATIC: the wait is a CShaderMan `condition_variable::wait_for` for the shader system to become ready; kicks producer `[0x492b8c8]`, waits on ready-flag `[rbx+0x50]` that never flips

Static disassembly of the wait chain from Reframe 15's two byte-identical invasive samples (`_research/ki0028-mainthread-condvar-wait-recon/disasm_condvar_wait.py`, WHGame image base 0x180000000). Answers Reframe 15's frontier "what is Main waiting for?". All claims are body reads with cited sites.

- **The wait is a genuine `std::condition_variable::wait_for` (200 ms timed) — NOT a `std::call_once` guard.** The return-address `0x1c1e7e0` is the post-wait epilogue of the MSVC-STL condition_variable timed-wait primitive `0x1c1e78c` (proven by the STL runtime IAT thunk table above it: `_Mtx_lock`/`_Mtx_unlock`/`_Cnd_wait`/`_Xtime_get_ticks`/`_Thrd_id`, `0x1c1e743`–`0x1c1e785`). Body: `lea rdx,[rdi+0x10]` (SRWLOCK), `lea rcx,[r15+8]` (CONDITION_VARIABLE), `SleepConditionVariableSRW`, re-check `GetTickCount64-rsi` vs timeout. It is byte-distinct from the exonerated call-once guard `0x1c1e988` (condvar 0x50c5fa8/lock 0x50c5fb0). So "Main is genuinely stuck in a real cv wait whose predicate stays false" is BODY-PROVEN — not the PROBE-M per-frame-trap.
- **The predicate is `*(bool*)(rbx+0x50)`** — a ready/done flag. `0x1de92a0` = `condition_variable::wait(lock, pred)`: `movzx r8d,[rbx+0x50]; test r8b,r8b; je done` loops the timed wait until the flag flips.
- **The owner pump-loop `0x9ace14`:** polls the SHADER-MANAGER singleton `[0x547bb50]` (slot `+0x108`) for a work item, kicks producer `[0x492b8c8]` (vtable `+0x720`, dl=1) with it, waits 200 ms on `pred=[rbx+0x50]`, loops (unlock→re-poll→re-kick→re-wait). The ready-flag never flips → Main pumps forever, which is exactly why the two invasive samples are byte-identical.
- **`[0x547bb50]` = CShaderMan** (VERIFIED cross-ref: `ki0028-cshaderman-pso-consumer-recon` — it holds `"Shaders/Cache/D3D12/"` at `+0x1108`). The owner wait is reached from CShaderMan lifecycle code (`0x9acc9c`, called once from `0x9accd7`); the sibling fn `0x9acc5c` in the same cluster dereferences the RENDERER singleton `[0x492b908]` (Reframe 14's gate, `call [[0x492b908]]+0x860`). So this cluster is CShaderMan coordinating with both the renderer `[0x492b908]` and the producer `[0x492b8c8]` — three distinct gEnv-table interface pointers (`0x492b8c8`/`0x492b908` are 0x40-apart slots, each runtime-installed, no static writer).

**This is the concrete mechanism behind CORRECTION 3/4's "the shader-SYSTEM init stalls upstream of PSO creation" (`gfx_calls=1`, compile workers idle, engine stops at the shader cache-version check) and CORRECTION 6/§12.B's "boot-phase sequencer never advances."** Now named: Main (the sequencer) is blocked in a CShaderMan `condition_variable::wait_for` for shader-system readiness; producer `[0x492b8c8]` never sets the ready-flag swap-ON. `draw_indexed=0`/black are downstream (Main never advances past shader readiness to build + submit scene geometry).

**Trust split (honest-uncertainty):** the CONSUMER side (the wait, the predicate, the CShaderMan owner, the kick site) is VERIFIED by body reads. That `[0x492b8c8]` is specifically the shader-compile producer, and that the FS swap is what stops it signaling, remains **INFERRED** — the `+0x720` kick body and the `[rbx+0x50]` writer are not yet read. **The frontier narrows to the producer and becomes a RUNTIME `/debug` question** (static took it as far as it goes, results-driven §4): after-hook `[0x492b8c8]+0x720` (does the kick fire?) + watch the `[rbx+0x50]` writer, swap-ON vs swap-OFF, to see whether the kick enqueues a job a worker never runs, or the producer's completion path is gated on state the swap leaves unset. Full recon: `_research/ki0028-mainthread-condvar-wait-recon/FINDINGS.md`.
