...
| 2026-06-20 | PROBE P-read launch (`kcdx-dev_2026-06-20_12-30-29.log`, swap seated kcdx_owned=28) | **DECISIVE — the READ is correct; the SIZE/FD identity of the pak handle is the defect.** Outcome A for the read: `FReadRaw_byPakIndex got=20096`, served `head64="<!--\r\n= Platform names =..."` + `tail32="...</ThreadConfig>\r\n"` — EXACTLY the known XML, full length. kcdx serves the right bytes. BUT outcome B on the size/fd path: `Fileno handle=3 ret=-1` — and the read request itself is `req_size=1 req_count=4294967295` (`0xFFFFFFFF` = `(size_t)-1`). So the engine asked to read `-1` bytes (it happened to get the full 20096 because the inflated buffer holds the whole entry). The `-1` count + `Fileno=-1` point at the same root: the engine's thread-config loader queries the file's SIZE/FD to size+validate its parse buffer, kcdx's PAK-backed handle has NO real OS fd → `Fileno` returns -1 (and the size query the loader uses returns a bad value) → the engine can't get a valid length → `CSystem::FatalError` "Error loading thread config" (the sibling fatal string P-static-2 read was "Couldn't get length for Config file"). MECHANISM: kcdx's pak handle serves bytes correctly but does not answer the engine's fd/size-by-handle query the way a real file handle would. NEXT: read what the engine's loader does with Fileno/the size query (static), and what kcdx's pak handle returns for size-by-handle (FTell didn't fire — the loader used Fileno + a -1-sized read). Fix = kcdx's pak handle answers the size/fd query so the loader sizes correctly. cap-115 unaffected (it tests resolution, not handle size/fd). |
| 2026-06-20 | Dump + dev-log re-read of the 12:17 crash (no launch) | **REFRAME — the crash did NOT move to another file; the SAME thread-config is now SERVED-BUT-REJECTED.** The dev log shows the thread-config's FULL lifecycle clean+immediate before the fatal (0ms gap, same tid=41852): `index-pak-serve result=1` → `PAK_READER read_entry pak="Engine.pak" usize=20096` (kcdx inflated the bytes) → `FOpen how=index-pak result=3` → `Fileno/FReadRaw_byPakIndex/FClose all tag=1` (clean read lifecycle) → `fatalerror_fired err_id=200`. The fatal message is STILL "Error loading thread config '%s'" (a3="%s" template, a4_ptr the file arg — same family as PROBE P). So kcdx now SERVES the file and the engine READS all 20096 bytes through kcdx and closes cleanly — then rejects it. NEW failure class: not file-not-found (Option 2 fixed that — necessary+correct), but the engine loading the thread-config FAILS PARSE/ACCEPT on the bytes kcdx served. The dump's stack region holding the a4 message arg is `????` (minidump didn't capture it); PROBE P2 (the deref) only fires in the MISS arm, which the now-pak-HIT thread-config no longer reaches → it didn't fire (count=0). Checkable unknown: do the bytes kcdx's `FReadRaw_byPakIndex` returns MATCH the real Engine.pak entry content + length (20096)? A short/wrong read = the engine parses garbage → "Error loading thread config" reject. → next probe observes the served bytes vs ground truth. This is a 2nd-symptom axis shift (not-found → served-but-rejected) — `/debug` §B.5 reframe. |
| 2026-06-20 | Option-2 acceptance launch (slot-1 serves the pak hit; cap-115 (d) added; `kcdx-dev_2026-06-20_12-17-45.log`) | **THE THREAD-CONFIG IS FIXED (not-found) — but the `0xC8` persists as a served-but-rejected read (see the re-read row above).** Outcome A confirmed for the thread-config: swap seated (`kcdx_owned=28`), slot-1 SERVED it (`how=index-pak-serve vpath="%engine%/config/engine_core.thread_config" result=1` — returned the `%engine%/` form unchanged), FOpen re-resolved + HIT (`how=index-pak disk="...Engine\Engine.pak" result=3`), `PAK_READER read_entry pak="Engine.pak" usize=20096` — kcdx SERVED the thread-config bytes from Engine.pak, NO `loose_open_failed`. Option 2 works exactly as designed. BUT `fatalerror_fired err_id=200` + `FAULTED code=200` STILL fire — so the crash is no longer the thread-config; a DIFFERENT config/system-init load now fails behind it (the same class — a config file the takeover doesn't yet serve correctly, or a downstream init step). The fatal's `a4_ptr` (the message-arg naming the file) was not deref'd in the log line; → read the dump to name the NEW failing file (PROBE P/P2 method). The fix is NECESSARY and correct; it peeled the first failing config off, revealing the next. cap-115 4/4 PASS (incl. (d)). |
| 2026-06-20 | FIX-1/FIX-2 acceptance launch (`%engine%/`→pak-root strip in ResolveVPath deployed; cap-115 (c) added) | **STILL CRASHES — fix targeted the WRONG key form; cap-115 (c) PASSED but tested the wrong path.** Swap SEATED (`vtable_swapped kcdx_owned=28`, `swap_live_first_open ./system.cfg`), so the run is decisive. cap-115 (c) PASSED (`ResolveVPath("%engine%/config/...")` HITS — the strip works for that input). But the LIVE FOpen never sees `%engine%/`: the boot trace shows (1) slot-1 `kcdx_AdjustFileName` is called with `%engine%/config/engine_core.thread_config` → `ResolveVPath` strips it → finds the **Pak** ByteSource → but slot-1 **intentionally thunks a PAK hit to the original resolver** (open_slots.cpp:328-339: a pak asset has no loose path to return as a string; FOpen owns the pak byte-serve) → the original expands `%engine%`→`engine\` → traced `how=miss-original`; (2) the engine then calls **FOpen with `engine/config/engine_core.thread_config`** (the post-AdjustFileName form, `%engine%` already gone) → FOpen's `ResolveVPath` gets `engine/config/...` → my strip matches only `%engine%/`, NOT `engine/` → index MISS → loose arm → `loose_open_failed engine/config/engine_core.thread_config errno=2` → `fatalerror_fired err_id=200` (0xC8). So: the FOpen lookup key is `engine/config/...`, not `%engine%/config/...`; the strip + cap-115 were built on the AdjustFileName-input form, never the FOpen-lookup form. ROOT of the residual: the alias is expanded to `engine/` BEFORE FOpen by slot-1's pak-hit→original thunk, and FOpen index-resolution does not reconcile the `engine/` (pak-root-alias-expanded) prefix to the pak-relative key. (`kcdx-dev_2026-06-20_11-57-38.log`) |
| 2026-06-20 | A1-fix acceptance launch (`47db4ec` deployed: index walks Data+Engine; cap-115 deployed) | **STILL CRASHES — fix incomplete; a key-NAMESPACE mismatch the index-coverage fix did not address.** The index NOW covers the Engine paks (`asset_index_built entries=307006 roots=2 paks=46`, was 41 Data-only), and cap-115 PASSED (the thread-config resolves to an Engine.pak HIT *under key `config/engine_core.thread_config`*). BUT the engine still fatals: `loose_open_failed engine/config/engine_core.thread_config errno=2` → `PROBE_P fatalerror_fired err_id=200` (same `0xC8`). ROOT OF THE RESIDUAL: Engine.pak stores the entry as `Config/engine_core.thread_config` → indexed under key `config/engine_core.thread_config`. The engine OPENS `%engine%/config/engine_core.thread_config` → kcdx normalizes the lookup to `engine/config/engine_core.thread_config`. **`engine/config/...` (lookup) ≠ `config/...` (index key)** → FOpen index-MISS → loose arm → errno=2 → fatal. cap-115 tested the entry's OWN key, not the engine's actual `%engine%/`-prefixed lookup key — it passed while the engine missed. The `%engine%` alias maps to the `Engine/` pak ROOT; inside that pak the path is pak-relative (no `engine/` prefix). The fix indexed the right paks but did not reconcile the key namespace. Verified by zipfile (`STORED ENTRY NAME: 'Config/engine_core.thread_config'`) + the live FS_BOOT_TRACE lookup key. |
...
| 2026-06-19 | P-static-2: static read of the REAL raise site from the binary (no launch). Derived the raise-site RVA from the DUMP (`base 0x7fffc7850000` + frame-01 return `0x7fffc9ca9a4a` → RVA `0x2459a4a`), NOT the GUARD decimal (`0x245F2CA` = a teardown sweep, wrong). Disassembled the containing function `0x2459810` + read its .rdata message strings. | **DECISIVE — RE-FRAMES the fault. The raising function is `CSystem::FatalError` (CryEngine `CryFatalError`), NOT an NGX/FSR2 assertion.** `rsi`=the `CSystem`/`ISystem` object, `edx`=an error id, `r14d=0xC8` threaded as the code; raise = `call [[handler]+0x58]` → `RaiseException(0xC8)`. The message strings it emits are the CrySystem fatal/config family: `d:\…\CryEngine\CrySystem\System.cpp`, `<CrySystem> Last System Error: %s`, **`Config file '%s' not found!`**, **`Couldn't get length for Config file '%s'`**, `'%s' -> invalid configuration`, `- Fatal Error`. The `ffxFsr2ResourceIsNull`/`NVSDK_NGX_UpdateFeature` frame labels are **nearest-export NOISE** (no PDB → cdb names every addr by the nearest export across a huge span of CrySystem code). So `0xC8` = CryFatalError on a **config/system-init failure**, reached from `C_Game::CreateInstance` — exactly matching the dev-log `loose_open_failed engine/config/engine_core.thread_config errno=2` + the system.cfg/pak.cfg reads. This SUPERSEDES P-static's "deliberate NGX null-resource abort" reading (which was itself built on the nearest-export names). (`_research/ki0026-ngx-raise-site-recon/`) |

## Fix cycle (the owned-%engine%-resolution fix — persisted before building, per plan-persistence; flip each row as it lands)

SETTLED FIX (user 2026-06-20, design fork resolved): **kcdx OWNS `%engine%` alias
resolution at the index-lookup normalization.** The verified mechanism (research-
disassembly reuse ladder, no fresh Ghidra): the engine alias resolver `FUN_180462664`
is a generic prefix-substitution over one CCryPak-global table; `%engine%` is one row,
live-captured by PROBE P2 as expanding to bare relative `engine\`. Engine.pak stores
its entries pak-relative (`Config/...` → key `config/...`). So the FOpen index-lookup
key for `%engine%/config/engine_core.thread_config` (open_slots.cpp:149 —
NormalizeVPath only lowercases + flips slashes, does NOT strip `%engine%`) is
`%engine%/config/engine_core.thread_config`, which MISSES the stored `config/...`.
The owned fix expands `%engine%/X` → pak-root-relative key `X` (DROP the `%engine%/`
prefix), so the lookup HITS the A1-indexed Engine.pak entry, kcdx serves it through
its own reader, the engine never fatals. Option A (drop) is forced by the stored
pak-relative key form (zero cross-pak collisions, asset_index.cpp:98-111). Lives at
the index-lookup sites that consume the engine's `%`-prefixed name: open_slots.cpp,
metadata_slots.cpp, enum_slots.cpp (all via NormalizeVPath / a shared expand step).

### Option 2 chosen (user, 2026-06-20) — slot-1 SERVES the pak hit; gated on probe P3-return

Architect Gate A `forward-and-wait`; user picked Option 2 (kcdx owns the pak resolve→serve, the total-takeover invariant true for engine-pak files), over Option 1 (also-strip-`engine/`, collision-free but a second alias band-aid). Two non-negotiables carried forward: (a) regression test asserts the LIVE FOpen `engine/`-form lookup hits (not just the `%engine%/` input cap-115 tests today); (b) PROBE the `%engine%`-AdjustFileName-return consumers before building (the return-string-breaks-a-caller risk is unverified).

STATIC EVIDENCE so far (results-driven §4, no launch): `_research/adjustfilename-outbuf-recon/FINDINGS.md` — the engine's NATIVE FOpen (0x4614A0) calls AdjustFileName internally and consumes its RETURN string to open. BUT kcdx's FOpen (`OpenResolvedAndMint`, open_slots.cpp:148-153) does its OWN `ResolveVPath(pName)` — it does NOT internally call slot-1. The boot trace shows `slot=AdjustFileName` then `slot=FOpen` as TWO independent kcdx-slot dispatches → graphics-init calls slot-1 standalone (gets the resolved string), then calls FOpen with that string. So if Option 2 makes slot-1 return the ORIGINAL `%engine%/...` for a pak hit, FOpen receives `%engine%/...` and its EXISTING `%engine%/` strip hits — the fix works WITHOUT a second strip. RESIDUAL UNKNOWN (the architect's flagged risk, narrowed not closed): does any boot caller open slot-1's return as a LITERAL loose file without routing through kcdx's FOpen? The trace shows only FOpen consuming it in the window, but the finding read only 3 of N AdjustFileName consumers. → PROBE P3-return.

### Reframe (post-Option-2): not-found → SERVED-BUT-REJECTED. Read-path serving is the new axis.

Option 2 is VERIFIED CORRECT (commit-worthy progress, not closure): the live trace proves kcdx serves the thread-config pak bytes (`index-pak-serve`→`index-pak`→`read_entry usize=20096`→clean Fileno/FReadRaw/FClose tag=1), cap-115 4/4. The `0xC8` persists as a DIFFERENT mechanism: the engine reads kcdx's served bytes and rejects them ("Error loading thread config" fires 0ms after the clean FClose). STATIC GROUND TRUTH (zipfile, no launch): the real `Config/engine_core.thread_config` is 20096 bytes of XML (`<ThreadConfig>...`, DEFLATE csize=3950); kcdx's `read_entry` logged the correct `usize=20096`. So kcdx inflated the right LENGTH — the open question is whether the engine's READ of those bytes through kcdx's read family returns the full correct content, or a short/wrong/mis-sized read the XML loader then fails to parse.

| Probe | Status | One-variable action |
|-------|--------|---------------------|
| P-read | NOT STARTED | Observe the engine's READ of the thread-config through kcdx vs ground truth (20096 XML bytes). Instrument the read path gated to the thread-config handle: log (1) every read-family call's REQUESTED size/count (does the engine ask for 20096, or size-by-handle first?), (2) kcdx's RETURNED byte count, (3) the first/last served bytes (vs `<!--\r\n= Platform names` … `</ThreadConfig>\r\n`). Also log what `Fileno`/size-by-handle returns for handle 3 (the engine may size its parse buffer from it). Outcome A: engine reads 20096 and gets the correct XML → the reject is NOT a read defect (look at how kcdx's handle/size answers a size-query the loader makes, or a different consumed slot). Outcome B: a short read / wrong count / wrong size-by-handle → the engine parses truncated XML → "Error loading thread config"; fix the read-family serving of an inflated pak buffer. Theory-independent: observes the raw bytes+count the engine receives vs the known-correct file. `/debug` continuation. |

| Probe | Status | One-variable action |
|-------|--------|---------------------|
| P3-return | DONE — Option-2 VERIFIED (the fix works; crash reframed, see P-read) | Built AS the fix (slot-1 pak hit returns original `%engine%/...` → FOpen re-resolves via the kept strip). Static reads narrowed it: kcdx FOpen does its OWN ResolveVPath (does not consume slot-1's return internally); the engine calls slot-1 then FOpen as two dispatches; only FOpen consumes the return in the boot window (3-of-N consumers read, `adjustfilename-outbuf-recon`). The live trace renders the verdict: Outcome A `index-pak-serve`→`index-pak` HIT, no 0xC8 → SAFE+FIXED; Outcome B a literal open of `%engine%/...` or FOpen still `engine/...` → a consumer opens the return literal → redesign. Build green, step-review `approve`, deployed+hash-verified. |
| P3-return | NOT STARTED (superseded) | Confirm slot-1's return string is re-resolved by kcdx FOpen, never opened literal. Instrument `kcdx_AdjustFileName`: on the thread-config PAK hit, log the string slot-1 returns (the candidate Option-2 return = original `%engine%/...`) and tag it; then confirm the NEXT FS op on that exact string in the boot window is a `kcdx_FOpen`/`OpenResolvedAndMint` that re-resolves it via the index (HIT), not a literal `_wfopen`/loose open. Outcome A (next op is FOpen→index-HIT) → Option 2 return=`%engine%/...` is SAFE, build it. Outcome B (some caller opens the returned string literal, or no FOpen re-resolve) → returning `%engine%/...` would break that caller; Option 2 must instead have kcdx own the whole resolve→serve (slot-1 serves the pak bytes / a different return form) — redesign. Theory-independent: observes the actual consumer of slot-1's return, falsifies "FOpen always re-resolves it". |

| Step | Status | One-variable action |
|------|--------|---------------------|
| FIX-1 | NEEDS REWORK | The `%engine%/`→pak-root strip in `ResolveVPath` is correct logic but targets the WRONG key form. Live trace: FOpen's index lookup arrives with `engine/config/...` (the alias already expanded to `engine/` by slot-1's pak-hit→original thunk), NOT `%engine%/config/...` — the strip matches only `%engine%/`, so FOpen MISSES → 0xC8 still fires. The strip stays (it's correct for the `%engine%/` form slot-1 sees), but it does NOT fix the FOpen lookup. Rework depends on the surfaced design fork (how kcdx reconciles the `engine/`-expanded-alias FOpen key to the pak-relative stored key — strip `engine/` too? have slot-1 serve the pak hit so FOpen never re-resolves? a normalized-alias index key?). |
| FIX-2 | NEEDS REWORK | cap-115 (c) PASSED but tested `ResolveVPath("%engine%/...")` — the AdjustFileName-input form, NOT the live FOpen-lookup form `engine/config/...`. The test must assert the form the engine ACTUALLY opens through FOpen (`engine/config/...`), which is the falsifiable claim that would have gone RED on this build. Rework alongside FIX-1 once the fork is settled. |
...
| P (live) run 1 | DONE — INCONCLUSIVE (swap never seated this boot) | NO CRASH, but the swap never fired — `cap-108-fs-takeover-seating` stayed PENDING + ZERO `swap_live_first_open`/`vtable_swapped`/`kcdx_vtable_built`/`seating_post_publish` → `HookedConstructStore` never ran → engine kept its native vtable (PROBE-G/J equivalent, always boots clean). Non-deterministic boot path (KI-0019: "no crash" runs common). PROBE P installed fine (`rva=0x2459810`, base `0x7fffc79c0000` verified). Re-launched the SAME build → run 2. |
| P (live) run 2 | **DONE — DECISIVE. ROOT CAUSE NAMED BY THE ENGINE.** | **CRASH, swap SEATED (`vtable_swapped`, `kcdx_owned=28`, `swap_live_first_open ./system.cfg`), PROBE P FIRED.** `PROBE_P fatalerror_fired err_id=200 (=0xC8) a3_str="%s" a4_ptr=0xd232bccc50 boot_window=yes`. The message is one vararg deeper than the probe captured (`a3="%s"` the fmt, `a4`=the real string); deref'd `a4` from the dump (`kcdx_2026-06-19_23-58-27.dmp`, `da`): **`"Error loading thread config '%engine%/config/engine_core.thread_config'"`**. So the `0xC8` is **`CSystem::FatalError` on a FAILED LOAD of `%engine%/config/engine_core.thread_config`** — NOT graphics/NGX (the NGX/FSR2 frame names were nearest-export noise; graphics-init is merely where the thread-config is consumed). This CONFIRMS the dev-log `loose_open_failed engine/config/engine_core.thread_config errno=2` (flagged since PROBE F) was CAUSAL, not incidental: the engine fatals because that config load returns not-found under the takeover. `CSystem::FatalError` is now LIVE-verified as the raise site (P-static-2 confirmed from the binary). REMAINING (one link to a falsifiable AP17 paragraph): why does kcdx's FOpen return not-found for `engine_core.thread_config` when PROBE-G (swap bypassed) boots clean — what about the takeover makes THAT `%engine%`-prefixed path miss? → PROBE P2. |
| P2 (live) | **DONE — DECISIVE. MECHANISM FULLY VERIFIED (AP17 complete).** | CRASH, swap seated, PROBE P fired (0xC8), and PROBE P2 captured: `vpath="engine\config\engine_core.thread_config" resolved_disk="engine\config\engine_core.thread_config" pct_expanded=yes resolved_exists_on_disk=no`. The resolved path is RELATIVE (`engine\config\…`, no drive) and **does not exist as a loose file** (`GetFileAttributesW`=INVALID). Static confirm (no launch): the file does not exist loose ANYWHERE under the game tree — it is **`Config/engine_core.thread_config` INSIDE `Engine/Engine.pak`** (verified via zipfile: `Engine.pak` 972 entries, hit `Config/engine_core.thread_config`). So the index-MISS arm's assumption "miss ⇒ loose file, `_wfopen` it" is WRONG for an engine-pak-resident file: kcdx's asset index covers `<game-root>/Data` paks, NOT `Engine/*.pak`. kcdx returns not-found → engine `CSystem::FatalError(0xC8)` "Error loading thread config". The objdiff_summary `diffs=0` (engine/kcdx/revert all 0) confirms once more it is NOT object state — it is the MISS resolving to a loose path kcdx then can't open because the file is in an engine pak kcdx never indexed. MECHANISM COMPLETE. |
...
- **VERIFIED ROOT CAUSE (PROBE P + P2 + static disk read — every link observed, none inferred).**
  The `0xC8` is `CSystem::FatalError` (CryFatalError) raised because kcdx returns
  NOT-FOUND for `%engine%/config/engine_core.thread_config` — a file that exists
  ONLY inside `Engine/Engine.pak` (as `Config/engine_core.thread_config`, zipfile-
  verified), which kcdx's asset index (built over `<game-root>/Data`) does NOT cover.
  Chain: engine opens the thread-config → kcdx index MISS → miss arm thunks the
  original AdjustFileName → resolves to the RELATIVE loose path
  `engine\config\engine_core.thread_config` (`pct_expanded=yes`) → kcdx `_wfopen`s
  that loose path → `errno=2` (`resolved_exists_on_disk=no`) because there is no
  loose file → kcdx returns a failed open → engine `CSystem::FatalError(0xC8)`
  "Error loading thread config" (PROBE P `a4` → that exact string). The DEFECT: the
  index-MISS arm assumes miss ⇒ loose file (resolve + `_wfopen`), which is WRONG for
  a file resident in an ENGINE pak kcdx never indexed — kcdx indexed the `Data` paks
  but not the `Engine/*.pak` archives the vanilla FOpen body walks. This is the
  totalizing-invariant gap (spec-conformance.md): the takeover owns "every file op"
  but its miss arm silently narrows the long tail to loose-only, leaving engine-pak
  files unreachable. Explains every prior "kcdx clean" probe: the bug is a MISS, not
  a mis-served value (PROBE K/N saw correct values + diffs=0), and PROBE G boots
  clean because the engine's own FOpen body reads the config from the pak. The fix
  is a DESIGN fork (the takeover's engine-pak coverage contract) — surface to the
  user. This COMPLETES the P-static-2 fact below (which named CSystem::FatalError +
  the config-failure family from the binary; P/P2 named the exact file + the miss
  mechanism live). (`_research/ki0026-ngx-raise-site-recon/FINDINGS-P2-mechanism.md`)
...
- BINARY-CONFIRMED, SUPERSEDES the NGX-resource reading of the P-static fact above
  (P-static-2): the raising function is **`CSystem::FatalError`** (CryEngine
  `CryFatalError`, RVA `0x2459810`), NOT an NGX/FSR2 assertion. Derived the raise-site
  RVA from the DUMP (`base 0x7fffc7850000` + frame-01 return `0x7fffc9ca9a4a` → RVA
  `0x2459a4a`); the `ffxFsr2ResourceIsNull`/`NVSDK_NGX_UpdateFeature` frame labels are
  nearest-export NOISE (WHGame has no PDB). The function reads `rsi`=the CSystem obj,
  `edx`=an error id, threads `r14d=0xC8`, and raises via the fatal-handler vtable. Its
  emitted .rdata strings are the CrySystem config/fatal family —
  `d:\…\CryEngine\CrySystem\System.cpp`, `<CrySystem> Last System Error: %s`,
  `Config file '%s' not found!`, `Couldn't get length for Config file '%s'`,
  `'%s' -> invalid configuration`, `- Fatal Error`. So `0xC8` = **CryFatalError on a
  config / system-init failure**, reached from `C_Game::CreateInstance` — matching the
  dev-log `loose_open_failed engine/config/engine_core.thread_config errno=2` + the
  system.cfg/pak.cfg config reads. The fault is the engine's OWN fatal-error path on a
  failed config/system-init step, NOT an NGX null-resource abort. Still consistent with
  PROBE G (swap-write is the trigger) + PROBE N (kcdx's open is byte-identical): the
  swap-write leaves the engine in a state where a later config/system-init step fails.
  (`_research/ki0026-ngx-raise-site-recon/FINDINGS.md`)
