...
| P (live) | pending — OWED (re-targeted by P-static-2) | **No longer an NGX-RVA hook.** P-static-2 identified the raising function as `CSystem::FatalError` (RVA `0x2459810`, a CryEngine function) raising `0xC8` on a config/system-init failure — the message NAMES the resource. P-live now hooks `CSystem::FatalError` itself at the boot window: capture `edx` (error id) + `rsi` (the CSystem obj) + the formatted message / args, to read the exact config/system-init operation it aborts on. Then confirm whether the swap-write makes that operation fail (vs PROBE-G's clean boot, where FatalError is never reached). Theory-independent (observes the actual abort reason). SAFE + kcdx-shaped: a CryEngine function via the resolve-by-name path (an Address Library entry, AP18-gated) or a one-off boot-window detour, NOT an NGX RVA. |
