...
**Status:** open — **resolved by the [file-system takeover](../design/file-system-takeover.md)** (§9; its own track, decoupled from Phase 11 / FIX A — user-decided routing 2026-06-13). The serve-execute confirmation AND the heap-corruption root-cause are re-attempted when the takeover lands, not now.

**Sibling: [KI-0019](KI-0019-ccrypak-fopen-reentrancy-av-on-inventory-open.md)** (cross-CRT `fseek`, FSR2/DLSS-init trigger, inventory-open AV 2026-06-13). Same `src/asset_overlay.cpp` HOOK 2 cross-CRT `FILE*` hazard class as this KI's `fclose` finding — different engine consumer of the same foreign handle. The takeover is **both KIs' shared home** (design §9): kcdx owning every file handle cradle-to-grave on its own CRT retires the `fclose` and the `fseek` cross-CRT instances together, structurally. Both close at the takeover plan's step 4.2.

## Resolution path (settled 2026-06-05; re-homed to the file-system takeover 2026-06-13) — re-attempt when the takeover lands

The crash is NOT root-caused (3 theories falsified; the cross-CRT FClose confirmed-real but not the trigger; the crash tracks cap-78's keyed-but-unopened `overlay_entry`, an unexplained shape). Rather than keep probing the current architecture, KI-0006 is re-homed to the **[file-system takeover](../design/file-system-takeover.md)** (§9; its own track, decoupled from Phase 11 / FIX A — design §11) because:

- **The takeover retires the confirmed-real hazard underneath it, structurally.** The cross-CRT `FILE*` free (PROBE D confirmed WHGame's `fclose` frees kcdx's `/MT` handle) exists because the engine's `ucrtbase` operates a handle kcdx's CRT minted. Under total ownership kcdx owns the engine CCryPak file object and its read family — every handle is minted/read/sought/closed on kcdx's own CRT, and no engine CRT op ever touches a kcdx handle. The boundary the `fclose` crossed is never crossed by construction, so the cross-CRT-free hazard class is removed at the source, not patched (design §9).
- **The takeover owns the serve/resolution seam** — the `asset_overlay.cpp` HOOK 1 + HOOK 2 (the served-`FILE*` path this crash rides) become kcdx vtable slots + the unified index (design §11). A fix attempted against the current two-hook seam risks being rebuilt by the takeover.
- **The takeover gives a kcdx-CONTROLLED, instrumentable serve/read path** — the serve-execute test no longer rides the engine's opaque mod-init loader (whose handle-close behavior is unobservable); kcdx owns the open/read/close lifecycle end-to-end.

NOT a guaranteed fix: KI-0006's specific corrupting write is unidentified and not provably in the takeover's path. Re-homing means the re-attempt happens against the architecture KI-0006 will ship on, with the confirmed cross-CRT hazard structurally addressed and a far more instrumentable serve/read path — not that the takeover auto-fixes it.

**Re-attempt plan (when the takeover lands):** (1) with kcdx owning the read family on a single CRT, re-run the serve-execute confirmation via the kcdx-owned slot path (not a mod-init overlay on the old two-hook seam); (2) if a crash still reproduces, root-cause it against the post-takeover architecture with the cross-CRT variable eliminated — the surviving facts (the `WHGame+0xB2DBA0` mangled-pointer victim, the cap-78-`overlay_entry`-keyed correlation) carry forward as the starting evidence; (3) the falsified theories below (record-synth, re-entrancy, mod-init-serve) stay falsified — do not re-test them.

**Status:** open (the serve mechanism is PROVEN; the EXECUTE-leg confirmation + the heap-corruption root-cause re-attempt when the takeover lands — both KIs close at the takeover plan's step 4.2).
