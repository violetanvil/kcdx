...
| H (2026-06-15, takeover-seating re-verification) | Repro inventory-open with the file-system-takeover vtable swap ACTIVE — does the seating spike alone fix KI-0019? (step 1.4 landed `3be161a`, P2/P4 PASS) | DONE | **STILL REPRODUCED — confirms the seating spike alone does NOT fix KI-0019, exactly as design §9 predicts.** Run 2026-06-15 10:06:34: the swap seated and went live (`FS_TAKEOVER vtable_swapped object=… kcdx_vtable=…`, then `swap_live_first_open first_vpath=./system.cfg` at 10:06:36 — the engine dispatched its first file open into kcdx's slot 36; cap-108 PASS, `ACCEPT-SUITE 1/1`). Then at 10:09:00, WITH the swap active, the inventory-open AV reproduced: 32× `[GUARD] FAULTED_FIRE hook=engine.ccrypak_fopen va=0x7FF95C5314A0` + a MiniDump (`kcdx_2026-06-15_10-06-34.dmp`). cdb fault stack (this session, verified): `ucrtbase!common_fseek_binary_mode_read_only_fast_track_nolock+0x89` → `common_fseek_nolock` → `common_fseek` → `ucrtbase!fseek+0x44` → `WHGame!CreateGameStartup+0x97932` (caller `0x7ff95c53200a`); `EXCEPTION_CODE c0000005`, "Attempt to write to address 0", `FAILURE_BUCKET NULL_POINTER_WRITE_c0000005_WHGame.dll` — the SAME cross-CRT `fseek` mechanism the CORRECTED-mechanism section already pins. **Why expected, not a regression:** the v1 seating spike owns ONLY slot 36 (FOpen) as kcdx; the read family — FSeek (slot 53), FClose (55), FRead, etc. — is still `THUNK(original)` (101 slots thunk in the spike, design §4.3/§4.4), so the read body still runs on the engine's CRT and the cross-CRT straddle on the read path is untouched. The swap proves the seating MECHANISM (the prerequisite); KI-0019 resolves at step 4.2 when the read-family slots flip THUNK→KCDX and kcdx owns the read (design §9 — owning the read family removes the straddle entirely). Forward-looking evidence (the step-4.2 re-verification starting point); KI-0019 STAYS OPEN. Captured: `_research/probe-archive/p2-p4-seating-and-ki0019-persists.md`. |
