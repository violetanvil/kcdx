...
| `kcdxHookInterface` v3 (raw floor under the wrapper; the only path for `Mid` / `Callsite` and the `InsertBefore`/`InsertAfter` statement inserts — register-and-defer, the insert apply path is not wired to fire yet on either surface; every install verb accepts a `kcdxFunctionRef` target via `opts.targetRef`) | Built | `kcdx.hook` raw `{...}` form + `kcdx.hook.insert_before`/`insert_after` | [hook.md](hook.md) |
...
| `kcdxStatementInterface` (static-bytes modification at a located statement — `ReplaceWith` writes a static `kcdxOp`'s bytes natively, zero per-call cost; `InsertBefore`/`InsertAfter` register-and-defer the callback form; consumes the `kcdxOp` / `kcdxLocator` value structs + an optional `kcdxFunctionRef` target via `opts.targetRef`) | Built | `kcdx.statement.*` | [statement.md](statement.md) |
...
| `kcdxFindInterface` (dev-mode function-discovery workbench — find a game function from a string/CVar/call-edge/name it carries) | **NYI** | `kcdx.find` | [find.md](find.md) |
| `kcdxLocator` value struct (where in a function an op applies — author-filled, consumed by `kcdxStatementInterface` and by the `kcdxHookInterface` inserts via `opts.statementLocator`) | Built (value struct); the `Resolve(module, target)` introspection accessor is **NYI** | `kcdx.locator.*` | [locator.md](locator.md) |
| `kcdxOp` value struct (what static change an op makes — author-filled, consumed by `kcdxStatementInterface`) | Built (value struct); the `EmitFor(kind, byteRangeLen)` introspection accessor is **NYI** | `kcdx.op.*` | [op.md](op.md) |
| `kcdxFunctionsInterface` (by-value `kcdxFunctionRef` mints — `GameByName`/`GameById`/`PluginByName` name a game-engine or plugin function → the reference carries address + verified/declared signature directly) | Built | `kcdx.functions.*` | [functions.md](functions.md) |
| `kcdxDllInterface::Declare` (declare your own DLL's functions, signature from source → exposed for cross-plugin hooking by name) | Built | `kcdx.dll.declare` | [dll.md](dll.md) |
