...
| `kcdxLocator` value struct (where in a function an op applies — author-filled, consumed by `kcdxStatementInterface`) | Built (value struct); the `Resolve(module, target)` introspection accessor is **NYI** | `kcdx.locator.*` | [locator.md](locator.md) |
